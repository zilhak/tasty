    ...
    R: Read,
{
    ...
    /// tasty patch: adds `Connection: close`, which `add_header` refuses.
    /// Only `Request::respond_and_close` uses it, together with actually
    /// closing the connection.
    pub(crate) fn with_connection_close(mut self) -> Response<R> {
        self.headers
