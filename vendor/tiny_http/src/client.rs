...
pub struct ClientConnection {
    ...
    // tasty patch: set by `Request::respond_and_close` on a request read from
    // this connection. Once set, no further request is read.
...
impl Iterator for ClientConnection {
    ...
    fn next(&mut self) -> Option<Request> {
        ...
        loop {
            // tasty patch: wait until the previous request released the stream,
            // then stop if that request asked for the connection to be closed.
            // Its unread body was not drained, so the stream is not at the
            // start of a request.
            self.next_header_source.wait_turn();
