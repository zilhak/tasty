...
/// thrown away.
///
/// tasty patch: if the connection's close flag is set when the destructor
/// runs, the remaining bytes are not read — the connection is being closed
/// instead of reused, so there is no next request to align the stream for.
    ...
    R: Read,
{
    ...
    close_connection: Option<Arc<AtomicBool>>,
}
    ...
    R: Read,
{
    ...
    /// tasty patch: skip the drain on drop once `flag` is set.
    pub fn skip_drain_when(&mut self, flag: Arc<AtomicBool>) {
        self.close_connection = Some(flag);
    }
}
...
{
    fn drop(&mut self) {
        if let Some(flag) = &self.close_connection {
