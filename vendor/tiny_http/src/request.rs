...
pub struct Request {
    ...
    // tasty patch: shared with the connection this request was read from.
    // Once set, the body reader skips its drain and the connection reads no
    // further request.
    ...
    W: Write + Send + 'static,
{
    ...
    let reader = if connection_upgrade {
        ...
        if content_length == 0 {
            ...
            let (mut data_reader, _) = EqualReader::new(source_data, content_length); // TODO:
            data_reader.skip_drain_when(close_connection.clone());
...
impl Request {
    ...
    /// Sends a response to this request, then closes the connection without
    /// reading the rest of the request body.
    ///
    /// This is a tasty patch over tiny_http 0.12.0. The response carries
    /// `Connection: close`. The unread part of a `Content-Length` body is not
    /// drained when the request is destroyed, and the connection reads no
    /// further request once this request's body reader has been released.
    /// A body that tiny_http already buffered while building the request
    /// (small `Content-Length` bodies) is released before this call, so on that
    /// path the connection ends when the client honours `Connection: close`.
    pub fn respond_and_close<R>(self, response: Response<R>) -> Result<(), IoError>
    where
        R: Read,
    {
        self.close_connection.store(true, Ordering::Release);
        self.respond(response.with_connection_close())
    }
