...
//! crate 마다 다르다. 기존 번역 카탈로그와 치환 결과의 호환성을 위해 유지한다.
//! 형태 통일 자체는 사용자에게 필요한 정보량을 늘리지 않으며, 기존 키와 placeholder를
//! 함께 바꾸어야 한다. 현재 계약은 `docs/dev-guide/paired-agent-handlers.md`를 따른다.
//!
//! **★ 보존하는 것은 형태 차이지 정보량 손실이 아니다.** [`params::TargetSurfaceError::Malformed`]
...
//! 다르고, 시험은 그쪽을 안 본다.
//!
//! **판정부만 고정해서는 모자란다** — 이 판정을 부르는 쪽이 그 실패를 자기 문구로
//! **덮어쓸** 수 있고, 그러면 여기서 실은 `key` 는 호출자에게 도달하지 않는다. 실제로
//! 훅 경로 두 자리가 그랬다(claude 는 판정부를 안 거치고 두 키를 직접 읽었고, codex 는
//! 거치면서 실패를 `--surface 를 대라` 한 문장으로 덮었다). 그래서 그 두 자리도
//! 각각 자기 시험으로 같은 축을 따로 고정한다 — 이 판정을 거치는 자리는 넷이고,
//! 넷 다 고정돼 있다.
//!
...
//!
//! 아래 두 **응답 shape**도 기존 공개 호출자의 호환성을 위해 유지한다.
//! 저장소 내부에 파싱 소비자가 없다는 사실만으로 외부 호출자의 기대를 바꾸지 않는다.
//! 현재 계약과 검증 범위는 `docs/dev-guide/paired-agent-handlers.md`에 정리한다.
//!
//! | 자리 | claude | codex |
//! |---|---|---|
...
//! **버리기만 한다.** 그래도 그 사실만으로 공개 성공 응답을 바꾸지는 않는다.
//! 호환 변경이 필요한 구체적인 요구가 생기면 외부 호출자와 시험을 함께 검토한다.
//!
...
//! 대응하는 시험도 함께 검토해야 한다. 시험은 두 형상이 같아야 한다고 주장하지
//! 않는다 — 현재 공개 응답과 호스트 응답 변환의 차이를 각각 고정한다.
//!
