...
pub mod populations {
    ...
    pub const SRC_RS: Population = Population {
        ...
        counted_on: super::CountedOn::Tree(
            "9a0b6a1be — 호스트 주입기를 `src/adapters/ipc/host_call.rs` 에서 `tasty-ipc` 로 \
             옮긴 트리에서 다시 셌다. 그 이동이 `src/` 에서 하나를 뺐고, 옮겨간 자리는 \
             `crates/` 라 이 모수 밖이다. 앞선 값 646 은 `353038a3e` 에서 쟀고 그 트리에서는 \
    ...
    pub const DOCS_MD: Population = Population {
        ...
        measured_on: "2026-09-20",
        counted_on: super::CountedOn::Tree(
            "194c8653a — IPC sequence lane 이 ADR 하나를 더한 트리에서 다시 셌다. 앞선 값 454 는 `c7887abcd` 에서 쟀고 그 트리에서는 맞았다. 그 앞의 453 은 `a6aa0b4b3` 에서 쟀고 그 트리에서는 맞았다. 그 앞의 452 는 `194c8653a` 에서 쟀고 그 트리에서는 맞았다. 그 앞의 451 은 `353038a3e` 에서 쟀고 그 트리에서는 맞았다. 그 앞의 450 은 `4e379d048` 에서 쟀고 그 트리에서는 \
