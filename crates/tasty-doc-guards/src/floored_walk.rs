...
pub mod populations {
    ...
    pub const SRC_RS: Population = Population {
        ...
        counted_on: super::CountedOn::Tree(
            "36599e609 — 크레이트 추출 물결이 착지한 트리에서 다시 셌다. **이 모수가 처음 \
             줄었다**(645 → 623, −22). `src/` 에서 사라진 파일은 지워진 것이 아니라 \
        ...
        how: "`src/` 를 뿌리로 `SkipBuildCaches` 순회하고 `rel` 이 `.rs` 로 끝나는 것 전부. \
              ...
              끝나는 줄을 센다 — 이 모수의 미추적 기여분은 0 이다(2026-09-20 재확인: 작업 \
              트리를 `find` 로 센 623 과 추적 트리 계수 623 이 같았다. 645 였을 때도, \
              2026-09-08 에도 같았다). \
              같은 좌변을 `scripts/check-intent-discipline.sh` 가 자기 실패문에 \
              `좌변(src/) 의 .rs N개` 로 찍으므로 **대조가 공짜다** — 두 수가 갈리면 술어가 \
              갈린 것이다(2026-09-20: 둘 다 623).",
    };
    ...
    pub const DOCS_MD: Population = Population {
        ...
        measured_on: "2026-09-20",
        counted_on: super::CountedOn::Tree(
            "36599e609 — 리팩토링 물결 셋이 착지한 트리에서 다시 셌다. 455 에서 30 이 \
             늘었고 그 대부분이 `docs/adr/` 이다. ★ 이 모수가 조용히 거짓이 되는 형태가 \
