...
//! `doc-guards.yml` 이다(ADR-0647). 그 오탐은 "필터 없는 채널이 하나도 없다" 쪽으로
...
/// 메우고 있었다. 이름으로 면제하면 성질이 같고 이름이 다른 것이 통째로 빠진다.
///
...
/// 자체다(`docs/dev-guide/ci-gates.md`의 실행 결과를 보는 단위). 좁은 술어는 자기가 세운 층의 정의와 모순됐다.
///
...
mod step_tests {
    ...
    /// 그 자리에 대한 항진명제가 된다. 아래 YAML 은 실제 워크플로의 *형태*를
