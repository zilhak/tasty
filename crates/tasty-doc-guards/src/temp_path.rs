//! **공유 temp 아래 고정 이름 임시 경로**가 새로 생기는 것을 집는다 — ADR-0644, 공유 임시 경로 격리.
//!
...
//! "쓰이는 것" 으로 만들고, 정당한 예외가 구조적인 곳에서는 지키려는 표보다 빨리
//! 썩는다.
//!
//! ## 유니크화로 인정하는 것
//!
...
//! | 변이 | 스캔 | 유닛 |
//! |---|---|---|
//! | `path_building_lines` 가 항상 빈 vec(당시 이름 `builds_path`) | 빨강 — 다만 `MIN_SITES` 하한이 잡는다 | 9 / 12 |
...
//!
//! **그래서 이 모듈의 보호는 아래 `#[cfg(test)] mod tests` 에 있다.** 스캔
...
/// **관측만 적는다**(저쪽은 소유가 다르다).
///
/// ### 사유 마커 검사를 더 늘리지 않은 이유
///
/// `docs/dev-guide/guard-verification.md`의 검사 도입 기준을 따른다.
///
...
mod tests {
    ...
    /// `docs/dev-guide/guard-verification.md`.
    #[test]
