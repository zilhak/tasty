...
const DOCS_FLOOR: Floor = Floor {
    ...
    why_this_gap: "도달 가능한 창에서 다시 실측(`fdca139c0`..`91ca7d37d`, 558 커밋, \
                   ...
                   의 64 다(2026-09-20 재측정 — 이 수는 안 움직였다). ★ 여유 112 는 그 사건 \
                   하나(64)를 넘어 48 이 더 남는다 — 좌변이 자란 만큼 벌어진 것이고, 하한을 \
                   그대로 두는 한 이 폭은 계속 벌어진다. 이 문장은 좌변이 448 이던 판에서 \
                   110 이라고 적혀 있었다: 모수가 파생이라 **수는 따라왔는데 그 수를 설명하는 \
                   이 문장만 안 따라왔다.** `docs/adr` 259 는 곱수에 안 넣는다 — 그것이 \
