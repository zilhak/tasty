...
const DOCS_FLOOR: Floor = Floor {
    ...
    why_this_gap: "도달 가능한 창에서 다시 실측(`fdca139c0`..`91ca7d37d`, 558 커밋, \
                   ...
                   결론은 같다). 감소 진폭이 관측되지 않았으므로 '진폭 × 몇 배' 를 쓸 근거가 \
                   없고, 대신 아직 안 일어난 사건의 크기에 건다: 문서 카테고리 하나가 접히면 그 \
                   아래 `.md` 가 통째로 빠지고, 지금 트리에서 그 크기의 최대는 `docs/features` \
                   의 64 다(2026-09-22 재측정 — 이 수는 안 움직였다). ★ 여유 221(좌변 559) 은 그 사건 \
                   하나(64)를 넘어 157 이 더 남는다 — 좌변이 자란 만큼 벌어진 것이고, 하한을 \
                   그대로 두는 한 이 폭은 계속 벌어진다. 이 문장은 좌변이 448 이던 판에서 \
                   110 이라고, 좌변이 502 이던 판까지도 117 이라고 적혀 있었다: 모수가 파생이라 **수는 따라왔는데 그 수를 설명하는 \
                   이 문장만 안 따라왔다.** `docs/adr` 364(2026-09-22) 는 곱수에 안 넣는다 — 그것이 \
