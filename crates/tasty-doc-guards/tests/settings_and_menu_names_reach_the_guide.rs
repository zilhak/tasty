...
//! (드래그 결과)가 사용자가 고르는 항목과 같은 구조체에 나란히 있다. 이름으로 가르려고 `lang/ko.toml` 의 `_label` 접미사를
...
/// 재는 사본과 판정하는 사본이 같으면 그 둘이 함께 줄어든다.
...
///
/// ★ 네임스페이스 명부(`MENU_NAMESPACES`)의 *값*은 여기 안 적는다. 명부에서
