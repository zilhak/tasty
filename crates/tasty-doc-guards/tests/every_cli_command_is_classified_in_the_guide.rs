...
///
/// `부채:` 로 시작하는 것은 "없어도 되는 것" 이 아니라 **아직 안 쓴 것**이다. 그 줄을
/// 지우는 방법은 하나뿐이다 — 가이드에 쓰는 것.
const NOT_IN_THE_GUIDE: &[(&str, &str)] = &[
    ...
    (
        "port",
        "부채: 원격 attach 장의 포트 발견 사슬에서 쓰인다. 그 장이 절차만 적고 명령을 안 적었다",
    ),
    (
        "terminal",
        "부채: 에이전트 자식 터미널 관리. 가이드의 에이전트 장이 아직 이 명령을 안 다룬다",
    ),
    ("pty", "부채: 헤드리스 PTY. 위와 같은 자리이고 같은 이유다"),
    ("memory", "부채: 에이전트 메모리 스토어. 가이드에 장이 없다"),
    ("output", "부채: 구조화 출력 관찰자. 가이드에 장이 없다"),
    ("telemetry", "부채: 에이전트 텔레메트리. 가이드에 장이 없다"),
    (
        "clipboard",
        "부채: 로컬 클립보드 조작. 사용자에게 보이는 기능인데 가이드에 없다",
    ),
    (
        "completion-strategy",
        "부채: 완료 판정 전략 레지스트리 조회. 가이드에 장이 없다",
    ),
    (
        "agent-stream",
        "부채: plugin 기여 명령(`tasty-plugin-agent-stream`). plugin 장이 이 명령을 안 적는다",
    ),
];
