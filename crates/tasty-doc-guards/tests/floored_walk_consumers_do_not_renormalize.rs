...
const SCAN_FLOOR: Floor = Floor {
    ...
    why_this_gap: "점 디렉터리와 빌드 캐시 표식을 제외한 Rust 파일 수다. 2026-09-08 b134d28e3에서1339개를 측정했고 미추적 파일 기여는 없었다. 당시1215커밋에서1229→1339로 증가하고 한 커밋의 최대 변화는3이었다. 하한1000의 여유339는 일부 누락을 허용하므로 전수 수집의 증거는 아니다. 옛 측정 SHA는 현재 이력에서 도달할 수 없다.",
};
