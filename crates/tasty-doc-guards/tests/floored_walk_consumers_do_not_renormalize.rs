...
const SCAN_FLOOR: Floor = Floor {
    ...
    why_this_gap: "이 모수는 저장소 전체의 `.rs` 파일 수다. 크레이트가 늘거나 갈리면 한 번에 \
                   수십 개가 움직이므로 여유를 넓게 둔다 — 여기서 잡으려는 사고는 순회가 \
                   통째로 죽는 것이지 파일 수의 변동이 아니다. 값의 계보: 2026-09-06 에 이 \
                   순회가 1286 을 냈다. 같은 날 `find . -name '*.rs' -not -path './target/*'` \
                   는 1289 를 내는데, 차 3 은 `site/target/` 안의 것이다 — 이름이 아니라 \
                   `CACHEDIR.TAG` 로 가지치는 순회는 최상위가 아닌 빌드 디렉토리도 건너뛴다.",
};
