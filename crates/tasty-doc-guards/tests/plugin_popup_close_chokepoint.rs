...
//! ([ADR-0636](../docs/adr/0636-overlay-scope-and-lifetime.md)의 팝업 결과 전달 규칙)는
...
fn close_popup_instance_is_only_called_from_the_drain() {
    ...
    assert!(
        ...
        "`close_popup_instance` 직접 호출은 `{CHOKEPOINT_FILE}` 밖에서 금지다 — \
         큐를 건너뛰면 `cancel_child_file_picker` 연쇄 정리(ADR-0636)가 안 돌아 자식 \
    ...
    for (rel, what) in [
        ...
        assert!(
            ...
            "{rel} ({what}) 가 `{GLUE_FN}` 을 거치지 않는다 — \
             매니저를 직접 치면 연쇄 정리를 건너뛴다(ADR-0636)"
        );
    }
}
