...
const SRC_FLOOR: Floor = Floor {
    ...
    why_this_gap: "src Rust 파일 수의 대량 누락을 검사한다. 2026-09-24 8d5d7a28f에서653개 중 깊이4 이하가428개였다. 하한420은 그 이하이므로 얕은 파일만 수집한 경우에는 별도 깊이 검사가 실패한다. 전체653과의 차이233을 허용 가능한 손실로 해석하면 안 된다. 얕은 파일 수가 하한 아래로 줄면 두 검사의 역할을 다시 확인한다.",
};

/// src를 첫 성분으로 세는 최소 도달 깊이. 총파일 수만으로 놓치는 얕은 재귀를 찾는다.
/// 2026-09-24 8d5d7a28f에서653파일 중 깊이4 이하428·깊이5 이상225·최대깊이7이었다.
/// 파일 수 하한420을 얕은 파일 수 이하에 둬 깊이 검사를 별도로 확인할 수 있게 한다.
