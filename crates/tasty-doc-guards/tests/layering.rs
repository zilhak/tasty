...
const SRC_FLOOR: Floor = Floor {
    ...
    why_this_gap: "이 모수는 `src/` 의 `.rs` 개수다. 이 자리의 여유는 움직임이 아니라 \
                   **계기 사이의 경계**가 정한다. 이 가드에는 하한 말고 깊이 하한과 앵커가 \
                   ...
                   것' 이다. 하한을 깊이 4 이하의 개수보다 높이면 그 절단을 하한이 먼저 \
                   잡아 버려 **깊이 하한이 한 번도 안 물린다** — 안 걸리는 술어는 없는 \
                   술어보다 나쁘다. 그래서 하한을 그 개수 아래에 둔다: 깊이 절단은 깊이 \
                   하한이, 그보다 큰 대량 손실은 이 하한이 잡는다. 실측 2026-09-20 \
                   (`91ca7d37d`): 깊이 4 이하가 **430** 개이고 하한 420 이 그 아래였다. \
                   다시 잰 2026-09-22(`86d08cc85`): 깊이 4 이하가 **420** 개로 하한과 **같다** \
                   — 하한이 그 수보다 높지 않으므로 성질은 유지되지만 남은 폭이 0 이다. 얕은 \
                   파일이 하나 더 빠지면 이 성질이 깨진다. 다시 잰 2026-09-22(`6c3c9a4fc`, \
                   좌변 633): 깊이 4 이하가 **421** 개로 남은 폭이 1 이다. 앞선 판은 이 수를 420 으로 재고(`eea7530d2`) 하한을 \
                   거기 맞췄는데, 그 좌표는 지금 main 에서 도달 불가다. 다시 잰 2026-09-23(`4bace058e`, \
                   좌변 646): 깊이 4 이하가 **426** 개로 남은 폭이 6 이다. 다시 잰 2026-09-23(`00e90bfce`, \
                   좌변 649): 깊이 4 이하가 **428** 개로 남은 폭이 8 이다. 다시 잰 2026-09-24(`d5a894e04`, \
                   좌변 651)도 깊이 4 이하 428 개 · 남은 폭 8 이다. 2026-09-24(`8d5d7a28f`, 좌변 653)는 깊이 4 이하 428개, 남은 폭 8이다. ★ 여유 233(좌변 653) \
                   은 견딜 폭이 아니라 **다른 계기가 맡은 구간**이다 — 그 구간이 비면 이 \
...
/// 나쁘다.
///
/// 다시 잰 2026-09-22(`86d08cc85`, `src/` `.rs` 632): 깊이 4 이하가 **420** 개, 깊이 5 이상이
/// 212 개, 최대 깊이는 7 이다 — 한때 430 까지 벌어졌던 깊이 4 이하가 하한과 같은 자리로 돌아와
/// 위 판정(420 ≥ 420)이 다시 그대로 성립한다. 남은 폭은 0 이다.
///
/// 다시 잰 2026-09-22(`6c3c9a4fc`, `src/` `.rs` 633): 깊이 4 이하가 **421** 개, 깊이 5 이상이
/// 212 개, 최대 깊이는 7 이다. 더해진 `src/app/window_lifecycle/register_focus_tests.rs` 가
/// 깊이 4 라 남은 폭이 1 이 됐다. 재귀가 깊이 4 에서 멈춘 순회는 여전히 하한을 통과하고
/// 여기서 걸린다.
///
/// 다시 잰 2026-09-23(`4bace058e`, `src/` `.rs` 646): 깊이 4 이하가 **426** 개, 깊이 5 이상이
/// 220 개, 최대 깊이는 7 이다. 남은 폭은 6 이고 위 판정(426 ≥ 420)이 그대로 성립한다.
///
/// 다시 잰 2026-09-23(`00e90bfce`, `src/` `.rs` 649): 깊이 4 이하가 **428** 개, 깊이 5 이상이
/// 221 개, 최대 깊이는 7 이다. 남은 폭은 8 이고 위 판정(428 ≥ 420)이 그대로 성립한다.
///
/// 다시 잰 2026-09-24(`d5a894e04`, `src/` `.rs` 651): 깊이 4 이하가 **428** 개, 깊이 5 이상이
/// 223 개, 최대 깊이는 7 이다. 새 프리셋 모듈 둘은 깊이 5 라 남은 폭 8 은 그대로다.
///
/// 2026-09-24(`8d5d7a28f`)에는 파일 653개 중 깊이 4 이하 428개, 깊이 5 이상 225개다.
/// 최대 깊이는 7이며 하한 420과의 차이는 8이므로 깊이 검사는 여전히 필요하다.
///
