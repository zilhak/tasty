...
const MARKER_FLOOR: Floor = Floor {
    ...
    why_this_gap: "★ 이 좌변은 **커밋에서 재현되지 않는다.** 다른 하한들과 \
                   ...
                   실측으로도 거짓이 되지 않아 폭을 아무것도 안 정한다. (같은 모듈의 \
                   `DOCS_DIR_FLOOR` 는 이 문제가 없다. `docs/` 아래에는 빌드 산출물도 로컬 \
                   폴더도 안 생겨서 추적 계수와 순회 값이 같다.)",
};
...
const TARGET_FLOOR: Floor = Floor {
    ...
    why_this_gap: "이 모수는 통합 테스트 타깃의 수 — 두 항의 합이다. 여유는 두 \
                   ...
                   **0 회**라 합의 관측 최대 감소도 16 이며, ★ 여유 49 는 그 **3.06 배**다 — \
                   세 번 겹쳐도(48) 견디고 1 남으며, 네 번 겹치면(64) 짖는다(2026-09-23, 합 171 = \
                   루트 46 + 크레이트 125 — 판정은 `walked < min` 일 때만 실패하므로 171 − 48 = 123 \
                   ≥ 122 다). 이 문장은 합이 171 이 되도록 '여유 45 · 2.81 배 · 세 번 겹치면(48) \
