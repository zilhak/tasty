...
//!
//! ## 점으로 시작하는 디렉토리는 **일부러 본다** (2026-09-08 판단, 실측 근거 있음)
//!
//! 공용 순회에는 점 디렉토리까지 자르는 변형이 있고, 레포 루트부터 훑는 자리는 대개
//! 그것을 써야 한다 — 커밋되지 않는 작업 폴더가 기계마다 다른 수를 만들기 때문이다.
//! **이 가드는 그 예외다.** 물음이 "이름만 보는 가드도 자르는 이름 아래에 있나" 이고
//! 그 이름 교집합은 `target` 과 `.git` 뿐이다. 점 디렉토리 안의 빌드 캐시는 **이름만
//! 보는 가드가 못 자른다** — 그러니 여기서 점을 자르면 위험을 바로 그 자리에서
//! 안 보게 된다. 거짓 안심이지 절약이 아니다.
//!
//! 실측이 그것을 뒷받침한다. 같은 프로브를 이름의 **점 하나만 바꿔** 레포 루트에 두 번
//! 심고 doc-guards 전수를 돌렸더니, 두 경우 모두 빨개진 타깃은 **이 하나뿐이고 같다.**
//! 즉 점 여부는 이 좌변의 판정을 **아무것도 바꾸지 않는다** — 점을 자르면 잃기만 한다.
//! (같은 실측에서 그 디렉토리를 **연** 통합 타깃은 80 중 11 이었다. 열고도 조용한
//! 열이 지금은 무해하지만, 그 술어가 넓어지는 날 모수에 들어온다.)
//!
//! ★ 아래 실패문은 이 빨강이 "이 기계의 파일시스템 상태" 에 귀속된다고 말한다. 그
//! 상태에는 축이 하나 더 있다 — **worktree 냐 원본 저장소냐.** 작업 폴더가 worktree
//! 에서는 레포 밖을 가리키는 심볼릭 링크라 순회가 안 들어가고(공용 순회가 링크를
//! 끊는다), 원본 저장소에서는 실물이라 들어간다. 그래서 같은 커밋이 **원본에서만**
//! 빨갈 수 있다. 확인은 그 폴더 안에 표식이 있는지 보는 것으로 끝난다 — 2026-09-08
//! 실측으로는 없었다.
...
/// 중 여기는 채널이다: 하한도 그 논거도 서는데, 그것이 **혼자 짖는지**를 재는 수단이 없다.
/// `Pick` 에 "안 담고 안 내려가기" 갈래가 없어(`Skip` 은 내려가고 `Take`·`TakeAndStop` 은
/// 담는다) 순회를 얕게 만드는 모든 변이가 표식 목록도 함께 바꾼다. 갈래를 하나 더 만들면
/// 잴 수 있지만, 재려고 재는 대상을 바꾸는 것이라 안 한다.
///
/// **되돌아올 조건**: `Pick` 에 그 갈래가 **다른 이유로** 필요해질 때. 그때 이 프로브는
/// 공짜가 되므로 그 자리에서 대조를 남긴다. 그 전에는 이 하한의 증거가 "새 값만 죽는다" 가
/// 아니라 "같은 변이에서 죽는 이유가 다르다" 까지라는 것을 알고 읽어야 한다.
...
const MARKER_FLOOR: Floor = Floor {
    ...
    why_this_gap: "★ 이 좌변은 **커밋에서 재현되지 않는다.** 다른 하한들과 \
                   ...
                   2026-09-08(`b134d28e3`): 추적 트리에서 나오는 디렉토리는 369 인데 같은 커밋의 \
                   작업 트리를 실제로 순회하면 390 이었다(차 21). 그 21 은 빌드를 한 번 \
                   돌렸는가에 따라 달라지므로 어떤 값을 적어도 다른 기계에서 틀린다. 그래서 여기 \
                   적힌 369 는 순회의 값이 아니라 **추적 트리에서 잰 값**이고, 그 성질이 \
                   하한으로는 오히려 맞다: 미추적 디렉토리는 더하기만 하므로 추적 계수는 순회 \
                   결과의 하계다. 재는 법: `git ls-tree -r --name-only <rev>` 의 경로에서 나오는 \
                   디렉토리를 센다. 여유의 크기는 따로 잰다. 추적 디렉토리 전체의 움직임은 \
                   `8bdbf1bdb` 직전 1215 커밋에서 361..369 였고 감소는 -1 이 두 번뿐이다 — 이 \
                   창에서 크레이트 분해는 한 번도 관측되지 않았다. 그래서 곱수를 관측 진폭이 \
                   아니라 아직 안 일어난 사건의 크기에 건다: 지금 트리에서 크레이트 하나가 \
                   통째로 접힐 때 빠지는 디렉토리의 최대가 10(`tasty-cli`)이고, 여유 30 은 그 세 \
                   배다. 앞선 판은 여기에 '여유를 절반쯤' 이라고만 적었는데, 절반은 어떤 \
                   실측으로도 거짓이 되지 않아 폭을 아무것도 안 정한다. (`no_checkbox_in_docs` 의 \
                   `DOCS_DIR_FLOOR` 도 같은 성질이다. `docs/` 아래에는 빌드 산출물도 로컬 폴더도 \
                   안 생기지만, 문서를 평범한 `rm` 이나 `git mv` 로 지우면 비워진 디렉토리가 \
                   작업 트리에 남는다 — git 은 빈 디렉토리를 추적하지 않고, 비워진 디렉토리를 \
                   함께 치우는 것은 git 이 직접 파일을 지울 때(`git rm` · checkout)다. 그러면 순회 값이 추적 계수보다 커지고 clean \
                   clone 은 추적 계수를 본다. 2026-09-23 문서 통폐합 회차가 그 형태였다: 작업 \
                   트리의 로컬 게이트가 초록인데 clean clone 이 빨갰다. 그래서 그 자리도 추적 \
                   계수로 잰다.)",
};
...
const TARGET_FLOOR: Floor = Floor {
    min: 122,
    // 이 좌변은 두 모수의 **합**이다. 합을 손으로 적어 두었더니 105 로 남았고, 두 항이
    // 39·101 로 움직이는 동안 합만 그 자리에 있었다. 이제 항에서 파생한다.
    ...
    why_this_gap: "이 모수는 통합 테스트 타깃의 수 — 두 항의 합이다. 여유는 두 \
                   항의 감소를 합쳐 잡는다: 도달 가능한 창(`fdca139c0`..`91ca7d37d`, 558 커밋, \
                   직선)에서 루트 쪽 최대 감소가 **16** 이고 크레이트 쪽은 같은 창에서 감소가 \
                   **0 회**라 합의 관측 최대 감소도 16 이며, ★ 여유 50 은 그 **3.13 배**다 — \
                   세 번 겹쳐도(48) 견디고 2 남으며, 네 번 겹치면(64) 짖는다(2026-09-23 `cc2e5e72e`, \
                   합 172 = 루트 46 + 크레이트 126 — 판정은 `walked < min` 일 때만 실패하므로 \
                   172 − 48 = 124 ≥ 122 다). 직전 판은 합 171 = 루트 46 + 크레이트 125 에서 '여유 49 · \
                   3.06 배 · 1 남는다' 였다. 그 앞에서는 합이 171 이 되도록 '여유 45 · 2.81 배 · 세 번 겹치면(48) \
                   짖는다'(합 167 = 루트 43 + 크레이트 124 에서 잰 값)에 남아 있었다 — 이 좌변을 빌려 \
                   쓰는 자리가 모수를 다시 잰 회차의 파생 자리 목록에서 빠졌기 때문이다. 그 앞에서는 \
                   합이 166 이 되도록 '여유 33 · 2.06 배 · 두 번 \
                   겹치면 짖는다' 에 남아 있었다. 앞선 판은 그 값을 `8bdbf1bdb` 직전 \
                   1215 커밋에서 루트 6 · 크레이트 0 으로 재고 여유를 세 배라고 적었는데, 그 \
                   좌표는 지금 main 에서 도달 불가다. 그 뒤 크레이트 항이 101 에서 113 으로 \
                   자라는 동안 이 문장의 '여유' 만 18 에 남아 있었다. 그 뒤 크레이트 항이 113 에서 \
                   115 로 더 자라는 동안 이 문장은 이번에는 32 에 남아 있었다 — **같은 일이 \
                   같은 자리에서 두 번 났다.** 항이 파생이어도 그 \
                   항을 설명하는 산문은 따라오지 않는다.** 앞선 판은 여유를 '넓게' 라고만 적고 하한의 \
                   목적을 '0 이 아님을 보장' 이라고 썼는데, 그러면 하한이 1 이어도 같은 문장이 \
                   참이라 폭을 아무것도 안 정한다 — 이 수가 판정이 아니라 실패문의 정보로 \
                   쓰인다는 것은 그 정보가 절반만 모인 순회에서 나와도 된다는 뜻이 아니다. 값의 \
                   계보: `ls tests/*.rs crates/*/tests/*.rs` 가 세는 것과 같은 두 모수의 합이고, \
                   그래서 합을 적지 않고 항에서 파생한다. 이 순회가 같은 것을 보는지는 수가 \
                   아니라 `the_target_walk_sees_both_roots_and_stays_flat` 이 확인한다 — 수를 \
                   박으면 낡고, 뿌리와 깊이는 안 낡는다",
};
