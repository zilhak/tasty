...
/// ```
///
/// 실측 2026-09-07(회차 85 base `4d5af2e05` + 가드 이동 배치 4):
/// 루트 **36** + 크레이트 **86** = **122** 이고 이 시험도
/// 122 를 센다. 09-06 의 99(52+47) → 09-07 의 111(53+58) → 지금 122 다.
/// **두 뿌리를 따로 세는 것이 중요하다** — 합만 보면 한쪽이 죽고 다른 쪽이 늘어난 것을
/// 못 가른다. 실제로 지금 그 형태가 진행 중이다: 가드를 `-p tasty` 에서
/// `crates/tasty-doc-guards/tests/` 로 옮기는 작업이 **합을 안 바꾸면서 두 뿌리를
/// 반대 방향으로** 움직인다.
///
