...
//!
//! ### 선언 경계 가드는 세우지 않는다 (2026-09-09 판정)
//!
//! 한때 제안된 형태가 있다 — *"`tests/**` 에서 `Mutex<` 를 선언하는 것은 `spawn_diag`
//! 안에서만"* 이라는 경계 가드. 그 제안은 **허용 목록이 한 줄로 끝난다**는 전제 위에
//! 서 있었는데, 그 전제가 지금 트리에서 성립하지 않는다: `spawn_diag` 밖의 선언이
//! 셋이다 — `tests/shared_instance_harness.rs` 의 `OBSERVED`, `tests/gui_common/mod.rs`
//! 의 `SHARED_INSTANCE`, `tests/attach_common/mod.rs` 의 `WRITE_LOCK`. 셋 다 정당하다.
//!
//! 그래서 세우면 **면제 셋에서 시작한다.** 바로 아래 "면제가 자라는 것 자체가 실패다"
//! 가 경고하는 경로의 출발점에 이미 서 있는 가드가 된다.
//!
//! 더 중요한 것은 술어가 **다른 물음**에 답한다는 것이다. 이 축의 결함은 선언이 아니라
//! *락을 쥔 범위 안에서 패닉하는 것*이고(아래 [`EXEMPT`] 의 사유가 그 한 축이다),
//! `Mutex<` 를 선언했다는 사실은 그것과 무관하다. 새 하네스가 자기 락을 들이는 것이
//! 실제로 위험해지는 지점은 **그 락을 어떻게 잡는가**이고, 그 자리는 이 파일이 이미
//! 본다: 생 `.unwrap()` 으로 잡으면 위반으로 잡히고, [`MIN_ACQUISITIONS`] 가 획득 자리
//! 수를 양방향으로 본다.
//!
//! 다시 열 조건: `spawn_diag` 밖 선언이 **다시 한 자리 이하**가 되거나, 선언 자체가
//! 획득 형태와 무관하게 결함을 만드는 사례가 실측되면.
//!
//! ## 면제가 자라는 것 자체가 실패다
//!
