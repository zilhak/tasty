//! **통합 테스트 하네스의 공유 락 획득**에서 사유 없는 생 `.unwrap()` 을 거부한다.
//!
//! ## 왜 (실측된 기전)
//!
...
/// 획득 자리 수의 하한. 토큰 오타·마스킹 실패·뿌리 소실로 수가 0 근처로
/// 떨어지면 실패한다. 정확한 수를 양방향으로 고정하는 래칫이 아니다 — 하한 이상인
/// 증감은 통과한다. 비면제 생 `.unwrap()` 이 없어도 분모가 살아 있는지 보는 검사다.
///
