...
//! 무시 대상인 것**이 없는지 본다.
//!
//! ADR-0647 이 가른 것이 이 구분이다. 읽는 것이 전부 `docs/**` 인 가드는 **문서만 담은
...
/// 그 크레이트를 링크해야 해서 의존 0 인 그 자리에 못 들어간다(ADR-0647 이 그 크레이트를
...
/// 순간 그 잡이 없는 타깃을 부르며 죽는다. 그래서 ADR-0647 이 그 자리를 "안 옮긴다" 로
...
/// 안 떠도 그 워크플로가 뜬다. `changelog_unreleased` 가 그 형태이고, ADR-0647 이 그
...
fn no_filtered_scan_guard_reads_only_ignored_paths() {
    ...
    assert!(
        ...
        "아래 가드는 읽는 경로가 **전부** `{WORKFLOW}` 의 paths-ignore 안이라, 자기가 \
         ...
         (ADR-0647). 크레이트 상수를 링크해야 해서 못 옮기겠으면, 그 상수를 소스 텍스트로 \
