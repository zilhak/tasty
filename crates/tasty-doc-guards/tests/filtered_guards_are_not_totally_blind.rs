...
//!
//! 공용 순회 래칫(`scripts/check-shared-walk-ratchet.sh`)이 세는 **직접 `read_dir(` 56 건**
//! 을 실패 처리로 갈랐다:
//!
...
//! 않았다.**
//!
//! 총합 56 은 안 움직인다 — 그 래칫은 **철자**를 세지 실패 처리를 안 본다. 셋이 옮겨간
...
//!
//! ★ 그래서 그 래칫의 상한 56 은 **"여기까지는 봐준다" 가 아니라 "여기까지는 조용히
...
///
/// 실측 2026-09-07(회차 85 base `4d5af2e05` + 가드 이동 배치 4): 이 수 **72** ·
...
/// 순간 그 잡이 없는 타깃을 부르며 죽는다. 그래서 ADR-0138 이 그 자리를 "안 옮긴다" 로
