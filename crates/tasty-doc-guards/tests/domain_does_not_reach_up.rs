...
//!    보였다([ADR-0002](../../../docs/adr/0002-domain-execution-and-ports.md)).
...
//! 안 들어간다(layering 가드가 같은 이유로 테스트를 뺀다 — ADR-0048).
//!
...
//!   크레이트를 떼는 날 그 경로가 경계를 넘는다 — ADR-0002 의 재검토 조건이 그 값을 잰다.
...
/// 그 뒤 headless 가 mirror 구조 op 를 forward 큐에 넣지 않고 거절하게 했다(ADR-0003, +6).
...
/// 대부분 "headless 에 소비자가 없다"(ADR-0003)이고 그 판정 자체는 정당하다. 이 못이 막는
...
fn the_domain_does_not_name_an_upper_layer() {
    ...
    assert!(
        ...
        "도메인(`src/core` · `src/ports`) 출하 코드가 상위 계층을 이름으로 부른다:\n{}\n\
         도메인은 조립·어댑터·GUI 를 모른다(ADR-0002). 처방은 방향을 뒤집는 것이다:\n\
...
fn gui_gates_in_the_domain_are_pinned() {
    ...
    assert!(
        ...
        "도메인 출하 코드의 `feature = \"gui\"` 가 {total} 개로 늘었다(고정값 \
         ...
         먼저 물어라: 이 항목은 headless 에 소비자가 없어서 가리는가(ADR-0003 — GUI 전용 정의 제외), \
...
fn the_domain_names_no_gui_crate_beyond_the_pinned_list() {
    ...
    assert!(
        ...
        "도메인(`src/core` · `src/ports`) 출하 코드가 GUI 크레이트를 부른다:\n{}\n\
         도메인에 WebView·egui·OS 창 타입이 들어오면 안 된다(ADR-0002). gui 게이트 \
