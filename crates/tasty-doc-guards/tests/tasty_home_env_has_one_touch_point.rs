...
//!
//! # 우회 형태가 **셋**이고, 지금은 막는 것이 다르다
//!
//! ```text
//! ① 락을 밖에서 획득한다        → 컴파일러가 막는다 (선언이 모듈 비공개다)
//! ② 락 없이 그냥 set_var 한다   → 이 가드가 막는다  (락이 필요 없는 경로다)
//! ③ 같은 이름의 락을 따로 세운다 → 이 가드가 막는다  (타입이 못 보는 나머지 절반)
//! ```
//!
//! ③ 이 ① 의 자리를 물려받았다. 락이 둘이면 서로를 안 기다려서, 직렬화는 한 개도 없는
//! 것과 같아진다 — 그런데 그 형태는 **컴파일된다.** 그래서 `LOCK_IDENT` 갈래는 승급 뒤에도
//! 할 일이 있고, 다만 그 갈래가 잡는 것이 ① 에서 ③ 으로 바뀌었다.
//!
...
//! 그 자리를 지키던 다른 가드의 시험을 함께 지웠다.
//!
//! 그래서 이 파일은 층이 둘이다:
//!
//! ```text
//! 첫 그물   가시성이 되돌려진다        → the_lock_declaration_stays_module_private
//! 두 번째   되돌린 뒤 실제로 획득한다  → LOCK_IDENT 갈래 (= 승급 전의 ①)
//! ```
//!
//! 두 번째만 있으면 **두 번 틀려야** 발화한다. 첫 그물이 한 번째에서 잡는다.
//!
//! # 모수가 `src/` 인 이유
//!
//! 루트 크레이트에 `lib` 타깃이 생겼지만 `TASTY_HOME_ENV_LOCK` 은 `pub(crate)` 라 여전히
//! 밖으로 안 나간다. 통합 테스트(`tests/`)는 그 락을 아예 못 잡고,
