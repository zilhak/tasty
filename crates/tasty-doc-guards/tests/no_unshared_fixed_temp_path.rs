...
//! 본다(ADR-0045, 공유 임시 경로 격리). 판정 규칙·극성·유니크화 인정 기준·잡지 못하는 사각은
...
//! ## 0 을 통과로 만들지 않는다
//!
//! 스캔이 죽으면 모수가 0 이 되고 0 은 언제나 초록이다(ADR-0048). 위반 목록이 비었다는
    ...
    if !c.silent.is_empty() {
        out.push(format!(
            "공유 temp 아래 고정 이름 임시 경로가 {} 곳 있다. 인스턴스/완주가 동시에 살면\n\
             같은 파일을 truncate 하거나 서로의 디렉터리를 지운다(ADR-0045, 공유 임시 경로 격리).\n\
