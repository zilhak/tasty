...
//! 본다(ADR-0644, 공유 임시 경로 격리). 판정 규칙·극성·유니크화 인정 기준·잡지 못하는 사각은
...
//! ## 0 을 통과로 만들지 않는다
//!
//! 스캔이 죽으면 모수가 0 이 되고 0 은 언제나 초록이다(ADR-0647). 위반 목록이 비었다는
...
/// 경로를 짓는 `.join(` 의 **수신자로 안 이어져** 자리로도 안 세어진 `temp_dir()` 줄의
/// 수. 하한이 아니라 **양방향 래칫**이다 — 실측 2026-09-06: 4(2026-09-08 재측정: 그대로).
///
/// 왜 이 칸이 필요한가: 여기 떨어지는 자리는 위반으로 잡히지 않고 **조용히 빠진다.**
/// 한때 이 칸에는 부류가 셋이었고 그중 하나가 **창을 넘겨 경로를 짓는 자리**였다.
/// 그때 처방은 창을 넓히는 것이 아니었다 — 실측(계단 11 점)으로 6~24 는 완전히
/// 평평하고, 40 과 400 에서 붙는 넷은 전부 **가짜 짝**이다(`valid.join(", ")` 같은
/// 문자열 join, 28 줄 아래 다른 테스트의 `.join(`). 400 에서는 `reasoned` 가 6 → 2 로
/// 무너진다. 넓히기가 답이 아니었던 이유는 물음이 거리가 아니었기 때문이고, 그래서
/// 판정을 **수신자 신원**으로 옮겼다(`temp_path::path_building_lines`). 그 부류는
/// 이제 이 칸에 안 떨어진다 — 남은 둘은 읽기 전용 전달과 **값이 파일 밖으로 나가는
/// 것**이다. 옮긴 날 이 수는 안 움직였다(4 → 4). 그래도 값이 아니라 **움직임**을
/// 지키는 이유는 남은 두 부류를 소스만으로 못 가르기 때문이다.
///
...
/// `sites` 중 자리의 줄들(경로를 짓는 줄 + 자리 둘레 `UNIQ_WINDOW` = 6) 안에 `process` 축은
    ...
    if !c.silent.is_empty() {
        out.push(format!(
            "공유 temp 아래 고정 이름 임시 경로가 {} 곳 있다. 인스턴스/완주가 동시에 살면\n\
             같은 파일을 truncate 하거나 서로의 디렉터리를 지운다(ADR-0644, 공유 임시 경로 격리).\n\
