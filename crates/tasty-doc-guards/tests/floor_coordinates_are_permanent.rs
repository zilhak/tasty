...
//!
//! 앞 회차는 규율대로 값 옆에 잰 트리를 적었다. 그런데 회차가 **체리픽으로 착지**하므로
...
//! 누군가 규약을 안 지킨 것이다.
//!
//! ★ **도달 가능성 자체는 안 잰다 — 이를 자동으로 확인할 실행 경로가 없기 때문이다.** 그것을 재려면
...
// cited_anchors_resolve의 모수를 통합 기준 트리에서 다시 측정해 임시 좌표 한 곳을 없앴다.
const LANE_TIP_SITES: usize = 2;
