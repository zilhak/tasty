...
//! (`doc-guards.yml`). 배경은 `crate::lib` 주석과 ADR-0048.
//!
//! ## 0 을 통과로 만들지 않는다
//!
//! 스캔이 조용히 죽으면(경로 오타·순회 중단) 모수가 0 이 되고 0 은 언제나 초록이다
//! (ADR-0048). 그래서 위반 목록이 비었다는 단언 **앞에** 훑은 파일 수·집은 복구 자리
...
fn no_shipping_lock_is_recovered_without_a_report() {
    ...
    assert!(
        ...
        "test-only 파일로 뺀 자리가 {} 곳뿐이다(하한 {MIN_TEST_ONLY}) — 선언 기반 판정이 \
         ...
         (`tasty_doc_guards::shipping_scope::test_only_files`, ADR-0048 이 그것을 정본으로 \
