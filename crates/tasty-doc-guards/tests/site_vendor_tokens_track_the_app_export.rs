...
//!   한다. 수가 아니라 이름이다: 명부 크기가 유지되면서 **내용이 바뀌는** 것도 잡는다.
...
//!
//! **이웃이 하나 있다.** `site_vendor_icons_match_the_app_transcription.rs` 가 같은 사본의
//! **아이콘 기하와 그 그릇**(`viewBox` · 선 굵기 · cap/join)을 같은 방식으로 본다. 둘을 합쳐도 위 문단은 그대로다 — 토큰도 아이콘도
//! 안 건드리는 결정은 여전히 두 판정기 모두의 좌변 밖이다.
//!
...
const LAGGING: &[(&str, &str)] = &[
    ...
    ("component", "preset-cfg-dim-opacity"),
    ("component", "preset-cfg-draft-fg"),
    ("component", "preset-cfg-field-gap"),
    ("component", "preset-cfg-footer-height"),
    ("component", "preset-cfg-footer-padding-x"),
    ("component", "preset-cfg-form-max-width"),
    ("component", "preset-cfg-form-padding"),
    ("component", "preset-cfg-header-height"),
    ("component", "remote-filter-menu-width"),
    ("component", "segtoggle-on-bg"),
    ("component", "segtoggle-on-fg"),
...
/// 값은 현재 규모(앱 818 · 사이트 791)보다 한참 아래로 잡는다. 이 수는 판정이 아니라
