...
//! 본다(ADR-0045, 프로세스 환경 격리). 판정 규칙·극성·직렬화 인정 기준·잡지 못하는 사각은
...
//! `#[test]` 격리. 명부를 안 쓰는 이유는 temp_path 와 같다.
//!
//! ## 왜 리포 전역 한 자리인가
//!
...
// 실측(2026-09-07): files=1309 · mutations=25 · serialized=25 · bare=0.
//
...
//
// 실측(2026-09-07): src 598 · crates 672 · tests 39 (합 1309 — 이 타깃이 `--nocapture` 로
// 직접 찍는 값이다).
//
...
//
// ★★ `tests` 뿌리만 여유가 3/4 이 아니라 **절반**이다. 다른 둘과 달리 이 뿌리는 **설계상
//    줄어드는 중**이라 — 의존 0 가드를 `crates/tasty-doc-guards/tests/` 로 옮기는 작업이
//    진행 중이고 실측으로 61 → 39 까지 내려왔다 — 3/4 여유를 두면 이동 몇 번마다 이 값을
//    다시 내려야 한다. 그렇게 내리는 값은 뿌리 생존 검사가 아니라 **세금**이 되고, 세금은
//    낼 때마다 느슨해진다. 위 문단이 말한 그대로다: 뿌리가 빠지면 0 이므로 낮은 하한도
//    똑같이 문다. 여유를 넓히는 쪽이 이 자리가 답하는 물음을 지킨다.
...
fn every_test_env_mutation_is_serialized() {
    ...
    assert!(
        ...
        "직렬화 증거 없이 프로세스 전역(env/cwd)을 만지는 테스트 자리가 {} 곳 있다.\n\
         병렬 cargo test 에서 서로의 상태를 덮어 순서 의존 flake 를 낳는다(ADR-0045, 프로세스 환경 격리).\n\
