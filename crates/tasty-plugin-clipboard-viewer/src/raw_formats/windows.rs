//! Windows raw 클립보드 포맷 열거/읽기(`clipboard-win`, arboard 와 동일 버전, TODO50).
//!
...
/// 가 실제로 읽는 건 `CF_UNICODETEXT` 뿐이지만, 셋 다 "텍스트"라 전부 제외한다(TODO50
/// "기타는 배타적 카테고리가 아니다" — 단일 ID 비교 금지).
...
pub(super) fn read_other() -> Vec<OtherFormatEntry> {
    ...
    for id in EnumFormats::new() {
        ...
        match raw::get_vec(id, &mut buf) {
            ...
            // 프로세스가 읽을 수 없는 핸들 타입 — 이 포맷만 건너뛴다(TODO50 개별
            // 격리 요구사항, 전체 "기타" 열거를 실패시키지 않음).
