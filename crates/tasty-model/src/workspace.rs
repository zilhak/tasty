...
///   디렉토리**(docs/dev-guide/attach-behavior.md#커스텀-이벤트-확장-streamcontrol-밖-raw-json-event-태그, 전체 탭이 아니라 활성 탭만). browse-only 원격
///   mirror 대상(docs/dev-guide/attach-behavior.md#커스텀-이벤트-확장-streamcontrol-밖-raw-json-event-태그).
/// - `content_candidates`: `Surface::attach_content_info()` 가 `Some` 을 반환한
///   surface `(surface_id, kind, plugin_id, file)` — 렌더 결과가 아니라 **원문**을
///   나르는 mirror 대상(docs/dev-guide/attach-behavior.md#markdown-content-채널). `mesh_candidates` 와 **같은 이유로 여기서 최종
            ...
            if let Some(pane) = self.pane_layout().find_pane(pane_id) {
                for tab in &pane.tabs {
                    tab.for_each_surface(&mut |s| {
                        ...
                        // image/mesh_demo 분류가 안 바뀐다(docs/dev-guide/attach-behavior.md#markdown-content-채널).
