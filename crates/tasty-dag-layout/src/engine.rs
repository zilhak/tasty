//! rust-sugiyama 호출을 이 모듈에 한정하고 공개 타입으로 변환한다.
//!
//! # 축 이름
//!
...
//!
//! # rust-sugiyama 0.4.0의 처리 조건
//!
//! - 사이클의 역방향 엣지는 라이브러리가 뒤집는다. self-loop는 처리하지 못하므로 호출 전에 제거한다.
//! - dummy vertex 좌표는 반환되지 않아 꺾임점은 crate::routing에서 계산한다.
//! - 반환 ID는 입력 순서의 인덱스다. 0..n 순서로 넣어 공개 인덱스와 일치시킨다.
...
fn call_library(
    ...
    let config = Config {
        ...
        // Up은 모든 소스를 레이어 0에 놓고 실노드 없는 중간 레이어를 만들지 않는다.
        // 따라서 y 좌표 순서로 복원한 레이어 번호가 엣지가 지나는 단계와 일치한다.
        ranking_type: RankingType::Up,
        c_minimization: CrossingMinimization::Barycenter,
        // 교차 감소 패스의 이전 release/x86_64 측정: 노드 200개는 7→22 ms, 500개는 약 7→455 ms.
        // 큰 그래프에서는 이 추가 비용을 피하도록 노드 수 상한을 둔다.
...
/// 같은 레이어의 최소 카드 간격을 확보한다. 임계값에 여유를 둬 부동소수 오차로
/// 정상 좌표를 밀지 않고, 간격이 부족할 때만 순서를 유지하며 옮긴다.
