...
//!
//! 본체 크레이트에만 있던 것을 여기로 올린 이유는 **소비자가 여럿 생겼기 때문**이다 —
//! `tasty-host-plugin` 의 handshake 대기 맵, `tasty-telemetry` 의 탐지 창,
//! `tasty-plugin-sdk` 의 host call pending 맵이 같은 모양(자료구조 임계구역 + 첫 1 회
//! 보고)을 각자 다시 만들 자리에 있었다. 소비자가 하나뿐일 때 미리 올리지 않은 것은
//! 모양을 잘못 잡기 쉬워서였다.
//!
