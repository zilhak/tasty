...
pub struct Slot {
    ...
    ///
    /// **이 값에는 개행이 있다** — 문단이 여럿인 doc comment 는 `long_about`/`long_help`
    /// 로 통째로 온다. 그러니 이 목록을 번역하려고 밖으로 꺼낼 때는 JSON 처럼 개행을
    /// 값 안에 담는 형식으로 꺼내고, TSV·`key=value` 같은 **줄 기반 형식으로 꺼내지
    /// 마라**: 줄 기반 형식은 꼬리를 조용히 버리는데 **자리 수는 그대로라 커버리지
    /// 래칫이 초록**이고, 반쪽 값이 `--help` 로 나간다. 그것을 잡는 것은 수를 세는
    /// 술어가 아니라 값을 원문과 글자 그대로 대조하는
    /// `every_english_help_value_matches_the_compiled_text` 하나뿐이다.
    pub english: String,
}
