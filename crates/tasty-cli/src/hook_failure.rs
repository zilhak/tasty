...
/// 만드는 게 아니라, 버려지던 정보를 붙잡아 두는 것이 이 모듈의 전부다.
fn format_line(method: &str, reason: &str) -> String {
    let surface = std::env::var("TASTY_SURFACE_ID").unwrap_or_else(|_| "-".into());
    // 줄 단위 레코드이므로 개행은 공백으로 접는다(한 실패 = 한 줄 불변식).
    let reason = reason.replace(['\n', '\r'], " ");
    format!(
        "{ts} method={method} surface={surface} reason={reason}\n",
        ts = utc_timestamp()
    )
}
...
pub fn record(method: &str, reason: &str) {
    let Some(path) = log_path() else { return };
    record_at(&path, method, reason);
}

/// 경로를 주입받는 실제 구현 — 테스트가 프로세스 전역 env(`TASTY_HOME`)를 건드리지
/// 않고 검증할 수 있게 분리했다(같은 프로세스에서 병렬로 도는 다른 테스트와의 경쟁 방지).
fn record_at(path: &std::path::Path, method: &str, reason: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new()
        ...
        .open(path)
    {
        let _ = f.write_all(format_line(method, reason).as_bytes()); // best-effort
    }
}
...
mod tests {
    ...
    fn line_is_single_line_and_greppable() {
        let line = format_line("claude.hook", "boom\nsecond line");
    ...
    fn record_appends_and_rotates() {
        ...
        record_at(&path, "claude.hook", "could not connect");
        record_at(&path, "claude.children", "ignored");
        ...
        record_at(&path, "codex.hook", "timed out");
    ...
    fn record_creates_missing_parent_dir() {
        ...
        record_at(&path, "claude.hook", "no port file");
