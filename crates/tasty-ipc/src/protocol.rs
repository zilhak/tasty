...
pub struct JsonRpcRequest {
    ...
    ///
    /// **봉투 검사는 메서드와 무관하다** — 길이 밖 키(빈 문자열, 상한 초과)는 `Read` 든
    /// `Mutate` 든 `-32602` 다. 메서드마다 다르게 검사하면 같은 봉투가 어디로 가느냐에
    /// 따라 유효하기도 무효하기도 해서, 봉투 수준 계약이라는 말이 성립하지 않는다.
    ///
    ...
    /// 하나뿐**이고, 재시도에 그것이 안 붙었다면 계약이 그 메서드에 안 걸린 것이다.
    ///
