...
pub struct JsonRpcRequest {
    ...
    ///
    /// **봉투 검사도 engine 라우터로 가는 요청에 한해 돈다.** 그 범위 안에서는 메서드와
    /// 무관해서, 길이 밖 키(빈 문자열, 상한 초과)는 `Read` 든 `Mutate` 든 `-32602` 다.
    /// 그 범위를 좁히는 것은 아래 ★ 와 같은 사실이다 — 검사가 보존소와 **같은 함수**
    /// (`idempotency::begin`) 안에 있어 그 함수가 닿는 범위를 그대로 물려받는다. 그래서
    /// App 층·namespace forward 로 끝나는 요청은 빈 키도 상한을 넘긴 키도 거절되지 않고
    /// **그냥 실행된다.** 즉 같은 봉투가 어디로 가느냐에 따라 유효하기도 무효하기도 한
    /// 상태가 지금 실재하고, 그만큼 봉투 수준 계약이라는 말이 아직 덜 성립한다. 그것을
    /// 참으로 만드는 형태는 검사를 `begin` 밖 · 라우터 **앞**으로 옮기는 것 하나뿐이다.
    ///
    ...
    /// 만들어져 표지가 `false` 인데, 그 코드 자신이 계약이 개입했다는 증거다 — 특히 앞의
    /// 것은 **실행을 막은** 답이다. 보존 범위 밖으로 밀려난 키도 다시 실행되고 표지 없이
    /// 성공을 낸다. 그래서 "표지 없는 재시도 = 계약 밖" 은 **성공 응답들 사이에서, 선언된
    /// 보존 범위 안에서만** 성립한다.
    ///
