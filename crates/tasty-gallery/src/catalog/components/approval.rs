...
use tasty_ui_widgets::{badge_dot, tag, BadgeVariant, Button, ButtonVariant, TagVariant};
...
pub fn draw(ui: &mut egui::Ui, theme: &Theme) {
    spec::stage(ui, theme, StageVariant::Wrap, |ui| {
        kit::frame_card(ui, theme, WIDTH, kit::panel_fill(theme), |ui| {
            ...
            kit::region_sym(ui, theme.spacing_md.value(), theme.spacing_md.value(), |ui| {
                ...
                kit::body(ui, theme, "Claude requests permission to run a shell command in workspace web:");
            ...
            kit::region_sym(ui, theme.spacing_md.value(), theme.spacing_sm.value(), |ui| {
                ui.horizontal(|ui| {
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        Button::new("Always allow").variant(ButtonVariant::Agent).show(ui, theme);
                        Button::new("Allow once").variant(ButtonVariant::Secondary).show(ui, theme);
                        Button::new("Deny").variant(ButtonVariant::Ghost).show(ui, theme);
                    });
                });
            });
        });
    ...
    spec::meta(
        ...
        &[
            TokenChip::new("accent-agent", "agent identity", theme.accent_agent().to_egui()),
            TokenChip::new("bg-app", "command block", theme.bg_app().to_egui()),
            TokenChip::new("accent-warning", "risky grant", theme.accent_warning().to_egui()),
        ],
    );
