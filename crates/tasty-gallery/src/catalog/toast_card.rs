...
//!
//! 재수출 이름을 옛 이름 그대로 둔 것은 호출부(단일 카드 specimen · kb import/export
//! specimen)가 `toast_card::draw_card` 로 부르고 있어서다 — 위젯 크레이트 루트에서는
//! 같은 이름이 너무 넓어 `toast_` 접두가 붙는다.
...
pub use tasty_ui_widgets::{
    ToastCardColors as CardColors, draw_toast_card as draw_card, toast_accent_color as accent_color,
};
