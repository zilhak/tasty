...
|-------|----------|-------------|-------------|---------|-----------|-------------|------------|---------------------|
| 1  | 없음 | **52** | 0.60 | 0.02 | 0.007 | 0.038 | 51 | 50 |
| 1  | 만재(10k) | **60** | 2.2 | 4.4 | 0.010 | 0.081 | 53 | 52 |
| 10 | 없음 | **513** | 4.9 | 0.08 | 0.014 | 0.051 | 507 | 505 |
| 10 | 만재(100k) | **601** | 33 | 41 | 0.023 | 0.099 | 527 | 521 |
| 30 | 없음 | **1541** | 13 | 0.14 | 0.030 | 0.057 | 1528 | 1518 |
| 30 | 만재(300k) | **1834** | 101 | 152 | 0.036 | 0.120 | 1581 | 1561 |

읽는 법:

- **지배 구간은 C5b 하나다** — 어느 조건에서도 close_total 의 83~97% 다. 스크롤백
  유무에 거의 영향받지 않고 surface 수에만 붙는다(surface 당 약 50ms). 탭 30개면
  그 자체로 1.5초이며, close 체감 지연을 더 줄이려면 여기를 봐야 한다.
- **C1 은 라인 총합에 선형이고 기울기가 작다** — 10k 라인당 약 3ms. 캡처가 스크롤백의
  저장 표현(`ScrollbackLine`)을 그대로 벌크로 가져오기 때문이다(아래 "캡처 비용"
  참조). 만재 30탭에서도 close_total 의 5% 남짓이다.
- **C2b(스크롤백 디스크 write)가 C1 보다 비싸다** — 만재 30탭에서 C1 101ms 대
  C2b 152ms. 캡처는 메모리 복제뿐이지만 C2b 는 30 만 라인을 직렬화해 파일로
  내리므로, 스크롤백이 만재일 때 "스냅샷 계열" 비용의 다수는 이제 캡처가 아니라
  write 쪽이다. 이 write 는 close 프레임 안에서 동기로 돈다.
