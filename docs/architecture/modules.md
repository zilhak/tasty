...
| 메서드 | 줄 | 역할 |
|--------|-----|------|
| `new()` | 27 | GPU 어댑터/디바이스/Surface/렌더러/egui 초기화 |
...
- `FontConfig` (17행): FontSystem + SwashCache + FontMetrics.
...
| 메서드 | 줄 | 역할 |
|--------|-----|------|
| `FontConfig::new()` | 24 | FontSystem 초기화 + 셀 크기 측정 |
...
테스트가 없는 모듈: `main.rs`, `state.rs`, `terminal.rs`, `renderer.rs`, `gpu.rs`, `font.rs`, `ui.rs`, `cli.rs`, `settings_ui.rs`, `ipc/server.rs`, `ipc/handler.rs`.
