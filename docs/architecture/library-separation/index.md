...
**예측 적중**: `tasty-renderer` 는 *장기 과제* 라 했고 지금도 분리 안 됨. *비권장* 이었던 ipc 2 건 + notification 도 분리 안 됨. **반전 추가 1 건**: `tasty-model` — F.A (headless 도입) 가 trigger 를 충족해 G.E 에서 분리.
...
| **본 바이너리** | `tasty` (`src/`, 479 `.rs` / ~91k LOC) | 위 33 crate 직접 의존 |

총 33 = 옛 권장 2 (terminal, hooks) + 옛 비권장 반전 2 (settings, plugin-protocol) + 옛 장기 과제 반전 1 (model, G.E) + 신규 28.
