...
Mutate 요청의 재시도는 요청 메시지의 `idempotency_key`로 구분한다.
저장소는 연결 대신 호출자 종류·ID와 키를 함께 사용한다.
...
응답 없이 채널이 닫힌 항목도 저장소에서 제거한다. 이 설계는 보존 범위와 재시작 여부에 관계없이 정확히 한 번 실행한다고 보장하지 않는다.
