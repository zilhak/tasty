...
clipboard viewer는 열릴 때 현재 내용을 한 번 읽으며 호스트의 주기적 수집·이력·재복사 API는 제공하지 않는다.
...
제3자 플러그인의 직접 OS 열기를 이 경로로 강제할 수는 없다.
...
클립보드 이력이나 장시간 이미지 편집 복원이 실제 사용 목적이 되면 보존 정책을 다시 정한다.
