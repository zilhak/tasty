...
- 위 "남은 상류 상한" 이 적어 둔 남용차단 쪽 빚(`401` 반복이 쿨다운으로 이어지지 않던 것)은 [ADR-0195](0195-abuse-counting-includes-rejected-tokens.md) 가 갚았다. body 바이트 상한은 [ADR-0200](0200-webhook-body-has-a-per-request-byte-cap.md)이 요청당 상한과 `413`으로 해소했다. 위 Consequences는 이 ADR의 결정 당시 상태이며, 현재 운영 계약은 두 후속 ADR과 [웹훅 문서](../features/webhook/index.md)를 따른다.
