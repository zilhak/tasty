...
발신자는 미리 등록한 작업의 실행만 요청하고 ACK에서 내부 실행 결과를 읽을 수 없다.
200은 접수이며 실제 실행 실패·부분 적용은 호스트 로그로 확인한다.
인증 없는 URL에 도달한 사람은 누구나 등록 작업의 실행을 요청할 수 있고 HMAC은 지원하지 않는다.
...
- 인증 실패로 등록 횟수를 차감하면 토큰을 모르는 발신자가 소유자의 실행 횟수를 소모한다.
