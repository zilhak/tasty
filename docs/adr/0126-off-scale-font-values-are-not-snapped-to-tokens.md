...
기계가 가르는 경계가 이 축에 둘 생겼다 — 그리고 **둘째는 자매 둘과 모양이 다르다.**

- **① 익명 반지름 금지** — `no_dot_radius_is_an_anonymous_literal`.
  `circle_filled(` · `circle_stroke(` · `circle(` 의 반지름 인자가 숫자 리터럴이면 실패.
  셋째 표기를 함께 막는 이유는 `FontId::new(` 과 같다(둘만 막으면 셋째로 재유입된다).
  본체 루트 위반 0 으로 섰다.
- **② 토큰 값 사본 금지** — `no_dot_named_const_copies_the_status_dot_token`.
  값 8 인 **점 이름** const 를 막는다.

> **정정.** 이 절은 처음에 "나타나면 두 자매 가드와 같은 모양(**값 × 자리**)으로 넓히면
> 된다" 라고 적었는데 **실측이 그것을 반증한다 — 이 축에는 "자리" 가 없다.** 폰트·반경
> 축은 `.size(` · `.corner_radius(` 라는 **문법적 자리**가 있어 값과 자리를 곱할 수 있다.
> 점 const 는 `X.value() * 0.5` 나 `vec2(X, X)` 같은 **일반 산술**로 소비되므로 소비
> 형태만 보고 "점 자리" 를 가릴 수 없다. 값만으로 재면 점이 아닌 상수를 잡는다 —
> 실측으로 `src/adapters/ui/popup/file_handler_picker.rs` 의 `VERTICAL_PADDING` ·
> `HORIZONTAL_MARGIN`(둘 다 `LogicalPx(8.0)`)이 그 자리다.
>
> 그래서 ②는 **이름 축**으로 세웠고, 그 대가는 감추지 않는다: 이름에 `DOT` 이 없는
> 상수를 점에 쓰면 통과한다. 자매 둘이 이름 축을 거부한 바로 그 이유이고
> (`SOMETHING_ELSE = 13.0` 도 잡아야 한다), 이 축에는 그 대안이 없다. **부류 밖에 대상이
> 없다고 단정하지 않는 가드**다.

점 축의 모수는 `SCAN_ROOTS` 에서 **갤러리를 뺀 것**이고, 목록을 따로 적지 않고 걸러서
만든다(따로 적으면 두 목록이 조용히 갈린다 — 자매 둘이 실제로 그렇게 갈렸다). 갤러리를
빼는 사유는 하나다: 전역 zoom 을 쓰므로 리터럴도 함께 커진다(ADR-0135). 점 지름은
**길이**라 그 예외가 걸린다 — 같은 파일의 색 계수 축이 갤러리를 모수에 **넣는** 것과
반대이고, 가르는 것은 경로가 아니라 **무차원이냐 길이냐**다. 그 제외가 하중을 받는지는
변이로 확인했다: 갤러리를 모수에 넣으면 실재 리터럴(`widgets/tutorial.rs` 의 2.5)에서
실제로 운다.
...
### 판정은 **가족별로** 한다 — 스케일을 합집합으로 보면 부류 하나가 통째로 숨는다

이 ADR 의 판정("값이 스케일 밖인가")을 **어느 스케일에 대고 재는가**의 문제다. DTCG
primitive 는 한 덩이가 아니라 가족이 여럿이다 — `size-*`(구조 길이) · `font-size-*`(텍스트)
· `radius-*`(반경). 세 가족을 **합집합으로** 놓고 재면 "스케일 안" 이 넓어지고, 그 넓어진
만큼이 **거짓 음성**이 된다.

실측으로 걸렸다. 구조 길이 자리의 20 · 10 · 13 · 17 은 `size-*` 에 **없고**
`font-size-*` 에 **있다**. 합집합으로 재면 31 자리가 "스케일 안" 으로 분류돼 이 ADR 의
대상에서 빠진다. 그러나 **길이 자리에 폰트 스케일의 수를 쓰는 것은 스케일을 지킨 것이
아니다** — 값이 우연히 겹칠 뿐 다른 가족에서 왔고, 디자인이 `font-size-*` 를 조정하면
근거가 사라진다.

그래서 판정 기준을 **"어느 스케일에 있나" 가 아니라 "어느 스케일에 있어야 하나"** 로
쓴다. 그 자리가 속해야 할 가족을 먼저 정하고, **그 한 가족에 대해서만** 안/밖을 판정한다.
가족은 값이 놓인 **자리**로 갈린다:

| 자리 | 재는 가족 |
|---|---|
| `LogicalPx`/`PhysicalPx` 생성자 · egui 기하 인자 | `size-*` |
| `.corner_radius(` 류 | `radius-*` |
| `FontId::*` · `RichText::size` | `font-size-*` |
| 아이콘 글리프 치수 | `icon_glyph_size_*`(12 · 14 · 15 · 16) — primitive 가족이 아니라 `Theme` 축이다 |

**이 규칙은 소스가 이미 지키고 있었다.** `CRUMB_GLYPH`(13) · `PLUG_HEADER_GLYPH`(17) 의
상수 주석이 "아이콘 스케일 밖" 이라고 정확히 가족별로 적고 있다. 합집합으로 잰 것은
**측정 도구 쪽**이었다. 그래서 이 절이 고치는 것은 코드가 아니라 **판정 절차**다.
...
### 배율 축 — **그릇과 내용은 같은 편이어야 한다**

앞의 형태들은 "이 값에 이름이 있나" 를 물었다. 이 절은 그 앞의 물음이다: **이 값이 UI
배율을 타야 하나.** 값이 같아도 부르는 이름에 따라 답이 갈리기 때문에 이름 물음보다 먼저
온다 — `Theme` 필드는 `zoomed()` 를 타고 `STRUCT_GAP_*` 같은 평상수는 안 탄다.

**규칙**: 한 레이아웃 안에서 **그릇과 내용이 배율에 대해 같은 편**이어야 한다. 하나가 타고
다른 하나가 안 타면 배율 ≠ 1 에서 **반드시** 어긋난다 — 0.85 에서 여백이 뜨고 1.2 에서
내용이 잘린다. 이건 디자인 결정이 아니라 **결함**이고, 값이 같아서 눈으로는 안 보인다.

**방향 고르는 법**:

1. **배율 1 에서 픽셀이 보존되는 쪽**을 고른다.
2. 양쪽 다 보존되면 → **타는 쪽으로 통일**한다. UI 배율은 크게 보려고 있는 것이라 안 타는
   것이 예외이고, 예외에 사유가 필요하다.
3. 안 타는 쪽으로 통일해야 한다면 **그 사유를 그 자리에 적고, 내용도 같이 안 타게** 한다.
   반쪽만 고정하는 것이 애초의 결함이다.

**판별식 — 파일이 아니라 그리는 대상으로 가른다.** 한 파일 안에서도 갈린다: 손으로 그린
축소 도형(`painter` 로 rect 를 찍는 자리)과 실제 theme 위젯(라벨·Select 등)은 배율에 대해
서로 다른 편에 설 수 있다. 실물로, preset 미리보기 위젯은 미니 탭 스트립을 손으로 그리고
그 아래 inline form 은 theme 위젯으로 짓는다 — form 쪽 간격만 평상수였고, 그것은 위젯이
커지는데 사이만 안 커지는 자리라 고쳤다. **"이 파일은 축소도다" 로 뭉뚱그리면 그 절반이 안
보인다.**

**선례가 이미 있다.** [`theme.rs`](../../crates/tasty-type-appearance/src/theme.rs) 의
"컴포넌트 치수(디자인 export 에 아직 토큰이 없는 자리)" 블록은 대응 토큰이 **없는데도**
리터럴을 금지하고 `ui_zoom` 을 곱한 접근자로 둔다. 그 사유가 이 규칙과 같은 문장이다 —
"상자만 고정이고 안의 폰트·간격·글리프는 커지므로 0.85 에서 여백이 뜨고 1.2 에서 내용이
잘린다". **토큰이 없다는 것은 리터럴로 둘 사유가 아니다.**

**갤러리에서는 이 검사가 성립하지 않는다.** 갤러리는 배율을 egui 전역
`ctx.set_zoom_factor` 로 걸고 `Theme` 은 `with_colors` 로만 만든다 — 리터럴도 토큰도 **똑같이**
커지므로 "이웃은 타는데 나만 안 탄다" 가 일어날 수 없다. 갤러리 자리는 이 축으로 **갈리지
않는다**(판별 불가이지 통과가 아니다).

> **두 lane 이 서로 모른 채 같은 경계에 닿았다.** 위 갤러리 제외는 이 축에서 이웃 검사를
> 돌리다 나온 것이고, 같은 파일 점 치수 축의 모수가 갤러리를 빼는 사유(ADR-0135, 전역 zoom
> 이라 리터럴이 함께 커진다)와 **독립적으로 같은 값이 됐다.** 두 축이 서로를 참조하지 않고
> 같은 선을 그었으면 그 선은 파일 규칙이 아니라 **성질**이다 — 가르는 것은 경로가 아니라
> 그 크레이트가 배율을 어디에 거는가다.
...
- **(색 계수 축)** 값이 opacity 토큰과 같으면서 역할도 같은 자리가 새로 나타난다 —
  ...
  **없고, 만들 수도 없다.** 폰트·반경 축의 `no_named_const_copies_a_*_token` 은
  `값 × 자리`로 재는데, 이 축은 **자리가 역할을 안 담는다** — `.gamma_multiply(` 라는
  자리는 disabled 디밍과 태그 테두리와 아이콘 톤을 구별하지 못한다.
  실측: 그 모양으로 세우면 오늘 **이름 8 종**이 걸리는데(`TAG_BORDER_OPACITY` ·
  `BADGE_STROKE_OPACITY` · `WARN_BADGE_STROKE_OPACITY` · `SCROLL_ARROW_OPACITY` ·
  `BORDER_MIX` · `SELECTED_BORDER_ALPHA` · `SWATCH_INHERITED_ALPHA` ·
  `EMPTY_ICON_MUTED_OPACITY`) 값이 토큰과 같을 뿐 역할이 다르다. 초록일 때도 거짓인
  가드가 되므로 세우지 않는다. **그래서 이 절반은 사람이 리뷰에서 읽어야 발화한다** —
  점 축이 이름 축으로라도 세울 수 있었던 것과 달리, 이 축에는 그 대안도 없다
  (이름은 사람이 붙이므로 이름으로 역할을 재는 것은 자기참조다).
