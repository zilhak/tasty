...
| 값 | 0.85 / 1.0 / 1.2 | `zoomed()` 경유 | 명명 const 로 빼는 대가 |
|---|---|---|---|
...
| `border_width` 1 | 1 / 1 / 1 | **안 탄다** | 없다 — **경유해도 마찬가지다** |
| `icon_stroke_width` 1.5 | 1 / 2 / 2 | **안 탄다** | 없다 — 다만 경유하면 **생긴다** |
...
- **(반경 축)** 지원 UI 배율 집합이 바뀐다 — 1.0 · 2.0 이 반올림 불변이 아니게 되면
  굵기 축에도 zoom 대가가 생겨 위 표가 통째로 다시 그려진다.
  `zoom_cost_differs_by_axis` 의 ③이 먼저 운다. 폰트 축은 영향받지 않는다.
...
- **(반경 축)** 값이 반경 토큰과 같은 명명 const 가 나타난다 — 폰트 쪽
  ...
  **이 트리거만 자동 채널이 없다** — 그 형태를 잡는 판정이 아예 없으므로 사람이 리뷰에서
  읽어야 발화한다(모수 = `SCAN_ROOTS` 아래 추적 `.rs`, base = `d7dc4079`; base 가 바뀌어
  반경 축 사본 가드가 생기면 이 단서는 만료된다). 대비되는 자리를 함께 적어야 등급이
  갈린다 — 위 다른 반경 축 트리거 둘은 `zoom_cost_differs_by_axis` 가 잡고, 그것은 lib
  유닛 테스트라 `crossplatform-check.yml` 의 `cargo test --workspace --lib --bins`
  (main push · PR)로 **자동으로 돈다.** **셋을 같은 등급으로 읽지 말 것.**
