...
호출자별 상한 검사 입력과 Allow 집계의 기존 의미는 바꾸지 않는다.
전역 진단을 얻으려고 거절 요청을 호출자 사용량에 더하지 않는다.
...
전체 스냅샷은 원자적이지 않고 프로세스 누계는 오래된 최대값을 포함한다.
...
- 호출자별 영속 이벤트에 모든 지연을 기록하면 진단이 저장 부하를 만든다.
...
새 요청 검사·응답 대기자·plugin 전달 단계·이벤트 생성 경로에는 대응 집계와 결과 기록을 추가한다.
호출자가 자기 요청만 진단하거나 재시작을 넘는 분석이 필요해지면 권한·보존 정책을 별도로 정한다.
