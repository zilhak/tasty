# ADR-0612: 요청은 라우팅 전에 권한을 확인하고 사용자 입력과 분리한다
...
반대로 안팎에서 같은 검사를 반복하면 rate token을 두 번 소비한다.
전용 API를 막아도 raw memory KV로 호스트 기록을 읽거나 위조할 수 있으면 보호가 성립하지 않는다.

## Decision

모든 외부 IPC와 plugin host-call은 라우팅 전 공통 check_request에서
권한, telemetry cap, rate-limit을 순서대로 한 번 검사한다.
허용된 요청의 사용량 집계와 거절 audit도 요청 단위로 수행한다. Allow audit의 영속 여부는 저장 정책이 정한다.
private CheckedRequest로 검사 결과를 전달하고 외부 checked 플래그나 Local 위장으로 대신하지 않는다.
새 host-call은 별도 요청이므로 새로 검사한다.

plugin 고유 namespace는 권한 보유 caller에게 ipc.invoke 토큰을 요구한다.
...
권한을 받는 caller의 raw memory KV에서 tasty. 접두사는 호스트 전용으로 예약한다.
...
다른 caller의 호스트 기록 노출·위조와 release 입력 재현을 제한한다.
...
기록을 멈추고 싶은 사용자를 위한 전체 telemetry opt-out은 제공하지 않는다.

## Alternatives Considered

- 조기 list마다 검사만 더하면 다음 조기 반환이 다시 빠지고 관측 위치도 갈라진다.
- 안팎 검사 유지가 안전한 중복은 아니다. rate token 소비는 실제 부수효과다.
...
새 진입점·라우터·caller·권한 위임 경로를 추가할 때 검사 횟수와 조기 응답을 함께 확인한다.
