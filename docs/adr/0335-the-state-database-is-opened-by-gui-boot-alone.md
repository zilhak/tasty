...
- **잃은 것**: 접근자 이름이 길어졌다. 그리고 `memory.db` 쪽은 여전히 `with_memory` 라
  "state" 에 대응하는 접두어가 한쪽에만 붙는다 — 218 자리를 옮기는 것은 이 결정의 값에
  비해 비싸다고 봤다.
- **운영 비용 / 유지 부담**: 이름을 인용하는 문서가 셋이다(`docs/dev-guide/build.md` 두
  줄, `docs/design/systems/storage.md` 한 줄). 이름을 다시 바꾸면 그 셋이 함께 움직여야
  한다. 그 정합을 보는 자동 채널은 없다.
