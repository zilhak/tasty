# ADR-0610: 에이전트 primitive 의 이름은 memory 키가 되기 전에 호출자 값 기준으로 판정한다
...
- **잃은 것**: 이름에 대문자 · 공백 · `/` 등을 쓸 수 없다는 제한은 그대로 남는다(lease 와 비대칭).
  task id 를 잘못 준 조회도 이제 `-32004`(not found)가 아니라 `-32602` 다 — 그런 id 로는 애초에
  저장될 수 없으므로 "없다" 보다 "틀렸다" 가 사실이다.
