# ADR-0034: 터미널 출력은 스트림과 바이트 위치로 이어 읽는다
...
출력 보존 범위보다 뒤처진 소비자는 여전히 출력을 잃지만 잃은 바이트 수를 확인할 수 있다.
...
- 소비자별 서버 mark는 등록·해제·누수 관리가 필요하다.
...
- [이벤트 위치 조회](0033-event-feed-delivery.md)
