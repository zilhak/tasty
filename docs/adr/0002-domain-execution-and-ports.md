...
`core::structural_exec`가 구조 변경 검증·적용·변경에 따른 후속 처리를 맡고 IPC와 forward가 함께 호출한다.
...
두 진입점에서 같은 검증과 후속 처리를 수행하고 핸들러가 쓰는 창 연산을 타입으로 확인할 수 있다.
...
IPC와 forward의 실패 문구 일치, 기존 문구 호환과 두 빌드의 변경에 따른 후속 처리를 각각 검증한다.
