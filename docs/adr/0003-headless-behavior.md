...
생성은 되지만 특정 빌드에서만 읽히는 값에는 항목별 이유를 적은 `expect`를 사용한다.
...
기능을 헤드리스에 추가할 때 cfg만 풀지 말고 이벤트 생성·처리 경로를 연결하고 응답 전에 처리가 끝나는지 확인해야 한다.
...
8라운드 제한에 닿으면 제한을 올리기 전에 이벤트가 반복해서 발생하는 원인을 살핀다.
