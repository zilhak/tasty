...
4. **결과 유실 감지** — result push 시점에 소유 popup 이 이미 사라졌으면 `warn` 을 남긴다. 연쇄 정리가 제대로 돌면 나오지 않아야 하는 조합이라 조용히 넘기지 않는다. 이벤트 자체는 그대로 보낸다 — "정확히 하나의 결과" 는 popup 생사와 무관한 계약이고, plugin 이 popup 밖에서 상관관계를 유지하고 있을 수도 있다.
