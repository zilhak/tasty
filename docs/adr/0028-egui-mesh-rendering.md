# ADR-0028: 플러그인이 그린 mesh를 호스트가 합성한다
...
고정 위젯 DSL을 늘리는 방식은 서식 있는 텍스트와 다양한 UI 기능을 각각 구현해야 하고,
픽셀 프레임만 보내면 글자 선명도와 확대 처리에 불리하다.

별도 프로세스에서 그리는 UI에는 전송 비용도 있다.
한 프로세스 안에서는 비용이 작은 애니메이션 한 프레임도 여기서는 입력 전달, tessellation, 인코딩과 GPU 갱신을 반복한다.
...
목록은 일정한 행 높이를 전제로 보이는 행만 배치하고 안정적인 행 ID를 쓴다.
