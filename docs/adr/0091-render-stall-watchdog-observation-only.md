...
그래서 `stall_watchdog::without_stall_watch(f)` 로 그런 구간을 감싼다. 구간 안에서는 보고하지 않고, 구간을 벗어나면 진입 시각을 다시 잡아 모달에 쓴 시간이 stall 로 누적되지 않게 한다. 현재 적용 지점은 `rfd::FileDialog` 호출 4 곳(설정 scripts / remote-transfer · 프리셋 필드 · 플러그인 추가)이다 — 전부 **사용자가 버튼을 눌러 여는** 경로다(IPC 로 열던 `fs.pick_file` 은 ADR-0161 로 제거됐다).
