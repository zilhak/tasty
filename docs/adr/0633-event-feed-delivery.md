# ADR-0633: 이벤트 피드는 짧게 보관하고 소비자가 읽은 위치를 관리한다
...
그렇다고 소비자마다 호스트에 cursor와 ACK 상태를 두면 느리거나 사라진 소비자의 관리가 서버에 남는다.
...
같은 요청을 반복해도 서버 cursor 부작용은 없지만 사건 생성·eviction 때문에 답은 달라질 수 있다.
...
- 서버별 cursor는 consumer 등록·해제와 상태 회수를 추가한다.
...
- [터미널 출력의 위치 조회](0634-output-cursor-contract.md)
