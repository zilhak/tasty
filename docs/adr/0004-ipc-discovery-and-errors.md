...
호출자가 이름을 틀린 경우와 플랫폼·빌드·호출자 조건 때문에 실행할 수 없는 경우는 해결 방법이 다르다.
모두 Method not found로 답하거나 plugin을 거치며 오류 코드를 잃으면 호출자가 잘못된 재시도를 하게 된다.
...
호스트 오류는 plugin host-call과 post-hook을 거쳐도 원래 `error_code`를 유지한다.
오류 문구의 기존 wrapper는 보존하며 코드가 없는 구 응답과 plugin 내부 오류만 기본 server error로 처리한다.
코드 전달과 사용자 표시 문구를 별도 계약으로 다룬다.

서버는 `system.info.capabilities`에 구현된 기능의 이름과 버전을 선언한다.
...
플랫폼 지원 범위나 이름 해석 방식이 달라지면 오류 코드의 조건을 다시 확인한다.
