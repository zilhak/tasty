# ADR-0009: 사용 기록과 진단 로그는 수명에 맞춰 저장한다
...
최근 파일 캐시는 Db가 소유하고 모든 창이 공유한다.
기록은 캐시 잠금 안에서 메모리 변경과 DB 저장을 순서대로 수행한다.
DB 저장 실패에도 세션의 목록은 모든 창에서 같은 상태를 유지한다.

튜토리얼은 주제·콘텐츠 개정 번호·완료 여부·재개 단계·행 버전을 별도 테이블에 저장한다.
...
부팅, append와 주기 타이머가 같은 정리 함수와 주기 제한을 사용한다.
...
창이 달라도 최근 목록이 일치하고 튜토리얼 진행이 레이아웃 설정이나 런타임 ID에 종속되지 않는다.
...
- 최근 목록을 매번 DB에서 읽으면 동일 시각 항목의 즉시 순서와 읽기 부수효과 문제가 생긴다.
- 튜토리얼을 config나 layout에 넣으면 사용자 선호 또는 창 복원 수명과 섞인다.
- allow 폴링 제외 목록은 새 폴링 메서드를 계속 관리해야 한다.
...
observer의 같은 밀리초에 많은 결과가 몰리거나 시계 역행·재시작으로 키 충돌이 보고되면 키 형식도 검토한다.
