...
- **운영 비용 / 유지 부담**: 발화 금지 정책은 여전히 자동 채널이 없다. 재는 법은 사람이
  돌려야 하고, 호출 이름이 IPC 파일에 안 나타나는 경로 셋을 원리적으로 못 본다.
...
- **곱 순서를 인자로 받기** — 두 길을 다 살려 두는 것은 "도출이 둘" 을 함수 안으로 옮긴
  것뿐이다. 기각.
- **발화 금지 소스 스캔 가드를 짓기** — 그 가드가 없어서 통과한 결함을 댈 수 없다. 실제로
  있었던 결함(예전 `window.create` 가 창 생성 실패를 사용자 토스트로 알리던 것,
  [ADR-0122](0122-winit-scheduled-fallible-ipc-returns-outcome.md) 가 고쳤다)은 winit 이벤트
  핸들러에서 났으므로 IPC 파일을 스캔하는 가드로도 안 잡혔을 것이다. 기각.
...
- IPC 요청이 사용자 토스트를 띄운 결함이 실제로 보고되면 — 그 결함의 경로를 보는 판정기를
  그 수정과 함께 짓는다. 재는 법: 결함 보고.
