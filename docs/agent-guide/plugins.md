...
| `plugin.upgrade_builtins` | `force?: bool` (기본 `false`), `restore_removed?: string[]` (id 배열, 기본 `[]`), `restore_all?: bool` (기본 `false`) | bundle 기준으로 builtin plugin 디렉토리를 재설치. semver 비교 기반. `force=true` 시 동일/하위 버전도 덮어쓰기. `restore_removed` 에 명시된 id 는 `removed_builtins` 에서 unmark 되어 같은 호출에서 재설치 후보가 된다. `restore_all=true` 면 전체 unmark (`restore_removed` 무시). 응답: `{items: [{id, action: {kind: "skipped"\|"upgraded"\|"reinstalled"\|"not_in_bundle"\|"failed", ...}}]}`. 실행 중 plugin 의 새 binary 적용은 `plugin.disable` → `plugin.enable` 시퀀스 필요 (graceful swap 도입 전). 상세: [`docs/dev-guide/plugin-ecosystem.md` §6](../dev-guide/plugin-ecosystem.md). |
