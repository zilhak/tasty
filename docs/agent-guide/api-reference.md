...
# Claude 브로드캐스트 / 대기
tasty claude-broadcast "다음 작업을 시작하세요\r"          # 모든 자식에 텍스트 전송
tasty claude-broadcast "결과를 보고하세요\r" --role worker  # role=worker인 자식에만 전송
tasty claude-wait --child 42              # child 42가 idle/needs_input/exited 될 때까지 대기 (기본 30초)
tasty claude-wait --child 42 --timeout 60 # 타임아웃 60초로 대기
```
