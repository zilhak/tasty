...
안에서만** 일어난다. 자체 구현으로 갈아끼워도 공개 API 는 그대로다 — 라이브러리 교체의 영향을 이 모듈 안으로 제한한다.
...
출력 방향과 일치하고, 카드가 가로로 긴 형태(168×48)라 화면 폭을 아낀다.
...
현재 사용하는 `rust-sugiyama`에서 좌표 오류나 패닉이 반복되면 자체 구현으로 교체를 검토한다 — 필요한 것은 (1) 위상 정렬 레이어 할당
