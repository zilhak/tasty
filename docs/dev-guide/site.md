...
랜딩 문구는 `site/src/lib/landing.ts`, 공통 푸터 소개는 `site/src/lib/strings.ts`에서
한국어와 영어를 함께 관리한다. 사용자가 에이전트와 동시에 할 수 있는 작업을 먼저 소개하고,
기능 설명은 구체적인 사용 상황과 자연스러운 안내 문장으로 쓴다. 사용자 상태 보호는
포커스·선택·스크롤처럼 보장하는 범위를 명시하고, 연동 설정이 필요한 알림은 그 조건을 함께 설명한다.
