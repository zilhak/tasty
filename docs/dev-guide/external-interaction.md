...
**자동화할 때 주의할 점.** 일정 시간 기다리는 것만으로 입력이 전달됐다고 보장할 수는 없다. 이 실험에서는 resize 후 얼마나 기다렸는지와 관계없이, 셸이 다음 입력을 읽을 때 첫 바이트가 사라졌다. 입력 전달을 확인해야 하는 자동화에서는 echo 결과를 검사하거나 앞에 무해한 바이트를 보낸다.
