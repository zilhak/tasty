...
hub.once_after(key, delay, precision, now);    // 일회성(실행 후 자동 제거)
hub.cancel(key);
...
TTL 기반 정리 세 가지는 `Lax`를 사용한다. 요청이 올 때만 정리하면 요청이 끊긴 뒤
만료된 상태가 계속 남을 수 있어 주기 실행을 함께 둔다.
...
이 정리는 수십 초 늦어도 되므로 다른 작업 때문에 깨어난 루프에서 함께 실행한다.
계속 유휴 상태라면 slack 경계에서 깨운다. 타이머가 정상적으로 처리될 때의 회수 지연은
`TTL + interval + slack` 이내다(PTY 기준 최대 6.5분).
...
<a id="every-는-바닥치기arm_derived-대상이-아니다"></a>

### `every` 는 최소 지연 보정(`arm_derived`) 대상이 아니다

아래 "파생 데드라인은 반드시 최소 지연을 보정한다" 규칙은 **`once_at` 계열**(외부 상태에서
...
- 재실행 시각은 허브가 직전 데드라인에 주기를 더해 **스스로 전진**시킨다(위 "반복
...
GUI는 데드라인에 맞춰 `EventLoopProxy::send_event`를 보내는 waker 스레드 하나를 쓴다.
`WaitUntil`은 창이 있을 때 깨움 지연을 줄이는 보조 수단이다. 둘은
`sync_timer_control_flow`에서 같은 데드라인을 받는다.

macOS 최소화나 트레이 상주처럼 창이 없을 때도 parked engine의 busy forward와
글로벌 훅은 실행돼야 한다. `WaitUntil`만으로 이를 처리하는 대안은 세 OS의 최소화·트레이
상태에서 1Hz busy forward를 검증하기 전까지 채택하지 않는다.

waker는 등록된 타이머가 없으면 무기한 기다린다. 새 데드라인이 생기면 `Condvar`로
대기를 갱신하므로 고정 주기로 계속 깨우는 스레드는 필요 없다.
...
실행과 정지를 처리한다. 이것은 죽은 waker 스레드를 재시작한다는 보장은 아니다.
...
최초 획득·무기한 대기·시간제한 대기 각각의 error 로그 한 번, 데드라인 실행,
...
가 타이머 처리 경로에서 해당 함수가 호출되는지 검사한다.
...
| `PluginTick::Rss` | 30s | Lax(slack 15s) | 관측용 — 스스로 wakeup 을 만들지 않고 ping 때 함께 실행 |
...
아무 tick 에서나 판정할 수 있다. 별도 tick 을 만들지 않고 `Ping` tick에서 함께 수행한다 —
...
에서 `hub.once_at` 을 직접 부르면 그 테스트가 fail 한다(채널은 [ci-gates](ci-gates.md)). 실패 지점이 단위 테스트가 닿지
않는 **호출부 한 줄**이라 같은 클래스가 두 번 재발했기 때문이다.

<a id="바닥치기는-2차-방어다--스케줄-대상-자체를-좁혀라"></a>

### 최소 지연 보정은 2차 방어다 — 스케줄 대상 자체를 좁혀라

최소 지연 보정은 스핀을 막을 뿐 **누수는 남긴다.** 해소되지 않을 상태로 데드라인을 만들면
...
가드가 풀리면 밀린 타이머는 **한 번만** 실행하고 위상이 재정렬된다(허브는 실행을
큐잉하지 않는다).
...
5. 절대 시각(외부 상태에서 파생한 데드라인)을 등록한다면 `arm_derived` 를 통과시킨다
   ...
   주기(`every`)면 해당 없다(위 "`every` 는 최소 지연 보정 대상이 아니다").
...
`timer.list`와 `tasty list timers`로 메모리에 있는 허브 상태를 조회한다.
별도 로그를 추가하지 않고도 유휴 인스턴스가 계속 깨어나는 이유를 확인할 수 있다.
회귀 검증에서는 계산한 기한 때문에 반복해서 깨어나는지, 인자가 붙은 키가 계속 남는지,
기능을 끈 뒤에도 해당 타이머가 남는지를 확인한다.
...
등록 / 취소 / 강제실행 API 는 만들지 않는다. 외부가 내부 스케줄을 흔들면 회수 지연
...
인스턴스의 wakeup 원인을 물어볼 방법이 사라지므로 양쪽에 연결한다.
