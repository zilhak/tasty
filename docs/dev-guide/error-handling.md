...
실측(2026-09-04, 베이스 `81327419`, `cargo clippy --workspace --all-targets --locked`):
이 lint 의 **유니크 위치 188 → 91** (같은 자리가 여러 타깃에서 중복 보고되므로 원시
진단 수는 그보다 크다). 전체 경고 총량은 다른 모든 변경과 함께 움직이므로 축으로 쓰지
않는다 — 봐야 할 수는 이 lint 의 것뿐이다. 남은 91 은 전부 프로덕션이고 전부 사유 주석이
달려 있다(그러니 "고쳐야 할 91건" 이 아니다). 이 수가 움직였다면 **프로덕션이
움직인 것**이다 — 테스트 증감은 더 이상 이 숫자를 흔들지 않는다.
