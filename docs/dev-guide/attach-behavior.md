...
- **변경 신호 수신자**: `markdown_changed` 는 **위 인가 술어가 참인 그 집합** — 즉 이 engine 의 워크스페이스를 하나라도 점유한 client 전부 — 에 push 한다. 그 surface 를 담은 워크스페이스의 holder 로 좁히지 않는다: 위 조회가 이미 engine 전체라, 좁히면 요청은 통과하는데 신호는 안 오는 client 가 생긴다. 별도 구독 표를 두지 않는 이유도 같다(요청할 수 있는 client 와 신호를 받는 client 가 갈리면 안 된다). 점유가 없으면 신호는 사라진다.
