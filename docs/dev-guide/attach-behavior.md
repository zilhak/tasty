...
- **gui-as-server 미지원**: 위 서버측 소비(`forward_mesh_frames`)는 헤드리스 부트스트랩(`boot::run_headless`)에만 배선돼 있다 — gui 인스턴스가 attach 서버(피점유측)가 되는 경로는 아직 이 파이프라인을 안 탄다(TODO 24 로 분리, 서버측 `PluginManager` 소유권이 gui 메인루프와 어떻게 공존할지 별도 설계 필요).
