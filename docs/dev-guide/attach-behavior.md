...
- **진실 원천은 surface 를 소유한 인스턴스다.** producer 4 종(완료 IPC/CLI, Claude 플러그인 훅, OSC 133 명령 완료, toast)이 전부 PTY 가 있는 쪽에서 돌고, 특히 `AttentionKind::NeedsInput` 은 Claude 훅에서만 나오므로 mirror 가 로컬로 도출할 방법이 **원천적으로 없다**. busy 는 그래도 로컬 폴링을 시도라도 할 수 있다는 점에서 attention 의 조건이 더 강하다.
...
- **자동 수렴이 보장하는 범위 / 남은 구멍 / 닫는 후속 작업**: 위 재-diff 가 **보장하는 것은 wire 축의 유실뿐**이다 — 프레임이 드롭되거나 지연돼도 서버 값이 그대로면 다음 tick 에 같은 값이 다시 나가 미러가 서버 기준으로 수렴한다. **보장하지 않는 것은 미러가 자기 store 를 로컬로 바꾼 경우**다: 서버는 자기 값이 바뀌지 않는 한 다시 push 하지 않으므로, 미러가 로컬에서 지운 값은 서버 값이 실제로 달라지기 전까지 되돌아오지 않는다. 실제로 열려 있는 구멍이 하나 있다 — `src/gfx/gpu.rs` 의 매 프레임 포커스 해제([features/surface-highlight](../features/surface-highlight/index.md) "해제(clear)" 1번 경로)가 mirror surface 도 예외 없이 지운다. 그래서 ① 서버가 raise → push → 미러에 표시 → ② 미러 사용자가 그 surface 를 포커스해 **로컬만** 해제(서버 store 는 그대로) → ③ 서버가 **같은 kind** 로 다시 raise → 서버 값 불변이라 forward 대상이 아님 → ④ 미러 표시는 서버 값이 실제로 바뀌거나 점유가 재수립될 때까지 돌아오지 않는다. 로컬 전용 환경에서는 ③ 에서 다시 뜨므로 **미러와 로컬의 관측 가능한 동작이 갈린다.** 이 구멍은 push 채널이 아니라 해제(clear) 축의 후속 작업 3 건이 닫는다 — (a) mirror surface 에 대한 로컬 raise·변이 억제, (b) 하드 점유 중 해제 권한을 홀더로 제한, (c) 미러의 해제를 서버로 forward(서버 값이 실제로 내려가야 같은 kind 재발동이 다시 push 된다). 아래 "적용 API 를 로컬 producer 와 분리한다" 가 비워 둔 자리가 그 셋이 붙을 곳이다.
...
- **적용 API 를 로컬 producer 와 분리한다**: `set_mirror_surface_attention` / `forget_mirror_surface_attention` 은 `raise_attention`/`clear_attention` 을 타지 않는다. 후자는 로컬 producer 축이고 mirror surface 대상 로컬 raise 억제·해제 forward(mirror→server)가 붙는 자리라, 서버 push 의 적용이 같은 함수를 타면 서버 값이 억제 게이트에 막히거나 서버가 내려준 해제가 서버로 되돌아가는 에코가 된다.
