...
- **적용 API 를 로컬 producer 와 분리한다**: `set_mirror_surface_attention` / `forget_mirror_surface_attention` 은 `raise_attention`/`clear_attention` 을 타지 않는다. 후자는 로컬 producer 축이다 — `raise_attention` 에는 위 "미러의 로컬 발동은 억제된다" 게이트가 **이미 있고**, `clear_attention` 은 해제 forward(mirror→server)가 붙을 자리다. 서버 push 의 적용이 같은 함수를 타면 서버 값이 그 억제 게이트에 막히고, 서버가 내려준 해제가 서버로 되돌아가는 에코가 된다.
