...
- **변경 신호 수신자 (아직 배선 없음)**: `markdown_changed` 는 **아직 발신자가 없다** — 서버가 실제로 내보내는 `event` 값(`attach_runtime.rs`/`stream_hub.rs`)에 이 이름은 없고, 이 절의 다른 불릿과 달리 가리킬 코드 좌표가 없다. 수신자를 **위 인가 술어가 참인 그 집합** — 즉 이 engine 의 워크스페이스를 하나라도 점유한 client 전부 — 으로 정하고 그 surface 를 담은 워크스페이스의 holder 로 좁히지 않는 것은 [ADR-0254](../adr/0254-markdown-attach-mirror-forwards-content-not-pixels.md) 항목 5 의 **결정**이며, 근거(좁히면 요청은 통과하는데 신호는 안 오는 client 가 생긴다 · 별도 구독 표를 두지 않는 이유도 같다 · 점유가 없으면 신호는 사라진다)도 거기 있다. 배선되면 이 불릿에 좌표를 채운다.
