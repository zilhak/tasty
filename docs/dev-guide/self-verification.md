...
`scan(true)`의 자리 목록을 도입 커밋 전후로 비교하고, 값·호출 위치·테스트 전용 여부를 확인한다.
...
제거한 대조도 실행한다. 전자는 테스트 전용이 아닌 치수의 판정에 나타나야 하고, 후자는 같은 기하가
테스트 전용 분류에서 제품 코드 분류로 이동해야 한다. detector의 두 회귀시험은 이 경계를 검사하며,
