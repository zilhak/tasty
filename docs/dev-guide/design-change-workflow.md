...
[1] planner ──작성──▶  디자인 요청문서  ──[2] 사용자 제출──▶  [3] Claude design       │
 (요청 산출물 정의)     (로컬 작업 폴더/            (사용자가 직접 제출)    (고충실 HTML/CSS 시안)  │
                        design-request/*.md)                                  │            │
                                                                              │ 시안 수령   │
                                                                              ▼            │
...
부분적인 자동 채널이 **둘** 있다. `crates/tasty-doc-guards/tests/site_vendor_tokens_track_the_app_export.rs` 가 두 사본의 **토큰 이름 집합**을, `crates/tasty-doc-guards/tests/site_vendor_icons_match_the_app_transcription.rs` 가 **아이콘 기하와 채움**을 대조한다 — 앱이 받은 것을 사이트가 못 받으면 그 차이가 명부와 어긋나 빨개진다. **그 둘이 덮는 범위와 이 절차가 덮는 범위는 다르다** — 판정기는 토큰 이름과 아이콘 기하 두 층만 보고, 그 둘을 안 건드리는 결정(문구 변경 · 구성 변경 · 컨트롤 삭제)은 양쪽 좌변을 똑같이 남겨두므로 **여전히 안 잡힌다.** 그 층은 이 단계(사람이 도는 재-vendoring)만 닫는다. 판정기가 초록인 것을 "사이트 사본이 최신" 으로 읽지 마라.
