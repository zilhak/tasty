...
☆ **왜 못 넣었는가는 아직 가설이다.** `GuiTestInstance::focus()`(`tests/gui_common/mod.rs`)에
**Linux 분기가 없다** — Windows·macOS 분기만 있고 Linux 는 sleep 만 한다. WM 없는 Xvfb 에서
그 창이 X 포커스를 갖는지 아무도 관리하지 않고, `enigo` 는 *그 순간 OS 포커스를 가진 무엇*에
넣는다. 이것이 소스에서 온 **가설**이고 직접 증거는 아직 없다 — 포커스를 보장해 그 셋만
다시 쏘면 갈린다. 갈리기 전까지 이 문단을 원인 판정으로 인용하지 마라.
...
위 원인이 갈리면 3 통과 / 30 실패를 원인별로 나눌 수 있다. 소스 전수로 각 시험이 누르는
조합을 뽑아 기본 preset(`preset_tasty` = `Default`)에 조인했다. 규칙 기반 조합(quick-switch
의 `<수정자> + 슬롯키`)도 preset 쪽에 펼쳐 넣었다.

| 갈래 | 수 | 이번 회차 결과 |
|---|---|---|
| 키 자극이 있고 **preset 밖 조합을 하나라도** 누른다 | **16** | 16 실패 |
| 키 자극이 있고 **누르는 조합이 전부 preset 안** | **10** | 10 실패 |
| 키 자극이 없다(마우스 전용) | **7** | 3 통과 · 4 실패 |

⇒ 30 실패 = **16**(조합 낡음) + **10**(조합은 멀쩡) + **4**(마우스). 33 = 26 + 7.
...
- 그리고 **열넷(10 + 4)은 조합 수정으로 설명되지 않는다.** 이쪽이 "기제가 정말 고장인가"
  라는 물음이 아직 소유하는 잔여다. 열 건은 `ctrl+,`·`ctrl+shift+i`·`escape` 만 누르는데
  — 셋 다 기본 preset 에 있다 — 그래도 전부 빨갛다.
