...
일반형으로 적으면 — **면제의 근거로 삼기 전에 세 가지를 순서대로 물어라.** ① 그 채널이
이 커밋을 보는가(트리거·경로 필터·push 여부), ② 그 잡이 지금 초록인가, ③ **그 조합이
이 물음을 판정하는가.** 셋째를 건너뛰면 꺼진 lint 와 안 도는 타깃이 통과로 읽힌다.
...
### 판정기가 아직 없는 축 — 시험이 자기 비용에서 퇴행하는 것

앞 두 갈래(기계가 판정할 수 있는데 안 도는 것 · 판정이 사람인 것)와 섞이지 않는
세 번째 갈래가 있다. **판정 자체가 없어서 채널을 물을 수도 없는 축**이다. 시험 하나가
같은 파일을 몇 번이나 다시 읽는가 — 그 축에는 게이트도, 시간 상한도, 회귀 시험도
없다. 그래서 어느 잡이 그것을 보는가라는 물음이 성립하지 않는다.
...
#### 가드를 지을 값이 있는가 — 세 형태와 각각이 무엇을 깨뜨리는가

- **정적 술어**("이름이 명사구인데 본문에 파일 I/O")는 오탐이 크다. 실측
  2026-09-08 로 출하 코드에서만 **27 건**, 시험까지 포함하면 **314 건**이 걸리는데
  그중 비싼 것은 0 이다. 이 형태를 하드 게이트로 걸면 정상 코드가 막힌다 —
  덜 잡는 결함보다 **옳은 수정을 막는 결함이 먼저**라는 규율에 정면으로 걸린다.
- **동적 상한 래칫**(레포 파일 최대 재열기에 상한)은 축을 정확히 겨냥하지만
  `strace` 가 필요해 **Linux 전용**이고, 전수 한 바퀴가 분 단위다. 크로스 플랫폼이
  1 급인 이 레포에서 한 OS 에서만 도는 게이트는 나머지 둘에서 미측정을 만든다.
- **자리 하나를 못박는 것**이 제일 싸다. 캐시가 있는 자리에서 도출 함수의 호출
  횟수를 세어 프로세스당 1 회 이하임을 시험으로 고정하면, 캐시를 지우는 커밋이
  그 자리에서 즉시 빨개진다. 일반형이 아니라 **아는 자리만** 지킨다.

셋 중 어느 것도 이 회차에서 짓지 않았다. 값은 "0 이 유지된다" 와 "그 0 을 지키는
것이 없다" 둘이고, 그 둘은 서로를 대체하지 않는다.
