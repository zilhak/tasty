...
☆ 미측정: 누적이 5 에서 멎는 이유는 안 쟀다.
☆ 열려 있는 자리: `press_alt` 는 Shift 를 안 보내고 `press_alt_shift` 헬퍼가 없다. preset 은
  `alt+w`=close_surface · `alt+shift+w`=close_workspace 다 — 뒷정리가 의도와 다른 액션에
  닿았을 수 있고, 같은 헬퍼 부재가 `ctrl+shift+o`(짝이 `alt+shift+e`)도 막고 있다.
...
### 여유 0 인 래칫은 **값이 아니라 delta 로 보고한다**

상한 래칫이 여유 0 이면(건수 == 상한) 어느 방향으로 움직여도 빨갛다. 그 상태에서
여러 갈래가 각자 자기 base 위에서 작업하면, **각 갈래가 초록인데 합친 트리만 넘치는**
형태가 생긴다. 두 갈래가 각각 하나씩 늘려도 **둘 다 같은 값을 보고한다** — 값은 그
갈래의 base 에서의 절대치라, base 가 다르면 갈래끼리 비교가 성립을 안 한다. 합치는
쪽은 그 보고들로 합친 결과를 예측할 수 없다. (CLAUDE.md 의 plugin 분할 착지 함정과
같은 부류다 — 거기서도 각자 규칙대로 했는데 합류 지점만 어긋난다.)
...
**diff 를 grep 해서 delta 를 세지 않는다 — 그 방식은 양방향으로 틀린다.** diff 의
바늘과 게이트의 바늘이 다른 것을 센다.
