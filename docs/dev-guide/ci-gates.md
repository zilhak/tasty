...
- **설정 필드** — 코드에 *사용자가 고르는 값*과 *내부 영속 슬롯*을 가르는 표시가 없다.
  ...
  대부분 누락이 아니라 어휘 차이다(가이드 "패인 분할 (좌우 / 상하)" ↔ `lang` "페인 수직
...
##### 그 진단은 지금까지 **0 회** 나왔다 (과거값) — 그리고 물음은 아직 현재형이다

두 시제를 갈라 적는다. **과거값은 값으로 적어도 낡지 않는다**; 현재형은 명령으로만 적는다
([ADR-0139](../adr/0139-numbers-in-docs-are-classified-by-lineage-not-by-name.md)).

- **과거값**: `command -v xvfb-run` 의 결과 줄은 **한 번도 나온 적이 없다.** 그 스텝이
  존재한 회차 **셋**에서 셋 다 `skipped` 였다 — 앞의 `cargo test (headless)` 가 실패하거나
  취소돼서다. 그보다 앞선 회차들에는 그 스텝이 **아예 없었다**(스텝 목록에 줄이 없다).
  즉 "0 회" 는 두 가지가 합쳐진 값이다: 스텝이 없던 구간 + 있었지만 건너뛰어진 구간.
- **현재형 물음**: *이 러너에 `xvfb-run` 이 있는가.* 여전히 **미측정**이다. 위 세 번의
  `skipped` 는 "없다" 의 증거가 아니다 — 물음이 아예 안 던져졌다.
