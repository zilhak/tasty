...
`9927130c` 실측) 기본 2025 / 헤드리스 1080 이고, **기본 조합에만 있는 949** 의 내역은:

| 헤드리스에서 사라지는 것을 무엇이 설명하나 | 수 | 비율 |
|---|---|---|
| **`#[cfg(feature = "gui")] mod …;` 아래에 있다는 것만으로 충분히 설명된다** | **909** | 95.8% |
| 게이트된 모듈 밖이므로 **항목별 cfg 밖에 원인이 없다** | 40 | 4.2% |

상위 기여: `adapters::ui` 463 · `view` 177 · `gfx` 31 · `app::attach_client` 30. src 안의
gui 게이트된 `mod` 선언은 67 개다.

**두 행의 근거 강도가 다르다.** 아래 40 은 배타적 귀속이다 — 게이트된 모듈 밖인데 사라졌으니
원인이 항목별 cfg 말고 없다. 위 909 는 **충분조건이지 유일 원인이 아니다** — 그중 몇이
자기 항목에도 cfg 를 달고 있는지는 재지 않았다(모듈 게이트 하나로 이미 사라지므로 채널
판정에는 영향이 없지만, "cfg 를 떼면 살아난다" 를 이 수로 추론하면 틀린다).
