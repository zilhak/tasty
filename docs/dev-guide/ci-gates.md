...
#### alt 조합은 "안 쟀다" 가 아니라 **이 스위트로는 못 잰다**

Ctrl 조합이 죽어 있을 때 alt 가 살아 있는지는 자연스러운 다음 물음인데, 이 파일로는
답이 안 나온다. `press_alt` 를 쓰는 시험이 열여섯인데 **그 열여섯 전부가 첫 alt 자극
앞에 ctrl 자극을 갖는다**(첫 alt 앞에 ctrl 이 없는 시험 0 건). 이름이 alt 를 가리키는
`test_workspace_switch_alt_number` 조차 Alt+1 에 닿기 전 `Ctrl+Shift+N` 셋업에서 죽는다.

⇒ 그 열여섯의 색은 전부 **ctrl 의 색**이다. alt 를 재려면 **첫 자극이 alt 인 시험**이
있어야 하고 지금 없다. 열여섯의 빨강을 alt 의 증거로 인용하지 마라.
