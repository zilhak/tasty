...
- **terminal**: winit 경로(`src/view/main/mouse.rs` `handle_right_button`)가 생산한다. mouse-tracking 위임(ADR-0019/0022) 판정이 여기 있어 winit-level 이어야 한다. **release 시점**에 큐잉한다(press 아님) — 버튼이 아직 눌려 있는 상태에서 `popup_at_rect()` 를 부르면 GTK 가 팝업을 realize/map 하지 않고 조용히 no-op 한다(실측 확인: `has_window`/`realized`/`mapped` 가 영원히 false 로 고정, X11 이벤트 큐에 CreateWindow/MapWindow 요청 자체가 안 나감). 비-terminal 이 egui `secondary_clicked()`(release 시점)로 이 문제를 애초에 안 겪는 것과 같은 이유로 맞춘 것 — layer 는 winit 그대로 유지하되 트리거 시점만 release 로 뒤로 민다.
