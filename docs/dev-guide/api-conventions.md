...
| 호출자가 실은 응답 대기 상한이 만료됐다 | `-32061` | **상태를 먼저 읽는다** (연결은 유지된다) |
...
- 봉투 검사(키 길이 1..=256)는 메서드와 무관하다 — **단 engine 라우터로 가는 요청에
  한해서다.** 그 안에서 길이 밖 키는 `Read` 든 `Mutate` 든 `-32602` 다. 검사가 보존소와
  같은 함수에 있어 아래 "무엇이 아직 안 걸리나" 의 좌변을 그대로 물려받기 때문이고, 그
  앞에서 끝나는 층에서는 **빈 키도 상한을 넘긴 키도 거절되지 않고 변경이 실행된다.**
  그래서 같은 봉투가 어디로 가느냐에 따라 유효하기도 무효하기도 한 상태가 지금 실재한다 —
  검사를 라우터 **앞**(`check_request`)으로 옮기는 것이 그것을 닫는 형태다.
...
**호출자가 요청마다 이 차이를 아는 값은 응답의 `idempotent_replay` 하나뿐이다.** 다만 그
표지는 **한 방향으로만** 답한다 — 붙어 있으면 계약이 걸린 것이지만, 안 붙은 것은 계약
밖이라는 뜻이 아니다. 안 붙으면서 계약은 걸린 경우가 셋이다:

- `-32063`(같은 키·다른 요청) — 에러라 표지가 `false` 인데, 그 코드 자신이 계약이 개입했다는
  증거다. 이것은 **실행을 막은** 답이다.
- `-32064`(실행은 됐고 답을 버림) — 같은 이유로 표지가 없고, 그 코드 자신이 "실행은 확실하다"
  를 말한다.
- 보존 범위 밖으로 밀려난 키 — 다시 실행되고 표지 없이 성공을 낸다. 그 경계는 위
  "보장의 경계는 값으로 선언된다" 가 값으로 내놓는다.

그래서 "표지 없는 재시도 = 계약 밖" 은 **성공 응답들 사이에서, 선언된 보존 범위 안에서만**
성립한다. 그 둘을 안 빼고 규칙을 문자 그대로 적용한 호출자는 `-32063` 을 "계약 밖" 으로 읽고
**키 없이 재전송**하는데, 그것은 계약이 막아 준 두 번째 효과를 스스로 실행하는 일이다.
메서드 단위 선언은 그 자리들이 배선된 뒤에 붙일 값이고, 배선 전에 선언하면 "착지한 것만
적는다" 를 어긴다.
