...
| plugin이 CLI를 제공한다 | `terminal.completion` · `terminal.completion_bind` | `tasty codex completion`이 codex.completion을 거쳐 호출한다. bind는 Network 권한이 추가된 host 메서드로 전달하고, 나머지 상태·구독·복구는 일반 메서드로 전달한다 |
