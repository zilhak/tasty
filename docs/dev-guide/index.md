...
| [unit-test-isolation](unit-test-isolation.md) | 유닛 테스트를 로컬 상태(홈 `config.toml` · env · 이 머신에만 있는 파일시스템 경로)로부터 격리하는 규칙 — 설정 주입 지점 + env RAII 가드 + 파일시스템 픽스처는 테스트가 직접 생성 + feature 별 테스트 게이팅 |
