...
| 문서 | 내용 |
|------|------|
| [e2e-tests](e2e-tests.md) | E2E 인스턴스 공유·환경 격리·timeout 정책 + GUI stderr 수명과 실패 진단 |
| [unit-test-isolation](unit-test-isolation.md) | 유닛 테스트를 로컬 상태(홈 `config.toml` · env · 이 머신에만 있는 파일시스템 경로)로부터 격리하는 규칙 — 설정 주입 지점 + env RAII 가드 + 파일시스템 픽스처 직접 생성·PID와 호출별 counter 격리·시계 nonce 한계 + feature 별 테스트 게이팅 + 병렬 경합(flake) 처방과 가드 검증(§7) + 공유 픽스처 `test_state()` 가 진짜 셸을 fork 한다는 것과 그 횟수를 세는 법(§8) |
