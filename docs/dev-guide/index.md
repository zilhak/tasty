...
| [guard-population](guard-population.md) | 모수가 걷기가 아니라 `const` 배열·fixture 에서 오는 가드 — drift 가 위험이고 비면 조용히 통과한다. 목록을 소스 추출로 바꾸는 절차(ADR-0133 이 못 다루는 갈래) · 푸시 tip 공용 모수 검사와 소비자 여유 재계측 + 직접 `read_dir` 자리의 갈래(순회의 뿌리로 가른다)와 세는 법 · 판정값이 0 이어도 정상인 뿌리는 판정값이 아니라 「봤다」로 잰다 |
