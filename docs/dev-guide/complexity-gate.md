...
- **중화로 못 고치는 오독이 하나 더 있고, 그것은 안고 간다 — 지금 25 줄.** `tokei` 는
  doc 주석(`///`·`//!`)에 딸린 줄을 **누락한다.** 실측(2026-09-09, 모수 = 동결 목록 23
  전량): 9 파일에서 code 를 합 **25 줄** 덜 세고, 같은 수의 blank 도 함께 잃으며, 잃은 줄은
  embedded Markdown 에도 안 들어간다(그쪽 code 는 9 파일 전부 0). 이 편향을 교정하지 않는
  이유는 **예산도 같은 계측기로 정해져 편향이 양변에서 상쇄**되기 때문이다 — 성장 탐지의
  좌변은 안 움직인다. 신호가 되는 것은 편향의 절대값이 아니라 그 **변동**(동결 파일에서
  doc 주석 블록이 드나드는 것)이고, 그 변동은 갈래 ㄷ 으로 나타난다. 재는 법은 둘째
  계측기 없이 같은 tokei 로 두 번 재는 것이다 — 사본에서 `grep -vE '^[[:space:]]*(///|//!)'`
  로 doc 줄을 뺀 code 와 원래 code 의 차가 그 파일의 편향분이다.
- **합이 내려가는 길은 넷이다 — 그중 둘만 예산을 내린다.** 실패 메시지가 갈래를 열거한다:
  ...
  틀린 한 번이 영구적이라, 요건을 못 대면 ㄷ 으로 읽는다.
