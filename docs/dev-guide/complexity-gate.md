...
| **파일 SLOC** | `tokei` + `scripts/check-file-size.sh` | **출하** code SLOC **1000** | `.complexity-file-allowlist` (현재 23개 — 도입 시 동결분 잔여 15 + 채널 부재로 쌓인 부채 8) |
...
| **계측 편향** | `tokei` 두 번 + 같은 스크립트 | doc 주석 편향 = 고정값 (**여유 0 · 양방향**) | 같은 파일의 `# doc-comment-bias:` 줄 |
...
- **동결의 성장은 자매 게이트가 본다 — `scripts/check-frozen-sum-ratchet.sh`.** `.complexity-file-allowlist` 는 **경로만** 담아 값이 없으므로, `check-file-size.sh` 만으로는 목록에 오른 파일이 얼마나 자라든 신호가 없다(실측: 두 달에 동결 18 중 **15 가 자라 +2406 줄**, 그중 `render.rs` 는 1002 → 1997). 그래서 목록에 오른 파일들의 출하 SLOC **합** 하나를 예산으로 두고 **양방향으로** 고정한다 — 합이 `예산 + 여유` 를 넘어도 실패하고, `예산` 아래로 내려가도 실패한다 (그 여유는 파일 임계와 **같은 값이어야 한다** — 둘이 구멍도 겹침도 없이 맞물리려면 파일 하나가 임계 아래로 남을 수 있는 폭과 합의 여유가 같아야 하고, 그래서 스크립트는 그 값을 적지 않고 `check-file-size.sh` 에서 읽어 온다)(그때는 예산을 내려 래칫을 조인다. 한 방향만 서는 것은 래칫이 아니다).
  ...
  - **저울은 하나다 — 항목 수준으로 가르지 않는다.** 합 하나만 보므로 **상쇄가 안 보인다**:
    실측(도입 3 일, 항목 24) 항목 11 개가 움직여 이동량 224 줄인데 총합 축이 본 값은 −14,
    이동량의 6% 였다. 그런데 항목마다 기준값을 두면 안 보는 구간이 **띠 × 저울 수**가 되어,
    같은 발화율에서 지금의 24 배가 된다(30 일 시뮬레이션: 총합 축 7 회 / 항목 축 같은 띠에서
    7 회에 구간 24 배, 구간을 맞추려 띠를 스무 분의 일로 줄이면 84 회). 근거·표·재검토 조건은
    [ADR-0205](../adr/0205-the-frozen-sum-stays-one-scale.md).
    - **다시 재는 법**: 60 일 커밋 계열에서 지금 목록의 각 항목을 커밋마다 `strip-cfg-test`
      + `tokei` 로 재고, 래칫을 그대로 돌린다(예산 = 러닝 최소, 띠 = B, 양방향). 총합 축은
      항목 합 하나로, 항목 축은 항목마다 따로 센 뒤 발화 수를 더한다. 두 축의 발화 수가
      같아지는 띠에서 `띠 × 저울 수` 를 비교하면 위 배수가 나온다.
...
- **오독은 계측기 쪽만이 아니다 — 판정기도 한 번 틀렸다.** `#[cfg(test)]` 스팬을 재는
  중괄호 수지를 **줄 하나씩** 세면 여러 줄에 걸치는 리터럴 안이 코드로 보인다. 실측
  (2026-09-10)으로 이 트리에 형태가 둘, 그것을 문 파일이 7 개 있었다 — 여러 줄 raw
  string(`r#"{…}"#`, 3 파일·전부 JSON 픽스처)과 `\` 로 이어붙인 여러 줄 문자열(4 파일).
  뒤쪽 중 셋은 가드 자신의 합성 픽스처라 그 문자열 **안의** `#[cfg(test)]` 가 속성으로도
...
- **그 사본은 계측기가 읽을 수 있는 형태로 넘긴다 — `--neutralize-char-literal-quotes`.**
  ...
  지운 줄이 다시 세어진다** — 값은 실제보다 크게 나오고, 계측기는 성공으로 끝나므로
  그 오진은 조용하다. 실측(2026-09-09): `src/core/attach_runtime.rs` 에 그 형태 한 자리가
  들어오자 이 게이트가 본 값이 1240 → 3046 이 됐는데 같은 구간의 원시 순증은 +364 였다.
  그래서 두 SLOC 게이트는 사본을 뜰 때 그 플래그로 문자 리터럴 **안의 한 글자**만
  중화한다(줄 수도 code 수도 안 움직인다). **내용 동등을 묻는 소비자
  (`scripts/check-plugin-version-bump.sh`)는 이 플래그를 쓰지 않는다** — 거기서 중화하면
  `'"'` 와 `'x'` 가 같아 보여 산출물이 달라졌는데 bump 를 안 요구한다.
- **중화로 못 고치는 오독이 하나 더 있고, 그 절대값은 안고 간다.** `tokei` 는 **doc 주석
  (`///`·`//!`) 바로 앞줄이 문자열 리터럴을 담고 있으면 그 줄을 어디에도 안 센다** — 사이에
  낀 빈 줄까지 함께 잃는다. 실측(2026-09-09, 모수 = 동결 목록 23 전량)에서 9 파일이 갈렸고
  code 손실과 blank 손실의 수는 서로 달랐다(잃는 blank 은 사이에 낀 빈 줄 수만큼이다).
  잃은 줄은 embedded Markdown 에도 안 들어간다(그쪽 code 는 9 파일 전부 0). 그때의 수와
  파일별 내역은 [ADR-0258](../adr/0258-the-measured-copy-is-neutralized-for-the-counter.md);
  **지금 값은 아래 좌변에 산다 — 이 문서는 그 수를 복제하지 않는다.** 절대값을 교정하지 않는
  이유는 **예산도 같은 계측기로 정해져 편향이 양변에서 상쇄**되기 때문이다 — 성장 탐지의
  좌변은 안 움직인다.
- **움직이는 것은 그 편향이고, 그래서 편향에 좌변이 붙어 있다 —
  `.complexity-file-allowlist` 의 `# doc-comment-bias:` 줄.** 게이트가 매번 같은 tokei 로
  두 번 재어(사본 · 그 사본에서 doc 줄만 뺀 것) 그 차를 **여유 0 으로** 고정한다. 양방향인
  이유는 두 방향의 위험이 다르기 때문이다: 편향이 **늘면** 합이 그만큼 내려가 미달 갈래로
  위장하고, **줄면** 합이 그만큼 올라가 띠 안에 통째로 들어가 **아무 데서도 안 운다.**
  움직였을 때의 처방은 예산과 편향을 **같은 폭으로 함께** 옮기는 것이다(합에 맞추면 같은
  커밋의 진짜 성장까지 사면된다). 게이트는 그 형태의 최소 재현(세 줄)도 매번 함께 재어,
  계측기가 오독을 고치거나 형태를 바꾸면 합·편향을 판정하기 전에 멈춘다.
- **합이 내려가는 길은 넷이다 — 그중 둘만 예산을 내린다.** 실패 메시지가 갈래를 열거한다:
  ㄱ 정말로 줄었다 · ㄴ 목록에서 항목이 빠졌다 · ㄷ 세는 술어가 깨져 **덜** 센다 ·
  ㄹ 세는 술어를 고쳐 **더** 세던 오독이 없어졌다. 예산을 내리는 것은 ㄱ 과 ㄹ 뿐이고,
  ㄷ 과 ㄹ 은 방향이 반대라 처방도 반대다. **관측은 같으므로 ㄹ 은 요건 둘로만 선다**:
  (1) 오독의 형태를 지목하고 그 형태의 **최소 재현을 시험으로** 남길 것, (2) 내려감이 그
  형태를 담은 파일에 **갇혀 있을 것**(미달 메시지가 파일별 내역을 전량 찍는다 — 고치기 전
  값과 견주는 표다). 변이 양방향(출하 +N · 테스트 전용 +0)은 온전성 확인일 뿐 판별법이
  아니다 — 특정 형태만 덜 세는 회귀는 그 둘을 모두 통과한다. 예산은 되돌아 올라가지 않아
  틀린 한 번이 영구적이라, 요건을 못 대면 ㄷ 으로 읽는다. **이 목록에 왔다는 것은 편향
  좌변이 초록이라는 뜻이다** — doc 주석 편향의 이동은 그 좌변이 앞에서 잡으므로 여기 오는
  시점에는 ㄷ 의 그 형태가 배제돼 있다.
  근거·기각한 대안(상시 독립 계측 대조 · 차이의 상한 래칫 · 미달 갈래 진단 열): [ADR-0258](../adr/0258-the-measured-copy-is-neutralized-for-the-counter.md).
  - **파일 전체**가 출하 밖인 경우 중 `#[cfg(test)] mod` 선언으로만 닿는 파일과 cargo
    통합 타깃은 판정기(`strip-cfg-test --blank-test-only-files`)가 통째로 빈 줄로 만든다
    ([ADR-0345](../adr/0345-the-file-sloc-gate-erases-whole-test-only-files.md) — 인라인
    범위를 지우는 위 결정은 ADR-0165 이고, 0165 는 이 축을 미뤄 두었다). 생성·전사 코드는 판정기가 안 지우므로 스크립트 `skip()` 의 이름 글롭에서만 빠진다.
    `skip()` 의 테스트 이름 갈래는 판정기와 **겹친다**. 그 대리인은
...
- **allowlist 안에 블록이 둘이다.** 위 15 건은 게이트 도입(2026-07-06) 시점에 동결한 18 건에서 래칫으로 세 건이 빠진 나머지이고, 아래 8 건은 게이트가 한 번도 실행되지 않은 60 일 동안 새로 임계를 넘은 것이다 — **정당화된 예외가 아니라 부채 대장**이다(도입 시점에 이미 초과였던 것은 0 건). 새 항목은 위 블록에 넣고, 아래 블록은 래칫으로 **지우기만 한다**. 근거는 [ADR-0131](../adr/0131-file-sloc-gate-needs-a-firing-trigger.md).
...
**게이트는 이미 선언 기반 판정을 부른다 — (나)가 겨냥한 사건은 임계 경로로는 더 안 일어난다.** 이 절은 원래 "(나)가 울면 선언 파서를 게이트 안에 들인다" 로 그 비용을 미뤄 두었다. 그 비용은 이미 치렀다: 게이트가 판정기에 `--blank-test-only-files` 를 넘겨(`scripts/check-file-size.sh` 의 `SHIPPING_JUDGE_FLAGS`) 선언 기반 판정(`shipping_scope::test_only_files`)이 선언상 출하 밖인 파일을 이름과 무관하게 0 줄로 만든다([ADR-0345](../adr/0345-the-file-sloc-gate-erases-whole-test-only-files.md)). 그래서 (나)의 모수 — 선언상 출하 밖인데 이름 때문에 게이트가 *재는* 파일 — 는 0 줄로 재어져 임계에 닿을 수 없고, 게이트가 그 파일의 등재를 **강제하는** 일은 없다. (나)가 우는 조건은 SLOC 이 아니라 그 모수의 파일이 `.complexity-file-allowlist` 에 **적혀 있는가**이므로, 누가 그런 파일을 목록에 손으로 적으면 (나)는 여전히 운다(가드 자신의 변이 시험이 그 경우를 단언한다). 다만 그때 뜻하는 것은 이 절이 말한 "대리인이 비용이 된 날" 이 아니라 불필요한 등재다. `skip()` 의 이름 글롭은 대리인으로 남아 있고 (가)는 여전히 개명 우회를 본다. **(나)를 무엇으로 다시 정의할지는 정해지지 않았다** — 그것은 `src/source_guards/sloc_gate_skip_proxy.rs` 의 코드 변경이 걸려 별도로 다룬다. 그때까지 이 문서의 (나) 서술과 그 가드의 (나) 주석은 그 모수가 임계로 등재될 수 있다는 전제 위에 서 있고, 그 전제는 지금 참이 아니다. 현재 수치는 이 문서에 적지 않는다 — 문서에 적힌 수는 낡고, 필요한 수는 (가)의 단정 메시지가 발화 시점에 그 자리에서 알려준다.
