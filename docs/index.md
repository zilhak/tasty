...
Workspace / Tab / Pane 레이아웃과 leaf surface 초기화 파라미터(kind, cwd, 시작 명령어, kind 별 params)를 디스크(`~/.tasty/presets/{workspace,tab,pane}/<name>.toml`)에 저장/재사용. 우클릭 메뉴/단축키로 저장·적용, PresetView 에서 편집, IPC `preset.*` 7종 + CLI `tasty preset {list,get,save,delete,rename,capture,apply}` 노출. CLI/IPC apply 는 항상 포커스 이동 없음. 상세: [features.md](features.md).
