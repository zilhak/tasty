...
### 워크스페이스 & 탭
GUI 사이드바 기반 워크스페이스 관리. 여러 에이전트 세션을 독립적으로 관리.

**현재 구현된 기능:**
- Workspace/Pane/SurfaceGroup/SurfaceNode 계층 데이터 모델
- egui 좌측 사이드바 (워크스페이스 목록) + 상단 탭 바 (Pane 전환)
- SurfaceGroup 트리 기반 재귀적 수평/수직 분할 및 scissor rect 렌더링
- 키보드 단축키: Ctrl+Shift+N(워크스페이스), Ctrl+Shift+T(Pane), Ctrl+Shift+E/O(분할), Alt+1~9(WS전환), Ctrl+Tab(Pane전환), Alt+Arrow(포커스)
...
### 분할 패인
GPU 렌더링 분할 패인. SurfaceGroup 트리 기반 재귀적 분할, scissor rect 렌더링. 기본 구현 완료.
