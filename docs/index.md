...
**현재 구현된 기능:**
- clap 기반 서브커맨드 그룹: new {window|workspace|tab|split|markdown|explorer}, close {tab|pane|surface}, claude {launch|spawn|children|parent|kill|respawn|broadcast|wait|hook}, list, select-workspace, send, send-key, notify, notifications, tree, surfaces, panes, info
