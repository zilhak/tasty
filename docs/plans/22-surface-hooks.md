...
```
tasty set-hook <surface_id> <event> <command>
tasty list-hooks [surface_id]
tasty unset-hook <hook_id>
```
...
tasty set-hook 3 process-exit "tasty notify 'Build finished'"

# Surface 5의 출력에서 "error" 패턴 감지 시 명령 실행 (향후)
tasty set-hook 5 output-match --pattern "error" "tasty notify 'Error detected'"
...
2. `tasty set-hook 3 process-exit "tasty send-keys 5 'deploy.sh\n'"` 등록
