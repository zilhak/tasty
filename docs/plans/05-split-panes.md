...
- SurfaceLayout 이진 트리로 관리한다.
- Panel::Terminal이 자동으로 Panel::SurfaceLayout으로 변환된다.
- 탭을 전환하면 SurfaceLayout 전체가 함께 전환된다.
...
`surface_regions(rect)` 메서드로 각 터미널 서피스에 할당할 영역을 계산한다.
...
4. Panel이 SurfaceLayout이면 내부 분할에 따라 여러 터미널을 렌더링한다.
...
| Surface | focused_surface (SurfaceId) | SurfaceLayout 내부 이동 |
...
| Ctrl+D | SurfaceLayout 수직 분할 (탭 내부) |
| Ctrl+Shift+D | SurfaceLayout 수평 분할 (탭 내부) |
...
- SurfaceLayout도 재귀적으로 하위 터미널을 리사이즈한다.
- PTY에 새 크기를 통보한다.

## 구현 현황

- PaneNode 이진 트리 기반 Pane 분할 구현 완료
- SurfaceLayout 이진 트리 기반 탭 내부 분할 구현 완료
- Panel::Terminal → Panel::SurfaceLayout 자동 변환 구현 완료
