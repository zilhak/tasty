...
- **reload 의 거절 보고**: `rejected` 는 이번 reload 가 버린 user 항목의 배열이다 — `[{"id", "reason"}]`, 없으면 `[]`. 사유는 `missing_owner_prefix`(id 에 `<owner>/` 접두사가 없어 설치 안 함)와 `missing_detector_or_action`(모든 출처를 겹쳐도 detector 나 action 이 없어 등록 안 됨) 둘이다. 파일을 읽거나 파싱하지 못해 reload 가 멈춘 경우는 이전 user 설정이 남고 `rejected` 는 빈 배열이다. 근거 [ADR-0426](../../adr/0426-file-handler-reload-reports-the-entries-it-dropped.md).
