...
요청이 묶일 워크스페이스는 **명시 `workspace_id` → `surface_id` 가 사는 워크스페이스 → 그 창의 활성 워크스페이스** 순으로 정한다. surface 를 댄 요청은 사용자가 어느 워크스페이스를 보고 있든 같은 곳에 묶인다. 둘 다 안 준 요청만 활성 워크스페이스로 떨어지고 그 값은 재현되지 않는다 — 재현이 필요하면 둘 중 하나를 준다([ADR-0617](../../adr/0617-workspace-identity-and-focus.md)). 이 값은 아래 영속 스코프와 함께 뜨는 알림의 워크스페이스를 정한다. popup 자체는 창 단위라 이 값에 안 걸린다.
