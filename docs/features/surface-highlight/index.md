...
- **해제(clear)**: `clear_surface_highlight(surface_id)`. 매 렌더 프레임 **실제 렌더 시점 포커스**
  surface 에 대해 자동 호출(`gpu.rs`) — 에이전트가 주입한 포커스가 아니라 실 사용자 포커스라
  불가침 원칙 1 에 안전.
...
- **AI Agent (IPC/CLI)**: `tasty surface completion --surface <id>` / IPC `surface.completion`
  ...
  권한 `Notification`.
- **사용자 트리거**: 직접 트리거 없음. 효과는 세 소비처로 표시되고, surface 에 포커스하면 해제.
