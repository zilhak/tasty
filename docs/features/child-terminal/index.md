...
- **Status**: Implemented
- **주체**: AI Agent
- **ADR**: [ADR-0621](../../adr/0621-occupancy-and-attach-admission.md) (soft 점유 소비자)
...
- **soft 점유 연결**: spawn 성공 시 `occupy_soft(child, parent)`, kill 시 `release_occupancy(child)`, release 시 `release_soft_occupancy(child, parent)` 를 **in-process core 함수**로 호출한다 (`occupancy.*` IPC method 는 없다 — ADR-0621 경계). soft 점유는 표시만 하고 입력을 차단하지 않으며(`attached=false`), parent 가 죽으면 focus 지연 청소로 풀린다.
...
설계 근거는 [ADR-0641](../../adr/0641-agent-state-and-completion.md)이다.
