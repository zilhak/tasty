...
- **타입어헤드 상태**: `type_ahead`(입력 버퍼 + 마지막 입력 시각)와 `scroll_to: Option<PathBuf>`(이번 프레임에 보이게 할 항목). `scroll_to` 는 **한 프레임만 산다** — 남겨두면 매 프레임 재스크롤이 되어 사용자가 휠로 다른 곳을 보는 동안 끌려간다. 규칙(순환 시작점·접두사·되돌리기)은 egui 도 파일시스템도 모르는 순수 모듈(`explorer/type_ahead.rs`)에 있고, 그리는 쪽은 결과를 선택과 스크롤로 옮기기만 한다.
...
### 사용자 트리거 (타입어헤드 — 단축키가 아니다)

목록에 포커스가 있을 때 영숫자를 치면 그 글자로 시작하는 항목이 선택되고 그 항목이 보이도록 스크롤된다(grid/list/detail 세 뷰 모두). 같은 글자를 이어 누르면 후보를 순환하고(마지막 다음은 처음으로), 서로 다른 글자를 이어 치면 접두사 검색이 된다. 마지막 입력으로부터 1 초가 지나면 버퍼를 버리고 다음 글자를 새 검색으로 읽는다. 매칭이 없으면 선택도 버퍼도 그대로다 — 오타 한 글자가 그 뒤 입력을 막지 않는다. 비교는 대소문자를 무시하고, `..` 는 렌더 전용 행이라 대상이 아니다.

**`KeybindingSettings` 필드가 없다.** 키 조합 → 액션 바인딩이 아니라 문자 입력 스트림 소비이기 때문이다(위 표와 다른 범주). 대신 사용자가 **수식 없는(또는 shift 만 붙은) 영숫자 바인딩**을 직접 만들면 그 글자는 타입어헤드가 소비하지 않고 단축키에 양보한다 — 한 키 입력이 둘 다 발화하기 때문이다(`Event::Text` 는 단축키 소비보다 먼저 egui 큐에 들어가고, 큐에서 빼는 경로가 없다. [key-mapping](../../design/policies/key-mapping.md) 의 "텍스트 입력과 단축키의 우선순위"). Tasty 프리셋 기본값에는 그런 바인딩이 하나도 없다.

버퍼를 소비하지 않는 조건 넷 — 이 중 하나라도 참이면 그 프레임의 글자는 무시되고 **버퍼도 비워진다**(돌아왔을 때 옛 접두사가 이어지면 사용자가 치지 않은 글자로 검색하는 것이 된다):

- 그 explorer surface 가 포커스된 surface 가 아니다 — egui 이벤트 큐는 윈도우 전역이라, 이 게이트가 없으면 한 번의 타이핑이 열려 있는 모든 explorer 의 선택을 동시에 움직인다.
- 모달·팝업·입력 다이얼로그가 떠 있다(`keyboard_overlay_open`).
- 주소창을 편집 중이다(`addr_editing`) — 그 글자는 `PathField` 가 받는다.
- 목록이 `LoadState::Ok` 가 아니거나 비어 있다.

디렉토리·정렬이 바뀌어 목록이 다시 적재되거나(`sync`), cwd/내부 탭을 바꾸는 액션이 적용되면(주소창 편집 취소와 같은 자리) 버퍼를 버린다 — 인덱스의 의미가 달라지기 때문이다.
