...
1. **남용차단 선검사** — 출처 IP 가 쿨다운 중이면 즉시 `429`. 경로 파싱·헤더 수집·**body 읽기보다 앞**이라, 차단된 출처의 body 는 메모리에 올라가지 않는다. 그 순서는 타입으로 강제된다(선검사를 통과한 요청만 body 를 읽을 수 있다 — [ADR-0199](../../adr/0199-the-block-is-decided-before-the-body-is-read.md)).
2. path/query 분리, 헤더 소문자 정규화, **바디를 요청당 상한까지만** 읽어 JSON 파싱(실패 시 `null`). 선언된 `Content-Length` 가 상한을 넘으면 한 바이트도 읽지 않고, 길이 선언이 없는 chunked 는 상한 + 1 바이트에서 멈춘다 — 둘 다 `413`([ADR-0200](../../adr/0200-webhook-body-has-a-per-request-byte-cap.md)).
...
요청 하나가 읽는 body 는 **기본 1 MiB** 까지다. UTF-8 변환 전에 읽은 바이트 수로 초과를 판정하므로 무효 UTF-8 chunked body 도 같은 상한을 적용한다. 넘으면 `413` 이고 시퀀스는 돌지 않는다. 등록 여부·인증 여부와 무관하게 매칭보다 앞에서 판정하므로, **없는 경로로 보낸 큰 body 도 `413` 으로 선거부한다.**

- `TASTY_WEBHOOK_MAX_BODY_BYTES` 로 조정한다(0·파싱 실패는 기본값). 수십 MB 짜리 payload 를 보내는 발신자를 붙이려면 이 값을 올린다.
- **요청당 상한이다** — 동시에 들어오는 요청 수는 이 값이 묶지 않는다(최악 점유 = 상한 × 동시 요청 수).
