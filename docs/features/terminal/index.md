...
**소거 명령과 걸친 커서**: ED/EL 은 **커서를 움직이지 않는 소거 연산**이고, 지우는 범위는 커서가 올라앉은 칸을 **포함**한다. 자동 줄바꿈이 대기 중인 상태("행 끝에 걸친 커서")는 커서 열이 화면 폭과 같은 값으로 나타나지만 그 칸은 그리드에 없다 — 커서가 실제로 올라앉은 칸은 마지막 열이고 줄바꿈은 다음 글자를 찍을 때로 미뤄져 있다. 그래서 범위를 셀 때 걸친 커서는 **마지막 열로 친다**: EL1(`CSI 1K`)은 행 전체를 지우고 그보다 한 칸 더 지우지 않는다(한 칸이 더 찍히면 그것이 다음 행으로 넘어가 **소거 명령이 줄바꿈을 일으킨다** — 부분 영역에서는 영역 스크롤 한 번 + 빈 이력 한 줄로 드러난다). 반대쪽 끝도 같은 규칙이다 — 0 열 커서에서 EL1 이 지우는 칸은 0 이 아니라 **하나**다. 걸친 상태는 소거 뒤에도 보존되어 다음 글자가 정상적으로 줄바꿈된다.

| 명령 | 걸친 커서(`cx == cols`)에서 | 0 열에서 | 커서 |
|---|---|---|---|
| EL0 `CSI 0K` | **마지막 한 칸을 안 지운다**(미해결) | 해당 없음 | 보존 |
| EL1 `CSI 1K` | 행 전체 | 한 칸 | 보존(걸친 상태까지) |
| EL2 `CSI 2K` | 행 전체 | 행 전체 | 보존(걸친 상태는 마지막 열로) |
| ED0 `CSI 0J` | **커서 행의 마지막 한 칸을 안 지운다**(미해결) | 해당 없음 | 보존 |
| ED1 `CSI 1J` | 커서 행 전체 | 한 칸 | 보존(걸친 상태까지) |
| ED2 `CSI 2J` | 화면 전체 | 화면 전체 | 보존(걸친 상태는 마지막 열로) |

"보존(걸친 상태는 마지막 열로)" 은 완전한 보존이 아니다. EL2·ED2 는 소거를 마친 뒤 커서가 제자리에 없어 구현이 원래 열로 되돌리는데, **그 까닭은 둘이 서로 다르다** — ED2 는 termwiz 의 `ClearScreen` primitive 자신이 채우기를 마치고 커서를 홈으로 보내고, EL2 는 그 primitive(`ClearToEndOfLine`)가 커서를 **안 옮기며** 커서 열부터 지우므로 **구현이** 행 전체를 지우려고 먼저 0 열로 옮긴 것이다. 되돌릴 때 걸친 자리는 커서 이동으로 가리킬 수 없고(폭과 같은 열은 마지막 열로 잘린다) 다시 걸치게 할 글자를 찍으면 그 한 칸만 pen 이 달라진다. 그래서 걸친 커서는 마지막 열로 내려앉고 대기 중이던 줄바꿈이 풀린다 — 0 열·홈으로 끌려가던 것보다 가깝지만 같지는 않다.

남은 "미해결" 은 EL0·ED0 둘이고 축이 다르다 — 커서가 아니라 **범위**다. 둘 다 지울 범위를 termwiz 의 primitive 가 커서 열에서 정하는데(`ClearToEndOfLine` · `ClearToEndOfScreen` 이 커서 열부터 행 끝까지를 채운다), 걸친 커서에서는 그 열이 그리드 밖이라 그 구간이 비어 마지막 한 칸이 범위에 안 든다. 고치려면 그 한 칸을 손으로 찍어야 하고 그러면 소거 직후의 pen(색·속성)이 그 칸에서만 달라지므로, 소거가 어떤 pen 으로 지우는가(BCE) 를 먼저 정해야 하는 별개 결정이다.
