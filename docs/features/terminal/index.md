...
| EL2 `CSI 2K` | 행 전체 | 행 전체 | **0 열로 옮긴다**(미해결) |
| ED0 `CSI 0J` | **커서 행의 마지막 한 칸을 안 지운다**(미해결) | 해당 없음 | 보존 |
| ED1 `CSI 1J` | **커서 행을 통째로 안 지운다**(미해결) | 한 칸 | 보존(걸친 상태는 풀린다) |
| ED2 `CSI 2J` | 화면 전체 | 화면 전체 | **홈으로 옮긴다**(미해결) |

"미해결" 로 적힌 넷은 형태가 둘로 갈린다. ED1 은 EL1 과 같은 범위 계산을 쓰면서 경계만 반대로 걸린 것이고, EL0·ED0·ED2 는 범위·커서를 termwiz 의 지우기 primitive 가 정하는 자리라 고치려면 그 primitive 를 손으로 쓴 텍스트로 갈아타야 한다 — 그러면 소거 직후의 pen(색·속성)이 함께 바뀌므로 별개 축의 결정이다. EL2 는 커서를 되돌리지 않는 것 하나뿐이다.
