...
"미해결" 로 적힌 셋은 형태가 둘로 갈린다. EL0·ED0·ED2 는 범위·커서를 termwiz 의 지우기 primitive 가 정하는 자리라 고치려면 그 primitive 를 손으로 쓴 텍스트로 갈아타야 한다 — 그러면 소거 직후의 pen(색·속성)이 함께 바뀌므로 별개 축의 결정이다. EL2 는 커서를 되돌리지 않는 것 하나뿐이다.
