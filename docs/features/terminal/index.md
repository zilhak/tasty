...
"보존(걸친 상태는 마지막 열로)" 은 완전한 보존이 아니다. EL2·ED2 는 termwiz 의 지우기 primitive 로 지우고 그 primitive 가 커서를 0 열·홈으로 끌고 가므로 구현이 원래 열로 되돌리는데, 걸친 자리는 커서 이동으로 가리킬 수 없고(폭과 같은 열은 마지막 열로 잘린다) 다시 걸치게 할 글자를 찍으면 그 한 칸만 pen 이 달라진다. 그래서 걸친 커서는 마지막 열로 내려앉고 대기 중이던 줄바꿈이 풀린다 — 0 열·홈으로 끌려가던 것보다 가깝지만 같지는 않다.

남은 "미해결" 은 EL0·ED0 둘이고 축이 다르다 — 커서가 아니라 **범위**다. 둘 다 지울 범위를 termwiz 의 primitive 가 커서 열에서 정하는데, 걸친 커서에서는 그 열이 그리드 밖이라 마지막 한 칸이 범위에 안 든다. 고치려면 그 한 칸을 손으로 찍어야 하고 그러면 소거 직후의 pen(색·속성)이 그 칸에서만 달라지므로, 소거가 어떤 pen 으로 지우는가(BCE) 를 먼저 정해야 하는 별개 결정이다.
