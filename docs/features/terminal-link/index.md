...
- **ADR**: [URL 대상은 picker·실행 계층에만](../../adr/0031-file-handler-routing.md) (원칙은 [identity](../../identity.md) §1)
...
- **게이트**: hover 링크가 우클릭한 surface 의 것이고, 그 surface 가 hard 점유 mirror 가 아닐 것(좌클릭과 같은 배제 — ADR-0021). `LinkModifier::None` 이어도 뜬다 — `hovered_link` 자체가 수식키 게이트를 통과한 결과이고, 좌클릭이 `None` 을 배제하는 이유("수식키 없는 클릭이 링크를 열어버리는 사고")가 우클릭 → 메뉴 → 항목 선택에는 성립하지 않는다.
