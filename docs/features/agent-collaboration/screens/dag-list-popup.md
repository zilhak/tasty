...
| 검색·필터 줄 | 검색 입력(이름 / workspace 이름) · 상태 드롭다운(`Any status` + 8 종) |
| 토글 줄 | "이 워크스페이스만" 체크박스 |
...
- **rollup 상태** — 노드와 **같은 8 종 어휘**로 글리프 + 철자를 함께 쓴다. 행과 그 행이 여는
  그래프가 서로 다른 말을 하지 않게 하기 위함이다. 라벨 색은 상태 accent 가 아니라 `-label`
...
보이지 않게 하기 위함이다. 이 배치 차이의 정합은
`.claude-workspace/todo/27-dag-graph-visual-parity-fix.md` 가 다룬다.
