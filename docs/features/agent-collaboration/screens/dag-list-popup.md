...
`Esc` 는 **안쪽부터** 소비된다 — 상태 필터 드롭다운이 열려 있으면 그 한 번은 드롭다운 몫이고
창은 남는다. 창의 `Esc` 가드가 본문보다 먼저 키를 읽으므로, 드롭다운이 열려 있는 프레임에는
창을 닫는 갈래를 양보하고 본문을 그대로 그린다(드롭다운이 자기 차례에 키를 소비하고 닫는다).
가드가 본문을 건너뛰면 그 프레임에 `Esc` 를 받을 주인이 사라져 드롭다운이 영영 안 닫힌다.

디테일은 surface 와 같은 렌더 함수(`draw_dag_graph`)를 그대로 부른다. 대상(`dag_id` /
`direction`)만 popup 이 자기 상태에서 빌려준다 — 두 벌로 갈라지면 토큰·구조 정합이 따로 논다.
popup 폭(560)이 상세 도킹 임계값(640) 아래라 노드 상세는 **항상 하단 시트**다 — 그래프 화면이
이미 가진 폭 분기가 그대로 작동한 결과다.

**크롬은 back bar 가 든다.** 같은 함수를 부르되 `DagChrome::BackBar` 로 부르면 헤더 띠도
캔버스 우하단 줌 클러스터도 그리지 않는다 — 한 노드를 보는 창에 띠가 둘이 되지 않게 하기
위함이다. 그 대신 back bar 의 actions 슬롯이 줌 클러스터와 러너 배지를 들고, 거기서 눌린
조작을 본문 호출이 받아 적용한다(back bar 가 본문보다 먼저 그려지므로 같은 프레임 안에
답이 나와 있다). DAG 선택기는 디테일에서 자리째 없다.
...
배지를 든다. 렌더는 여전히 그래프 화면 한 벌이고, **크롬을 누가 드는가**만 `DagChrome` 두
