...
탭/분할을 만들 때 PTY 를 **지연 생성**할 수 있다. 이 경우 트리 leaf 는 `deferred_spawn` 을 가진 `EmptySurface` placeholder 로 들어가고(빈 layout 이 아님), 탭이 처음 활성화될 때 `ensure_initialized` 가 PTY 를 띄워 `TerminalSurface` marker 로 교체한다. 외부(IPC `surface.list`, 트리 JSON)에는 `type:"Terminal"`, `pty_ready:false` 로 보고된다 — 아직 안 뜬 터미널 자리.
