...
- **등록된 kind 는 `surface.kinds` / `tasty list surface-kinds` 로 묻는다.** 이 조회가 읽는 것은 매니페스트가 아니라 `SurfaceKindRegistry` — 런타임의 사실이다. 여기 나오는 kind 가 정확히 `--type <kind>` 로 만들 수 있는 kind 이고(끄거나 지운 plugin 의 kind 는 곧바로 빠진다 — [ADR-0534](../../adr/0534-a-disabled-plugins-surface-kinds-are-withdrawn-not-erased.md)), host 내장 4 종도 함께 나온다(그쪽은 plugin 이 아니라 어느 `plugin.*` 조회에도 안 나온다). 칸은 kind · 표시명 i18n 키 · icon · **실제** 렌더 경로(`rendering`: `host-egui`/`egui-mesh`/`webview`/`remote`) · 출처(`source`: `host`/`plugin` + `plugin_id`) · 필수 params 다.
