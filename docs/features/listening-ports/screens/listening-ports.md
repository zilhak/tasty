...
- **팝업 프레임**: 660×520 (디자인 canonical), 제목 "Listening ports".
- **헤더 행**:
  - 좌: `전체 보기 (system)` 체크박스 — scope 토글 (Tasty ↔ System).
  - 태그: `{listening} listening` — 현재 scope 의 listening 포트 수.
  - 우: 단일 라인 검색 입력.
- **테이블 (7컬럼)**: Port / Proto / Address / Process / Workspace / Tab / State.
  ...
  - Port 셀: 클릭 가능 (브라우저 오픈).
- **footer**: `{shown} of {total} ports` 카운터.
