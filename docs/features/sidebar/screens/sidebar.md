...
- **하단 버튼** (각각 다른 기능으로 위임 — 여기선 링크만):
  - **도구** (`icons::TOOLS`) → `features/tools-menu/` *(재작성 예정)*
  - **플러그인** (`icons::PLUG`) → `features/plugin-system/` *(재작성 예정)*
  - **설정** (`icons::SETTINGS`) → `features/settings/` *(재작성 예정)*
