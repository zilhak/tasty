...
- **좌측 클러스터**(표시 전용): 브랜치 점(`accent_success`, 지름 `statusbar-dot-size` 6)+이름 · surfaceId · `<셸> · <cols>×<rows>`.
...
- **우측 클러스터**(clickable): 팔레트 칩(`<단축키> palette`) · 테마 토글(글리프 + 테마명).
...
- **테마 light / dark** — 토글 글리프(해 / 테마). 색은 두 쪽이 같다.
