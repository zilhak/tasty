...
**연결이 끊기면 정리만 하고, 재연결은 자동이 아니다**: heartbeat TTL 만료·force-detach 등 원격발 disconnect 로 세션이 끊기면 `cleanup_mirror_workspace`(`src/app/attach_client.rs`)가 mirror workspace/터미널/터널을 정리하고 앵커의 재attach 게이트(`auto_attach_active`)를 푼다 — 여기까지는 자동이다. 그러나 **자동 재연결(재attach 시도)은 하지 않는다**: disconnect 로 정리된 앵커는 워크스페이스가 활성화되는 **전환 프레임**(다른 워크스페이스에서 이 워크스페이스로 옮겨온 순간)에만 트리거되고, "이미 활성 상태로 계속 남아있는 것"은 트리거가 아니다(`src/app/auto_attach.rs::maybe_trigger_auto_attach` 의 재진입 대기 게이팅, [`dev-guide/attach-behavior` "GUI 자동 재연결 스코프"](../../dev-guide/attach-behavior.md#gui-자동-재연결-스코프)). 즉 사용자가 그 워크스페이스를 보고 있는 동안 조용히 끊기면, **사용자가 다른 워크스페이스로 갔다가 돌아오는 등 명시적으로 재진입해야** 재attach 가 시도된다 — 끊기자마자 사용자 조작 없이 알아서 재연결을 재시도하지는 않는다. 이 전환 요구는 **disconnect 를 겪은 앵커에만** 적용된다 — 매핑을 방금 새로 설정한(disconnect 이력이 없는) 앵커는 워크스페이스 전환 없이도 즉시 트리거된다.
