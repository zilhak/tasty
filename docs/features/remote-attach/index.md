...
**연결이 끊기면 정리만 하고, 재연결은 자동이 아니다**: heartbeat TTL 만료·force-detach 등으로 세션이 끊기면 `cleanup_mirror_workspace`(`src/app/attach_client.rs`)가 mirror workspace/터미널/터널을 정리하고 앵커의 재attach 게이트(`auto_attach_active`)를 푼다 — 여기까지는 자동이다. 그러나 **자동 재연결(재attach 시도)은 하지 않는다**: 트리거는 앵커 워크스페이스가 활성화되는 **전환 프레임**(다른 워크스페이스에서 이 워크스페이스로 옮겨온 순간)에만 일어나고, "이미 활성 상태로 계속 남아있는 것"은 트리거가 아니다(`src/app/auto_attach.rs::maybe_trigger_auto_attach` 의 엣지 판정, [`dev-guide/attach-behavior` "GUI 자동 재연결 스코프"](../../dev-guide/attach-behavior.md#gui-자동-재연결-스코프)). 즉 사용자가 그 워크스페이스를 보고 있는 동안 조용히 끊기면, **사용자가 다른 워크스페이스로 갔다가 돌아오는 등 명시적으로 재진입해야** 재attach 가 시도된다 — 끊기자마자 사용자 조작 없이 알아서 재연결을 재시도하지는 않는다.
