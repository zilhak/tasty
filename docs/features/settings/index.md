...
- **General**
  ...
  - Permissions(macOS 전용 — 권한 상태 표시, [모든 권한 요청하기](파일 폴더와 화면 기록을 한 번에 요청하며 요청 중에는 비활성), 전체 디스크 접근 권한 패널 바로가기. 권한 요청이 시작되는 유일한 화면이다 → [macos-permissions](../macos-permissions/index.md)).
