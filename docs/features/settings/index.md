...
- **General**
  ...
  - Permissions(macOS 전용 — 권한(TCC) 상태 표시 + [모든 권한 요청하기](파일 폴더·화면 기록 일괄 요청, 요청 중 비활성) + 전체 디스크 접근 권한 패널 바로가기. 권한 요청이 시작되는 유일한 자리다 → [macos-permissions](../macos-permissions/index.md)).
