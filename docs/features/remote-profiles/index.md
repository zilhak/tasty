...
| CLI (프로필 CRUD) | `tasty tool remote-profile list\|show\|add-ssh\|add-attach\|edit\|remove\|detect` — ssh + tasty-attach 통합. `add-ssh`(순수 연결) / `add-attach`(`--ssh-ref` XOR 인라인 + `--remote-tasty`/`--port-mode`/`--port-file`). `--identity` → path passkey 자동 생성. `list --kind ssh\|tasty-attach` 필터 |
...
| IPC | `remote.profile.{list,get,add,detect,remove}`(kind-generic — `kind="tasty-attach"` + `fields` 로 tasty-attach CRUD 도 양면 노출) · `remote.passkey.{list,get,add,remove}` (값 마스킹). 구 `tool.ssh.*`/`ssh.profile.*` 는 alias 한시 호환 |
