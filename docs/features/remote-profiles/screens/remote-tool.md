...
- **로컬 SSH config 섹션** (원격 접속 프로필 탭 전용, 프로필 목록 **아래**): 구분선 + `로컬 SSH config` 헤더 + config 경로(mono 캡션) + 재로드 아이콘. 행 = alias 이름 / `HostName[:Port]` 캡션 / 우측 가져오기 아이콘 하나.
  ...
  - 빈 상태 문구는 원인 3갈래를 가른다 — 파일이 없으면 `ssh config 가 없습니다.`, 있는데 열리지 않으면(권한 등) `ssh config 는 있지만 읽을 수 없습니다(권한을 확인하세요).`, 열리는데 alias 가 0건이면 `ssh config 에 가져올 호스트가 없습니다.` 셋은 사용자가 할 일이 각각 다르다(만들기 / 권한 고치기 / Host 적기). 어느 경우에도 섹션 자체를 숨기지 않는다(기능의 존재를 알린다).
