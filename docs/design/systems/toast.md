...
**Toast는 사용자 행동(키보드 단축키 / 마우스)에서만 발사된다.** CLI/IPC를 통한 에이전트 동작은 사용자의 시각 상태에 영향을 주면 안 되므로 토스트를 띄우지 않는다. `docs/design/systems/popup.md`의 "사용자 행동과 에이전트 행동의 분리" 원칙을 그대로 따른다.
...
- 페이드: 등장 80ms / 소멸 160ms (애니메이션은 알파만, 위치 이동 없음 — `docs/design/systems/theme.md`의 "터미널 콘텐츠 애니메이션은 0ms" 규칙은 비-터미널 알림 UI에는 적용되지 않음)
