...
3. **드래그 이동** — 팝업이 선언한 **드래그 핸들**(`drag_handle`) 영역을 드래그로 팝업 전체 이동. 타이틀바 팝업은 타이틀 영역이 핸들(기본). 타이틀바 없는 팝업도 `DragHandle::Region` 으로 헤더 띠를 직접 선언해 이동 가능. **위젯 우선 중재**(`is_using_pointer`)로 핸들 띠가 위젯(검색 입력·버튼)과 겹쳐도 위젯 클릭이 항상 우선되어 충돌하지 않는다(8번 입력 우선순위 참조).
