...
tasty 의 영속 데이터는 텍스트 파일과 SQLite에 나눠 저장된다. 사용자가 직접 편집·버전관리할 대상은 텍스트(TOML / 쉘 스크립트)로, 앱이 자동 누적·갱신하는 데이터는 SQLite 로 담는다.
...
- `SCHEMA_VERSION` 상수 하나. `ensure_schema()` 가 `PRAGMA user_version` 을 보고 분기:
  - `0`(새 DB) → `SCHEMA_SQL` 1회 적용 + `user_version` 을 `SCHEMA_VERSION`으로 설정.
...
기존 버전의 DB에도 `CREATE TABLE IF NOT EXISTS`로 테이블을 추가할 수 있다. `tutorial_progress`가 이 방식을 사용한다([ADR-0009](../../adr/0009-state-storage-and-retention.md)). 버전값만으로는 추가 여부를 알 수 없으므로 `an_existing_database_still_gets_the_tutorial_table`이 기존 DB에도 테이블이 생기는지 확인한다.

컬럼 변경이나 데이터 이동을 이 경로에 넣지는 않는다. 그런 마이그레이션이 필요하면 SCHEMA_VERSION 변경과 새 DB 시작 정책을 함께 검토한다.
...
최근 파일 캐시는 Db가 소유하고 모든 창이 공유 핸들로 조회한다. 종류별 최신순·중복 제거·10개 상한을 유지한다. add는 같은 잠금 안에서 메모리 갱신과 DB 저장을 순서대로 수행해 같은 초의 순서와 저장 실패 때의 메모리 상태도 창마다 같게 한다. 조회는 최대 10개 경로 스냅샷을 반환하며 매번 DB를 다시 읽지 않는다. DB 없는 테스트 캐시는 독립적이다. 다른 프로세스의 변경 감지는 보장하지 않으며 그런 요구가 생기면 재검토한다.

### 튜토리얼 진행

튜토리얼 진행은 state.db의 tutorial_progress에 주제·콘텐츠 개정 번호·완료 여부·재개 단계·행 버전을 저장한다. 스키마 검증을 통과한 DB에 중복 없이 테이블을 추가하고 버전 불일치 보호를 유지한다. 완료는 되돌리지 않고 재개 위치는 낙관적 버전 검사로 오래된 View의 덮어쓰기를 막는다. 런타임 객체 ID는 저장하지 않아 실습 재개 시 준비부터 시작한다. 설정·레이아웃과 진행 기록을 섞지 않는다. 외부 콘텐츠 기여·정식 migration·객체 복원 요구가 생기면 소유권과 revision을 검토한다.
...
부팅·추가 저장·주기 타이머가 같은 정리 함수를 쓰며, 정리는 한 시간 간격으로 제한한다.
...
`state.db` 는 WAL 덕분에 단일 프로세스 종료 시점에 일관성이 보장된다. 별도 주기 백업은 만들지 않는다. 수동 백업은 앱이 종료된 일관된 상태에서 `state.db`, `state.db-wal`, `state.db-shm` 세 파일을 함께 복사한다.
...
같은 `TASTY_HOME`을 공유하는 여러 인스턴스는 지원하지 않는다. 그래도 저장 직전에 파일을 다시 읽어 여전히 파싱할 수 없는지 확인한다. 부팅 이후 다른 인스턴스가 정상 파일을 썼을 수 있기 때문이다.

재확인과 rename 사이에는 잠금이 없어 그사이 정상 파일이 쓰이면 백업으로 옮겨질 수 있다. 지원하지 않는 공유 구성에서 남아 있는 경합이며 파일은 삭제되지 않고 백업 한 칸(최대 9개)을 사용한다.
