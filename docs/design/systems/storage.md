...
| 경로 | 포맷 | 내용 | 관리 주체 | 코드 |
|------|------|------|-----------|------|
| `state.db` (+ `-wal`/`-shm`) | SQLite | 최근 markdown 파일, 클립보드 히스토리 스키마 자리 | 앱 | `src/db.rs` |
| `memory.db` (+ `-wal`/`-shm`) | SQLite | 에이전트 메모리 (별도 스키마·연결) | 앱 | `crates/tasty-memory/` |
| `config.toml` | TOML | 사용자 설정(셸·외관·단축키·언어·클립보드 옵션 등) | 사용자 | `crates/tasty-settings/` |
...
`config.toml` / `ssh-profiles` / `file-handlers.toml` / `themes/*.toml` / `bashrc.user` 는 **사용자 편집·버전관리 대상**이다. 주석·diff 추적에 텍스트가 적합하고 앱이 자동으로 덮어쓰지 않으므로 SQLite 로 옮길 이점이 없다. 반대로 최근 파일·클립보드 히스토리처럼 앱이 자동 누적하는 데이터는 SQLite 가 맞다.
