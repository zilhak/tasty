...
`+`는 **구분자 역할**과 **키 이름 역할**을 동시에 가진다. 파서는 왼쪽부터 `ctrl+`, `shift+`, `alt+` 프리픽스를 순서대로 벗겨내고, 남은 문자열 전체를 키 토큰으로 본다. 따라서 `"ctrl++"`는 "Ctrl + `+` 키"로 올바르게 해석된다.

| 바인딩 | 해석 | 매칭되는 입력 |
|--------|------|--------------|
| `ctrl++` | Ctrl + `+` 키 | Ctrl 누른 상태에서 `+` 문자 입력 |
| `ctrl+plus` | 동일 (심볼 이름 별칭) | 동일 |
| `ctrl+-` | Ctrl + `-` 키 | Ctrl + `-` |
| `ctrl+minus` | 동일 | 동일 |
| `ctrl+=` | Ctrl + `=` 키 | Ctrl + `=` |
| `ctrl+equals` | 동일 | 동일 |
| `ctrl+shift+=` | Ctrl+Shift+`=` | Shift까지 함께 요구 |
| `ctrl+` | 무효 (키 부분 없음) | 매칭 안 됨 |
| `ctrl` | 무효 (모디파이어만) | 매칭 안 됨 |

심볼 키는 문자(`+`)와 이름(`plus`) 중 어느 쪽으로 저장되어도 양방향 매칭된다. 또한 **모디파이어 키 단독 입력(Ctrl/Shift/Alt/Super/Meta/Hyper/Fn 등)은 어떤 바인딩과도 매칭되지 않는다** — 파서가 아니라 매처가 구조적으로 차단한다.
