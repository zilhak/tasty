...
- **처리(headless)**: window 가 없어 위 drain 이 통째로 `gui` feature 게이트이므로, headless 는 `crate::intent::headless::drain_pending_intents` 가 engine 하나짜리로 같은 계약을 수행한다 — IPC 요청 처리 직후(**응답 송신 전**) · plugin 호출 결과 회신 전 · 메인 루프 블로킹 대기 직전. cascade 는 gui `handle_core_event` 중 engine 상태로 완결되는 것만(attention · notification 적재 · terminal mark · cwd 표시 · settings 적용/저장) 수행하고, view redraw/toast/theme 재설치·알림음은 소비처가 없어 제외한다. 근거·범위: [ADR-0111](../../adr/0111-headless-drains-the-intent-queue.md).
