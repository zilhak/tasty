...
└── HashMap<WindowId, Box<dyn Window>>
    ├── MainWindow 1   (Modality: Modeless, 계열: TerminalHostWindow)
    ├── MainWindow 2
    ├── ...
    └── [선택적] SettingsWindow 또는 QuitWindow  (Modality: Modal, 엔진 전역 최대 1개)
```
...
├── ModalWindow         (supertrait)
│   ├── SettingsWindow
│   └── QuitWindow
└── TerminalHostWindow  (supertrait)
    └── MainWindow
```

- **Window**: 공통 인터페이스 (`base()`, `handle_event()`, `render()`, `modality()` 등).
  `sealed::Sealed`를 supertrait으로 가져 크레이트 외부에서 직접 구현 불가.
- **ModalWindow**: 모달 계열 default 동작 (`reveal_after_first_render`, `on_escape`).
- **TerminalHostWindow**: 일반 윈도우 계열 default (`has_sidebar`).
- **WindowBase**: 모든 구현체가 `pub base: WindowBase`로 composition하는 공통 필드
...
- 자체 wgpu 서피스 + egui 컨텍스트 보유 (`WindowBase.gpu`)
- wgpu adapter/device는 엔진에서 공유
- 윈도우 간 통신은 엔진을 통해 수행 (직접 통신 없음)

### MainWindow

`TerminalHostWindow` 계열의 유일한 현재 구현체. 여러 워크스페이스/패인/탭을 가지며
터미널 계열 Surface를 호스팅한다. 미래에는 `StandaloneSurfaceWindow`,
`StandaloneWorkspaceWindow` 등이 같은 계열에 추가될 수 있다.

### SettingsWindow / QuitWindow

`ModalWindow` 계열의 현재 구현체. 각각 설정 UI / 종료 확인 다이얼로그를 담는다.
...
- 이벤트 디스패처는 `WindowCtx { modal_active: true }`를 다른 Modeless Window에 전달
