...
누르고 새 탭이 선택되는지 `tab.list` 로 본다)([ADR-0568](../../adr/0568-a-user-gesture-navigation-in-a-plugin-webview-makes-its-file-dispatch-a-user-action.md)). `recent.query`는 state.db에 귀속된 공유 캐시를 조회하므로 어느 창에서 호출해도
