...
**모든 CLI/IPC 명령은 focus에 의존하지 않고, ID로 대상을 직접 지정할 수 있어야 한다.**

AI 에이전트는 현재 focus 상태를 모르거나 제어할 수 없는 상황이 빈번하다. 따라서:

- 모든 명령에 `--surface`, `--pane`, `--tab` 등 명시적 대상 지정 파라미터가 있어야 한다
- focus fallback은 편의 기능일 뿐, 유일한 동작 경로가 되어서는 안 된다
- 리소스를 생성하는 명령은 응답에 생성된 리소스의 ID를 반드시 포함해야 한다
  - 예: `new tab` → 응답에 `tab_id`, `surface_id`, `pane_id` 포함
  - 예: `new markdown` → 응답에 `tab_id`, `pane_id` 포함
