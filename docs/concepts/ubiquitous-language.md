...
- **점유(occupation)** — 주체(원격 사용자 | AI Agent)가 surface/workspace 에 대해 선언하는 지속·가시 관계. **약한(soft: advisory 마커, write 허용) / 강한(hard: 배타 + 다른 주체 readonly; 원격 attach 가 사례)** 2계층(ADR-0021). 계층과 무관하게 대상→점유자는 1:1(배타), 주체→대상은 1:N. self-release 또는 로컬 사용자 force-detach 로만 해제.

### 원격 연결 (→ [features/remote-profiles](../features/remote-profiles/index.md))

- **원격 접속 프로필(Remote profile)** — 타입(`kind`, 열린 string) 태그가 붙은 범용 연결 디스크립터. 비밀을 담지 않고 Passkey 를 이름으로 참조만 한다. 2-레이어(ADR-0020): **`ssh`** = 순수 연결 정보, **`tasty-attach`** = attach 스펙(ssh 를 `ssh_ref` 로 참조하거나 인라인 + remote_tasty/port_mode/port_file). attach 는 tasty-attach kind 를 읽는 **소비자** — "주소 저장(ssh) ≠ attach 스펙(tasty-attach)".
- **Passkey** — 별도 named 자격증명 저장소. `kind = path`(파일 참조) | `inline`(0600 파일로 materialize). at-rest 는 항상 파일 경로(toml 에 비밀 0). 값은 로컬 GUI Reveal 로만 열람, IPC/agent 엔 영구 마스킹([ADR-0011](../adr/0011-secrets-and-local-trust.md)).
...
- **같은 동작의 이름도 한 낱말이다 — 일본어 "새 윈도우" 는 `新しいウィンドウ` 다.**
  한 파일 안에서 `[menu.macos]` 만 `新規ウィンドウ` 였고 `[tray]`·`[jump_list]` 는
  `新しいウィンドウ` 였다. `新規` 가 macOS 일본어 메뉴의 관례어라 그 절만 다른 것이
  의도일 수 있었는데, **그 관례를 저장소 안에서 확인할 자리가 없다** — tasty 는
  `with_default_menu(false)` 로 NSMenu 를 직접 소유해서 OS 가 제목을 주는 항목이 하나도
  없고, 그래서 Apple 의 표기는 저장소에도 런타임 화면에도 안 들어온다. 잴 수 없는 것을
  근거로 삼는 대신 저장소 안의 선례를 썼다: 한국어에서 같은 갈림을 설정 쪽 표기
  (`새 윈도우`)로 모았고, 일본어의 설정 쪽 표기가 `新しいウィンドウ:`
  (`settings.keybindings.new_window_label`) 다. 그 동작의 네 자리 중 셋이 이미 그 낱말이었다.
  ☆ 이 판정의 범위는 **"새 윈도우" 라는 동작 이름 하나**다. 일본어의 "새 X" 계열 전체가
  한 낱말이라는 뜻이 아니다 — `[pane_context_menu]` 는 `新規ターミナル` 계열이고
  `[explorer.tab]` 은 `新しいタブ` 다. 그 갈림은 여기서 판정하지 않았다.
