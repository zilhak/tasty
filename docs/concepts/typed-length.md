...
셋째 가드의 술어는 "`src` 전체에 없다" 가 아니라 **"전환 전선 밖에 없다"** 다. 본체 전환이 진행 중이라 아직 남은 영역이 있고, 그 영역은 면제 목록이 아니라 **경로 한 줄과 건수**로 이름 지어져 있다 — 건수가 상한이라 전선은 줄어들 수만 있다. 그 가드가 설계상 못 잡는 형태(테스트 코드, 배율성 이름, 0~1 값, `static`·`let`)는 그 파일의 모듈 주석이 열거하고, 각 형태를 겨냥한 테스트가 사각의 모양을 못박는다.
