...
- **외부 Codex 실행** — `launch`/`spawn`/`respawn` 의 공통 명령 빌더와 `reboot` 의 resume 빌더는 셸의 `codex` alias/function 을 우회한다. Linux/macOS 의 sh/bash/zsh 에서는 `command codex` 로 현재 `PATH` 의 실행 파일을 찾는다. Windows resume 은 `cmd.exe /d /c codex` 로 PowerShell alias/function 과 cmd AutoRun 을 건너뛰고 외부 실행 파일 또는 `.cmd` 런처를 찾는다. `launch`/`spawn`/`respawn` 의 환경변수 대입과 프롬프트 파일 읽기는 기존 POSIX 셸 구문이며, Windows cmd/PowerShell 용 전달 구현은 아직 없다. 실행어만 바뀌며 아래 정책 우선순위·기본값·명시 옵션은 그대로다. alias 에 넣어 둔 옵션을 적용하려면 tasty 의 정책 플래그나 전역 설정에 명시한다.
...
- Given sh/bash/zsh 에 `codex` alias 또는 function 이 우회 옵션을 추가한다 When `launch`/`spawn`/`respawn`/`reboot` 명령을 생성해 실행한다 Then 외부 Codex 에는 tasty 가 결정한 정책 인자만 전달되고 환경변수와 프롬프트 전달 방식이 유지된다.
