...
- **body** — well(border+radius+bg-app 스크롤 컨테이너) 안에 타입별 콘텐츠. 오늘은 Text(mono pre) 뿐 — Image/HTML/기타 포맷/파일 목록은 자매 작업이 추가.
...
popup`. header/type-bar(배지)/body(well)/footer 4행 + empty/read-failed/already-open 3
CenterState 를 토큰으로 전사(본체/plugin crate 비의존, 픽셀 동일성 비목표). 세그먼트(2개 이상
타입) 상태는 오늘의 실 데이터가 Text 뿐이라 아직 specimen 에 없다 — [[48/49/50/52]]가 타입을
늘리면 추가한다. 3자 매핑: [design-gallery-mapping.md](../../../design/systems/design-gallery-mapping.md#clipboard-viewer-overlays).
