...
- **코드**: `crates/tasty-plugin-markdown/`(`src/render.rs` = `pulldown-cmark` → sanitize(`ammonia`) → CSS 주입 HTML 문서 생성, `src/main.rs` surface 라이프사이클/네비게이션/팝업, `src/watch.rs` idle 자동 리로드) — [ADR-0065](../../adr/0065-markdown-webview-render-channel.md)(EguiMesh→Webview 전환 결정) · [ADR-0028](../../adr/0028-plugin-egui-mesh-render-channel.md)(egui-mesh 채널, 팝업만 계속 사용)
...
- **idle auto-reload(입력 없이도 갱신)** — webview-kind surface 는 `surface.set_context`/`paint`(=egui-mesh 전용 forward)를 **아예 받지 않는다** — 그래서 idle watch(`src/watch.rs`)가 사실상 유일한 자동 갱신 경로다. `on_start` 에서 별도 스레드를 띄워 `RELOAD_CHECK_INTERVAL_SECS`(1초) 주기로 열려 있는 모든 markdown surface 의 파일 mtime 을 stat 하고, 변경을 감지하면 host 를 왕복해 **plugin 자신의** `markdown.reload` IPC 를 호출한다(`host.call`) — CLI/사용자가 같은 메서드를 부르는 것과 동일 요청이 단일 dispatch 루프에 직렬 도착해 위 레이스-없음 보장을 그대로 공유한다.
