...
- **idle auto-reload(입력 없이도 갱신)** — `MdDoc::poll_reload` 자체는 `paint`(=host 가 `set_context` 를 forward할 때) 안에서만 돌고, host 는 기본적으로 (크기/포커스 변경·사용자 입력·테마 변경·첫 bootstrap) 중 하나가 있을 때만 forward 한다 — 즉 surface 에 아무 입력도 없으면 파일이 바뀌어도 화면이 갱신되지 않는다. 이를 메우기 위해 plugin 은 `on_start` 에서 별도 스레드(`src/watch.rs`)를 띄워 `RELOAD_CHECK_INTERVAL_SECS`(1초) 주기로 열려 있는 모든 markdown surface 의 파일 mtime 을 stat 한다 — paint 와 무관하게 독립적으로 돈다. 변경을 감지하면 **read 는 하지 않고** `PluginEvent::SurfaceInvalidated{surface_id}` 만 host 에 emit 한다(plugin state 이중 소유 회피). host(`egui_mesh.rs` 의 `MeshForwardState::invalidated` 플래그)는 이 알림을 받으면 그 surface 를 forward 게이트에 무입력으로 1회 통과시키고, 그 결과 도달한 `paint` 안에서 기존 `poll_reload` 가 실제 재-read 를 수행한다 — 스크롤/포커스는 `repaint_after_reload` 와 동일하게 보존된다. 결과적으로 **surface 에 아무 입력이 없어도 외부 파일 수정이 1~수 초 내 반영**된다. `markdown.navigate` 제자리 이동은 같은 surface_id 로 `create_surface` 가 다시 불리므로 감시 대상 경로도 자연스럽게 갱신된다.
...
- **인라인 이미지** — `egui_commonmark` 의 `load-images` feature(`egui_extras` 의 file+image 로더)로 로컬 이미지를 렌더한다. 비트맵은 plugin 자기 `egui::Context` 의 텍스처로 올라가 나머지 chrome 과 함께 tessellate·mesh 전송된다 — 별도 Canvas/SharedBuffer 경로 없음([ADR-0030](../../adr/0030-image-egui-mesh-bitmap-texture.md)의 "비트맵 = egui 텍스처" 결정 그대로, markdown(B1) 은 이미 mesh-only 였으므로 신규 host/프로토콜 코드 0). **상대 경로**(`![](img.png)`, `![](./sub/img.png)`)는 현재 마크다운 파일의 base_dir 기준으로 해석한다 — `src/render.rs::image_uri_prefix` 가 `CommonMarkViewer::default_implicit_uri_scheme` 에 `file://<base_dir>/` 를 주입해, scheme 없는 모든 image dest 를 라이브러리가 그 경로 아래로 앵커한다. 지원 포맷은 PNG/JPEG(`image` crate, 마크다운에 실제로 쓰이는 두 포맷만 — 뷰어 스코프). **알려진 제약**: (1) *절대* 경로 이미지(`![](/abs/img.png)`)는 라이브러리가 scheme 유무만으로 판정해 상대경로와 동일하게 이 base_dir prefix 를 앞에 붙이므로 의도한 절대경로가 아니라 base_dir 아래 잘못된 중첩 경로로 해석된다(미지원, 크래시 없이 로드-실패 로 우아하게 처리됨). (2) 원격 `http(s)://` 이미지는 `fetch` feature(`egui_extras/http`) 를 켜지 않아 미지원(scheme 이 있어 base_dir prefix 는 안 붙지만 로더가 없어 로드-실패). 파일이 없거나 깨진 이미지도 크래시 없이 egui 로더의 인라인 에러 표시로 처리된다.
