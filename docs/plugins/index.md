...
| [agent-stream](agent-stream/index.md) — `com.tasty.agent-stream` | 에이전트 세션 transcript tail → 구조화 스트림 이벤트 수집 + SSE 엔드포인트 방출 · 요청별 턴 correlation · 인증 웹훅 배선(headless), `bundle=false`로 배포 제외 | cli · ipc · sse |
