...
> **예제로서**: **도구 메뉴 항목 + popup** 예제 — main/render 모듈(조회 로직은 `tasty-git-core`)로 분리한 구조의 레퍼런스 → [plugin-development](../../dev-guide/plugin-development.md#도구-메뉴-항목--popup).
...
- **렌더링** — 팝업 콘텐츠를 **egui-mesh** 로 그린다(ADR-0028): plugin 이 자기 egui Context 에서
...
- **worktree 전환** — rail 행 선택 시 그 worktree 의 workdir 로 다시 열어 status/log/diff 를
  다시 조회한다. **실제 checkout/working dir/HEAD 변경은 없다**(플러그인 popup 내부 상태만 변경).
- **repo 핸들** — 로컬 모드는 활성 worktree 의 `Repository` 핸들 하나를 들고 재사용해 조작마다
  ...
  [ADR-0030](../../adr/0030-bundled-plugin-data.md).
