...
- **내부 구현은 적지 않는다.** docs 는 내부 *동작*(WHAT — 규칙·상태 전이·제약)까지만 적는다. 파일·함수 콜사이트(`mouse.rs → parse_link …`), feature gate, GUI 의존 같은 *구현*(HOW)과 빌드/로드맵 상태(`Phase …`, `구현 예정`, `이관 상태`)는 **코드가 진실이고 즉시 rot 하므로 docs 에 두지 않는다.**
