...
## 검증 인스턴스를 어느 디스플레이에 띄우나 — 기본값이 답이 아니다

이 문서군의 절차는 대부분 GUI 인스턴스를 하나 띄운다. 아무것도 지정하지 않으면 그것은
**셸이 물려받은 디스플레이**, 즉 사람이 실제로 보고 있는 화면 위에 뜬다. 여러 검증이
동시에 도는 환경에서는 그 선택이 측정 자체를 바꾼다.

실측(2026-09-06). 공유 인스턴스에 `surface.list` 를 50 ms 마다 던져 **한 왕복의 최대
지연**을 잰 결과다.
...
- **CPU 를 탓하지 마라.** 부하를 CPU 로만 준 팔이 제일 조용하다.
- **"디스플레이를 공유해서" 도 아니다.** 한 Xvfb 에 전부 몰아도 정체가 없다. 정체는
  실제 GPU·컴포지터가 붙은 X 서버에서만 났다.
- **그러므로 기본값(사람의 화면)이 가장 나쁜 선택이다.** IPC 왕복이 초 단위로 밀리면
  타이밍을 보는 절차는 제품이 아니라 **디스플레이 경합을 재게 된다** — 초록도 빨강도
  근거를 잃는다.

**지시**: 화면 그림 자체가 판정 대상이 아닌 절차(IPC 왕복·상태 조회·타이밍)는 전용
Xvfb 에서 돌린다. 캡처가 목적인 절차의 Xvfb 함정(Xauthority 상속·WM 부재·재렌더 유발)은
[screenshot-methods](screenshot-methods.md) 에 이미 있다. 사람의 화면을 쓰는 것은
**그 화면이 판정 대상일 때만**이다.

**안 갈린 것**: 위 팔들은 "실제 X 서버" 와 "하드웨어 GPU 경로" 를 못 가른다 — Xvfb 는
서버뿐 아니라 렌더러도 소프트웨어다. 가르려면 두 번째 하드웨어 X 가 필요하고, 안 쟀다.
