...
- 라이선스 고지(MIT 본문 · 번들 폰트의 OFL 본문 · 제3자 고지 문서)는 산출물 안에 함께 들어 있습니다. `.deb` 은 `/usr/share/doc/tasty/`, `.rpm` 과 `.AppImage` 는 `usr/share/licenses/tasty/`, `.tar.gz` 는 압축을 푼 디렉토리 최상단입니다. 릴리스 페이지에서도 따로 내려받을 수 있도록 해 두었지만, 그렇게 올라간 릴리스는 아직 없습니다.
