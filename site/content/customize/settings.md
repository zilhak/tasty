...
- 숫자 칸은 **입력**해서 바꿉니다. 드래그해서 올리거나 내리는 칸은 없습니다. 치는 동안에는 값이 끌려가지 않고, 칸을 떠나거나 `Enter` 를 누를 때 허용 범위 안으로 맞춰집니다 — 범위를 벗어난 값을 쳐 두면 칸이 빨개지고 그 아래에 허용 범위와 실제로 저장될 값이 한 줄로 나옵니다.
