...
**비라틴 키보드에서도 위치로 맞춘다.** 러시아어 · 그리스어처럼 라틴 문자가 아닌 배열을 써도 단축키는 키캡의 라틴 위치로 인식된다 — 러시아어 배열에서 `H` 자리(러시아어로는 `Р` 가 나오는 키)를 수식키와 함께 누르면 `Ctrl+Shift+H` 단축키가 그대로 동작한다. 배열을 바꿔도 단축키를 다시 잡을 필요가 없다. 마크다운 · 웹 미리보기 화면 위에서도 같다.
