...
검색창 · 파일 선택 · 명령 팔레트처럼 입력을 받는 창에 **포커스가 가 있는 동안에는 단축키가
하나도 동작하지 않습니다.** 일부러 그렇습니다 — 검색창에 글자를 치는데 `Alt+W` 가 서피스를
닫아버리면 곤란하니까요. 창이 떠 있기만 하고 포커스가 없으면 단축키는 평소대로 동작합니다.

빠져나오는 방법은 둘입니다.

- **`Esc`** — 포커스된 창을 놓습니다. 바깥을 클릭했을 때 닫히는 종류의 창이면 함께 닫히고,
  그렇지 않은 창은 열린 채로 포커스만 풀립니다. 열려 있어도 포커스가 없는 다른 창은
  그대로 남습니다.
- **창 바깥을 클릭** — `Esc` 와 같은 결과입니다.

설정 창이나 알림 패널이 떠 있을 때는 `Esc` 가 그쪽을 먼저 닫습니다.
