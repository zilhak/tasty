...
You can see the current state in the same tab (only shown on macOS). Full Disk Access is not part of that button because no app can request it. Use **Open Full Disk Access settings** below it to open System Settings and add Tasty yourself. Folder permissions cannot be checked automatically, so they read "Cannot check automatically".
...
- **I granted everything, but folder prompts still appear** — the startup notice only looks at permissions it can check. Folder permissions (Downloads · Documents · Desktop · volumes) are left out because accessing a folder to check can open a permission prompt. Granting Full Disk Access covers those folders too.
