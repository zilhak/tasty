...
`disable`은 플러그인이 끝나기를 기다리지 않고 바로 돌아옵니다. 응답하지 않는 플러그인은 뒤에서 최대 2초 뒤에 강제로 종료됩니다. 그 사이에 `enable`하면 이전 프로세스가 완전히 끝난 뒤에 다시 시작되므로, `tasty plugin list`의 실행 여부가 잠깐 꺼진 채로 보일 수 있습니다.
