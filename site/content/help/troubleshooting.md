...
| 파일 | 내용 |
|---|---|
| `config.toml` | 설정. 설정 창에서 저장한 값이 여기 들어갑니다 |
...
- **Tasty 터미널 안에서 `tasty` 를 쳤는데 새 창이 안 뜹니다** — Tasty 안에서 인자 없이 실행하면 새 창 대신 도움말을 보여줍니다. 새 창은 `tasty new window`, GUI 를 강제로 띄우려면 `tasty --launch`.

설치 절차 자체는 [설치](../getting-started/install.md) 에 있습니다.

## macOS 권한 프롬프트

**증상** — 첫 실행 직후 권한 프롬프트가 연달아 뜹니다. 순서는 다운로드 · 문서 · 데스크탑 폴더 → (연결돼 있으면) 외장 · 네트워크 볼륨 → 화면 기록입니다. 프롬프트가 떠 있는 동안에도 창은 정상 동작합니다.
...
- **창이 클릭 · 키 입력 · CLI 에 전혀 반응하지 않습니다** — 5초 넘게 멈추면 `~/.tasty/crash-reports/hang-*.log` 가 자동으로 남습니다. 파일의 `Render phase` 가 `acquire` / `submit` / `present` 면 GPU 드라이버 쪽 문제입니다 — 드라이버를 업데이트합니다. Tasty 는 스스로 복구하지 않으므로 강제 종료하고 다시 띄웁니다.
- **갑자기 종료됐습니다** — `~/.tasty/crash-reports/crash-*.log` 를 봅니다. 문제를 신고할 때 이 파일을 함께 붙입니다.

## 설정이나 창 구성이 초기화된 것처럼 보일 때
...
- **저장해둔 창 구성이 복원되지 않았습니다** — `~/.tasty/layouts/` 의 슬롯 파일이 손상됐을 때도 같습니다. 원본은 `01.json.bak` 처럼 옆에 남습니다.
...
- **`No running tasty instance found (port file not found at …)`** — Tasty 창이 떠 있지 않습니다. 메시지에 적힌 경로가 `~/.tasty/tasty.port` 가 아니면 다른 홈 디렉토리(`TASTY_HOME`)를 보고 있는 것입니다. 이 메시지는 설정 언어(`general.language`, 기본값 영어)를 따르므로 한국어로 설정했다면 "실행 중인 tasty 인스턴스를 찾을 수 없습니다" 로 나옵니다.
...
- **OS 알림이 안 뜹니다** — Tasty 창이 활성일 때는 OS 알림을 보내지 않고 앱 안의 패널 · 테두리 · 배지로만 알립니다. 창이 비활성일 때만 OS 알림이 가며, 초당 1회로 제한됩니다. **설정** > **알림** <!-- en: Notifications --> 의 **알림 활성화** <!-- en: Notifications enabled --> 가 꺼져 있지 않은지 봅니다. 패널은 `Ctrl+Shift+I` (macOS `Cmd+Shift+I`) 로 엽니다.
