...
현재 상태도 같은 탭에서 볼 수 있습니다(macOS 에서만 보입니다). 전체 디스크 접근 권한은 앱이 요청할 수 없어 이 버튼에 포함되지 않습니다. 그 아래의 **전체 디스크 접근 권한 설정 열기** <!-- en: Open Full Disk Access settings --> 로 시스템 설정을 열어 직접 추가하세요. 폴더 권한은 자동으로 확인할 수 없어 "자동 확인 불가" <!-- en: Cannot check automatically --> 으로 표시됩니다.
...
- **권한을 다 줬는데 폴더 접근 프롬프트는 여전히 뜹니다** — 시작 안내는 확인할 수 있는 권한만 봅니다. 다운로드 · 문서 · 데스크탑 · 볼륨 같은 폴더 권한은 확인하려고 폴더에 접근하면 권한 요청 대화상자가 뜰 수 있어 안내 판정에 넣지 않습니다. 전체 디스크 접근 권한을 주면 이 폴더들에도 접근할 수 있습니다.
- **권한을 줬는데도 안내가 다시 뜹니다** — 직접 빌드한 앱의 서명이나 macOS 권한 설정이 바뀌었을 수 있습니다. ad-hoc 서명으로 다시 빌드하면 권한을 다시 허용해야 할 수 있습니다. `./scripts/macos-codesign-identity.sh --create` 로 "Tasty Dev" 인증서를 한 번 만들고 이후 빌드에 같은 인증서를 사용하세요. 인증서를 사용해도 권한 유지가 보장되지는 않으므로 시스템 설정에서 상태를 함께 확인하세요. 인증서로 바꾼 직후에는 목록에서 기존 Tasty 를 빼고 다시 추가하세요.
