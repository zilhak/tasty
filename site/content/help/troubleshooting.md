...
**증상** — 첫 실행 직후 권한 프롬프트가 연달아 뜬다. 순서는 다운로드 · 문서 · 데스크탑 폴더 → (연결돼 있으면) 외장 · 네트워크 볼륨 → 화면 기록이다. 프롬프트가 떠 있는 동안에도 창은 정상 동작한다.
...
- **"Tasty 에 전체 디스크 접근 권한 주기" 안내가 떴다** — 전체 디스크 접근 권한이 없어 보일 때 한 번만 뜬다. 앱이 직접 요청할 수 없는 권한이라 **설정 열기** <!-- en: Open settings --> 로 시스템 설정을 열고 Tasty 를 목록에 직접 추가한다. 이 권한을 주면 파일 접근 프롬프트(다른 앱의 데이터 · 다운로드 · 문서 · 데스크탑 · 볼륨)가 사라진다. 다만 다른 앱 제어(Automation) · 화면 기록은 별개 권한이라 그대로 남는다. 다시 보려면 **설정** > **일반** > **권한** 의 **시작할 때 전체 디스크 접근 권한 안내 표시** <!-- en: Show the Full Disk Access notice at startup --> 를 켠다. 같은 탭의 전체 디스크 접근 권한 상태는 추정값이라 틀릴 수 있고, 이 값으로 막히는 기능은 없다.
...
## 설정이나 창 구성이 초기화된 것처럼 보인다

- **설정이 기본값으로 돌아갔다** — `~/.tasty/config.toml` 이 TOML 로 해석되지 않으면 기본값으로 시작한다. 원래 파일은 지워지지 않는다. 그대로 두었다가, 설정을 저장해 파일을 새로 쓰는 순간 같은 폴더의 `config.toml.bak` 으로 옮겨진다. 그 파일을 열어 고친 뒤 `config.toml` 로 되돌리면 설정이 그대로 살아난다. 어느 줄이 문제인지는 `~/.tasty/debug.log` 에 적힌다.
- **저장해둔 창 구성이 복원되지 않았다** — `~/.tasty/layouts/` 의 슬롯 파일이 손상됐을 때도 같다. 원본은 `01.json.bak` 처럼 옆에 남는다.
- **"저장되지 않는다, `.bak` 파일을 옮기거나 지워라" 는 알림이 떴다** — 원본을 옆으로 옮기려 했지만 `.bak` 부터 `.bak.9` 까지 아홉 자리가 이미 차 있어 더 둘 곳이 없는 경우다. 원본을 지우지 않으려고 저장 자체를 멈춘 상태이며, 그 세션 동안은 계속 저장되지 않는다. 쌓인 `.bak` 파일 중 필요 없는 것을 옮기거나 지운 뒤 다시 켜면 정상으로 돌아온다.
- **"설정 파일을 읽지 못했다" 는 알림이 떴다** — 파일이 있는데 **읽지 못한** 경우다(권한 문제나 디스크 오류). 이때 Tasty 는 그 파일을 건드리지 않고 저장도 하지 않는다 — 그대로 두면 지금 화면의 기본값이 원래 설정을 덮어쓰지 않는다. 권한을 고치거나 파일을 다른 이름으로 옮긴 뒤 다시 켠다.
