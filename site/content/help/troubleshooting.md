...
**증상** — 첫 실행 직후 권한 프롬프트가 연달아 뜰 수 있습니다. Tasty는 다운로드 · 문서 · 데스크탑 폴더, 연결된 외장 · 네트워크 볼륨, 화면 기록 순으로 권한을 확인합니다. 실제 프롬프트는 기존 승인 상태와 macOS 정책에 따라 표시됩니다. 이 확인은 앱의 화면 처리를 멈추지 않도록 별도로 진행합니다.

**원인** — 터미널 안에서 실행한 명령이 파일을 읽으면 macOS 는 그 접근을 Tasty 의 것으로 봅니다(Terminal.app · iTerm2 도 같습니다). 그대로 두면 새 폴더를 처음 건드릴 때마다 작업 도중에 프롬프트가 떠서 에이전트가 멈추므로, Tasty 는 시작 직후에 미리 물어봅니다. 이미 선택한 권한은 보통 다시 묻지 않습니다. 시작 시 확인을 끄는 설정은 없으며, 이후 작업 중에도 macOS가 추가 권한을 요청할 수 있습니다.
...
- **"허용되지 않은 권한이 있습니다" 안내가 떴습니다** — 전체 디스크 접근 권한이나 화면 기록 권한이 없어 보이는 동안 시작할 때마다 뜹니다. 어떤 권한이 어떤 상태인지는 **설정** > **일반** > **권한** 탭에서 봅니다. **권한 설정 열기** <!-- en: Open permission settings --> 를 누르면 Tasty 의 권한 화면이 열립니다. 전체 디스크 접근 권한은 앱이 직접 요청할 수 없어, 그 화면의 **전체 디스크 접근 권한 설정 열기** <!-- en: Open Full Disk Access settings --> 로 시스템 설정을 열고 Tasty 를 목록에 직접 추가합니다. 이 권한을 주면 파일 접근 프롬프트(다른 앱의 데이터 · 다운로드 · 문서 · 데스크탑 · 볼륨)가 사라지고, 다음 시작부터 안내도 뜨지 않습니다. 다만 다른 앱 제어(Automation) · 화면 기록은 별개 권한이라 그대로 남습니다. 안내를 끄는 설정은 없으며 매번 시작할 때 현재 상태를 다시 확인합니다.
- **권한을 다 줬는데 폴더 접근 프롬프트는 여전히 뜹니다** — 시작 안내는 확인할 수 있는 권한만 봅니다. 다운로드 · 문서 · 데스크탑 · 볼륨 같은 폴더 권한은 macOS 가 상태를 물어볼 방법을 제공하지 않아(물어보는 행위 자체가 프롬프트입니다) 판정에 넣지 않습니다. 이 폴더들은 전체 디스크 접근 권한을 주면 함께 덮입니다.
- **권한을 줬는데도 안내가 다시 뜹니다** — 직접 빌드한 앱의 서명이 달라졌거나 macOS의 권한 설정이 바뀌었을 수 있습니다. ad-hoc 서명으로 재빌드하면 기존 승인을 다시 받아야 할 수 있습니다. `./scripts/macos-codesign-identity.sh --create` 로 "Tasty Dev" 인증서를 한 번 만들고 같은 인증서로 서명해 설치할 수 있습니다. 권한 유지까지 보장하는 것은 아니므로 시스템 설정에서도 승인 상태를 확인하세요. 인증서로 바꾼 직후 한 번은 목록에서 기존 Tasty 를 빼고 다시 추가해야 합니다.
- **전체 디스크 접근 권한 상태가 "확인 불가" 로 나옵니다** <!-- en: The Full Disk Access status shows "Unknown" --> — 이 상태는 macOS 에 묻는 API 가 없어 추정으로 구합니다. 추정에 쓰는 파일이 이 macOS 버전에 없으면 허용 여부를 판단할 근거가 없어 확인 불가로 표시하고, 이 권한을 이유로는 시작 안내를 띄우지 않습니다(화면 기록이 미승인이면 그 이유로는 뜹니다). 이 값으로 막히는 기능은 없습니다.
