...
- **"Tasty 에 전체 디스크 접근 권한 주기" 안내가 떴습니다** — 전체 디스크 접근 권한이 없어 보이는 동안 시작할 때마다 뜹니다. 앱이 직접 요청할 수 없는 권한이라 **설정 열기** <!-- en: Open settings --> 로 시스템 설정을 열고 Tasty 를 목록에 직접 추가합니다. 이 권한을 주면 파일 접근 프롬프트(다른 앱의 데이터 · 다운로드 · 문서 · 데스크탑 · 볼륨)가 사라지고, 다음 시작부터 안내도 뜨지 않습니다. 다만 다른 앱 제어(Automation) · 화면 기록은 별개 권한이라 그대로 남습니다. 안내를 끄는 설정은 없습니다 — 껐다는 기록을 남기면 나중에 권한이 초기화됐을 때 알려줄 방법이 없어지기 때문입니다.
