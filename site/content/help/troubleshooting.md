...
**해결 — 미리 한 번에 받아둡니다.** **설정** > **일반** <!-- en: General --> > **권한** <!-- en: Permissions --> 에서 **모든 권한 요청하기** <!-- en: Request all permissions --> 를 누르면 다운로드 · 문서 · 데스크탑 폴더 → (연결돼 있으면) 외장 · 네트워크 볼륨 → 화면 기록 순으로 하나씩 물어봅니다. 앞의 것에 답해야 다음이 뜨고, 프롬프트가 떠 있는 동안에도 윈도우는 정상 동작합니다. 이미 허용 · 거부한 항목은 다시 뜨지 않으므로 몇 번을 눌러도 괜찮습니다 — 볼륨을 새로 연결했을 때 다시 누르면 그것만 추가로 묻습니다.
...
현재 상태도 같은 탭에서 봅니다 (macOS 에서만 보입니다). 전체 디스크 접근 권한은 앱이 요청할 수 없어 이 버튼에 포함되지 않습니다 — 그 아래 **전체 디스크 접근 권한 설정 열기** <!-- en: Open Full Disk Access settings --> 로 시스템 설정에서 직접 추가합니다. 폴더 권한은 상태를 물어볼 방법이 없어 "조회 수단 없음" <!-- en: Cannot be observed --> 으로 표시됩니다.
