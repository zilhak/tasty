...
- **"허용되지 않은 권한이 있습니다" 안내가 떴습니다** — 전체 디스크 접근 권한이나 화면 기록 권한이 없어 보이는 동안 시작할 때마다 뜹니다. 어떤 권한이 어떤 상태인지는 **설정** > **일반** > **권한** 탭에서 봅니다. 전체 디스크 접근 권한은 앱이 직접 요청할 수 없어 **설정 열기** <!-- en: Open settings --> 로 시스템 설정을 열고 Tasty 를 목록에 직접 추가합니다. 이 권한을 주면 파일 접근 프롬프트(다른 앱의 데이터 · 다운로드 · 문서 · 데스크탑 · 볼륨)가 사라지고, 다음 시작부터 안내도 뜨지 않습니다. 다만 다른 앱 제어(Automation) · 화면 기록은 별개 권한이라 그대로 남습니다. 안내를 끄는 설정은 없으며 매번 시작할 때 현재 상태를 다시 확인합니다.
