...
본문 크기 제한은 인증보다 먼저 적용됩니다. 청크로 전송한 본문에 잘못된 UTF-8 바이트가 있어도 같은 크기 제한이 적용됩니다. 토큰이 없어도 본문이 상한을 넘으면 `413 payload too large`를 받으며 동작은 실행되지 않습니다. 이 상한은 요청 하나의 크기를 제한하며 동시 연결 수를 제한하지는 않습니다.
