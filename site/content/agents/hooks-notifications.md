...
**서명 검증은 없습니다.** GitHub 등이 쓰는 HMAC 서명 헤더를 보내와도 Tasty 는 보지 않습니다. 발신자를 가리는 수단은 둘뿐입니다 — 추측할 수 없는 URL, 그리고 걸었을 때만 동작하는 고정 토큰(`--auth-*`). **`--auth-*` 를 걸지 않으면 그 URL 에 닿는 누구든 그 동작을 실행시킵니다.**
