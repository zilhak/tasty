...
등록하면 `http://127.0.0.1:28429/<16자리 id>` 형태의 URL 이 출력됩니다. 바깥에서 부를 때는 호스트 부분을 실제 주소로 바꿉니다 — 그 자리에서 바로 `curl` 하기 편하라고 넣은 표기일 뿐, 실제 bind 주소가 아닙니다.
...
| 401 | 인증 실패 (반복하면 429 로 차단됩니다) |
| 404 | 없는 URL |
| 405 | 허용되지 않은 메서드 |
| 410 | 기한·횟수 만료 |
| 413 | 본문이 요청당 크기 상한(기본 1 MiB)을 넘음 |
| 429 | 같은 출처가 10초에 20회 이상 실패(`401`·`404`·`405`)해 60초 차단 중 |
