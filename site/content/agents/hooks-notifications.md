...
터미널에서 일어난 일(프로세스 종료, 특정 출력, 벨)에 명령을 자동 실행하는 **훅**, 사람에게 알리는 **알림**, 바깥에서 HTTP 로 Tasty 를 깨우는 **웹훅**을 다룬다. 셋을 조합하면 "빌드가 끝나면 알림" 같은 자동화를 CLI 만으로 만든다.

## 서피스 훅

특정 서피스(터미널)에서 이벤트가 일어나면 셸 명령을 실행한다.
...
- `--once` 를 붙이면 한 번 실행 후 자동으로 지워진다. 기본은 계속 유지.
- 훅 명령은 백그라운드에서 실행되며 터미널 입력을 막지 않는다.
- `command-completed` 는 셸 통합 없이는 절대 발화하지 않는다. 셸 통합이 없는 서피스에는 안내 배너가 한 번 뜬다.

명령에는 이벤트 정보가 환경변수로 들어온다.
...
`--command` 대신 미리 등록한 **훅 핸들러**를 이름으로 연결할 수 있다. 같은 핸들러를 여러 훅과 웹훅이 공유한다.
...
사용자 핸들러는 **설정** <!-- en: Settings --> › **핸들러** <!-- en: Handlers --> › **훅 핸들러** <!-- en: Hook Handlers --> 탭에서 추가·편집한다. 저장하면 `~/.tasty/hook-handlers.toml` 에 기록되며, 파일을 직접 써도 된다 (`tasty hook-handler reload` 로 반영).
...
`kind = "ipc_sequence"` 로 두면 셸 대신 Tasty 내부 동작 목록(`calls = [{ method = "...", params = {} }]`)을 실행한다.
...
터미널 프로그램이 보내는 알림 시퀀스(OSC 9 / 99 / 777)와 벨도 같은 알림으로 모인다.

### 어디에 나타나나

- **알림 패널** — `Ctrl+Shift+I` (macOS `Cmd+Shift+I`) 로 연다. 최신순 목록에 워크스페이스·제목·본문·경과 시간이 보이고, **이동** <!-- en: Jump --> 으로 그 워크스페이스로 간다. 열면 전부 읽음 처리되며 **모두 읽음** <!-- en: Mark all read --> 버튼도 있다.
- **서피스 테두리** — 알림이 난 서피스에 파란 테두리. 그 서피스로 포커스하면 사라진다.
- **사이드바 배지** — 주의가 필요한 서피스가 있는 워크스페이스 행에 개수 배지.
- **OS 알림** — Tasty 창이 비활성일 때 시스템 알림 (초당 1회 제한).
...
단방향 알림과 달리, 에이전트가 위험한 작업 전에 사용자의 응답을 **기다리는** 게이트다.
...
- 팝업이 뜨고 알림도 함께 간다. 사용자는 팝업 버튼 클릭, 팝업에서 숫자 키 `1`~`9`(선택지 순서), 또는 `tasty approval respond --id <ID> --choice approve` 로 답한다.
- `--severity info` 는 팝업 없이 알림만, `warn`/`danger` 는 팝업 + 알림. `danger` 는 사용자가 직접 답해야 한다.
- 팝업은 Esc 로 닫히지 않는다 (우회 방지). `tasty approval list` / `get` / `history` 로 조회.

## 웹훅 (외부 → Tasty)

CI 나 다른 서비스가 HTTP 요청을 보내 Tasty 안의 동작을 일으키게 한다. Tasty 는 지정 포트 하나를 열고, 웹훅마다 추측할 수 없는 URL 을 발급한다.
...
- 설정 파일은 `~/.tasty/webhooks.toml`. 처음 실행 시 `28429` 가 기본으로 기록된다.
- 포트가 비어 있거나 bind 에 실패하면 Tasty 는 다른 포트로 몰래 바꾸지 않고 경고(토스트)만 낸다. 포트를 고쳐 재시작한다.
- 외부에서 들어오려면 공유기 포워딩·방화벽은 직접 연다. HTTPS 는 앞단 리버스 프록시에 맡긴다.
...
# 인라인 동작 정의 — 바디의 값을 ${body.x} 로 끌어다 쓴다
...
등록하면 `http://127.0.0.1:28429/<16자리 id>` 형태의 URL 이 출력된다. 바깥에서 부를 때는 호스트 부분을 실제 주소로 바꾼다.
...
쓸 수 있는 핸들러 id 는 `tasty hook-handler list` 로 확인한다. `--sequence` 는 Tasty 내부 동작(알림 보내기, 텍스트 전송 등)을 순서대로 적은 JSON 목록이다.

### 응답과 관리

호출 측이 받는 응답은 상태 코드와 고정 문구뿐이다 — 내부 결과는 절대 돌려주지 않는다.
...
웹훅은 셸 명령을 직접 실행할 수 없다 (Tasty 내부 동작만). 셸이 필요하면 웹훅 → 알림 → 서피스 훅처럼 이어 붙인다.
...
`command-completed` 는 모든 종료 코드에 걸리고, `command-completed:0` 처럼 정수 하나를 붙이면 그 코드일 때만 걸린다. 실제 종료 코드는 훅 명령에 `TASTY_HOOK_EXIT_CODE` 환경변수로 들어온다.
