...
- **`Error (…)` 줄 아래에 `data: {…}` 줄이 붙습니다** — Tasty 가 실패의 분류를 함께 보냈습니다(예: 메모리 저장 실패의 `storage_failure`, 거절 사유의 `reason`). `data: ` 뒤는 한 줄 JSON 이라 스크립트는 첫 줄 대신 이 값으로 분기할 수 있습니다. 첫 줄 `Error (코드): 메시지` 는 이 줄이 있든 없든 같습니다. `tasty events follow` 와 `tasty plugin audit-follow` 는 아직 이 줄을 붙이지 않습니다.
