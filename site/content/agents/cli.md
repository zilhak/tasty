...
`list pressure`는 응답이 느릴 때 원인을 고르는 데 씁니다. 답이 열두 덩어리로 나뉘어 있고 **세는 대상이 서로 다릅니다** — `queue_before_gate`는 명령이 큐에서 기다린 시간이라 나중에 거절될 요청까지 세고, `handler_after_gate`는 실제로 처리된 것만 세며, `plugin_round_trip`은 Tasty가 플러그인의 답을 기다린 시간이고(답이 실제로 온 것만 셉니다), `db`는 저장소에 쓴 내용이 디스크에 안착하는 데 걸린 시간입니다. 기다린 시간이 크면 밀린 것이고, 처리 시간이 크면 명령 자체가 무거운 것이며, 셋째가 크면 그 시간은 플러그인 안에 있었던 것이고, 넷째가 크면 디스크가 느린 것입니다. 두 숫자를 빼서 "거절된 수"로 읽지 마세요 — 그렇게 안 됩니다(거절된 수는 아래 `gate_refusals`에 따로 셉니다). 값은 이 인스턴스가 켜진 뒤의 누계이고, 잰 적이 없는 평균은 `null`로 옵니다.
...
열둘째 `gate_refusals`는 요청이 **왜 안 먹었는지**를 셉니다. `judged`는 Tasty가 권한을 따져 본 요청 전부이고(CLI 요청도, 플러그인이 Tasty를 부른 호출도 셉니다), 돌려보낸 요청은 이유마다 한 칸입니다 — `permission_denied`(권한이 없음, 오류 코드 -32001), `cap_blocked`(텔레메트리 한도에 걸림, -32007), `throttled`(호출 빈도 제한에 걸림, -32010). 한 요청은 많아야 한 칸에 세집니다. 처방이 셋 다 다릅니다 — 권한을 청하거나, 한도를 풀거나, 기다리면 됩니다. `permission_denied`는 권한 검사가 돌려보낸 요청만 셉니다. 그 거절은 모두 오류 코드 -32001로 오지만, -32001이 모두 그 거절인 것은 아닙니다. 권한 검사는 권한이 모자란 경우 말고도 플러그인·에이전트가 없는 명령을 부른 경우와 자기에게 허락되지 않는 명령을 부른 경우도 돌려보냅니다. 이 둘은 권한을 청해도 풀리지 않고, 다른 명령을 불러야 합니다. 세션 토큰 없이 쓰는 CLI 요청은 권한 때문에 돌려보내지지 않으므로, 없는 명령을 불러도 이 칸에는 세지지 않습니다. -32001인데 이 칸에 안 드는 것도 두 가지입니다. 세션 토큰 거절(형식이 틀렸거나, 모르거나 만료됐거나 폐기된 토큰 — 낡은 `TASTY_SESSION_TOKEN`이 그렇습니다)은 권한 검사 앞에서 답하므로 `judged`에도 안 듭니다. 그리고 권한 검사를 지난 뒤에 돌아온 -32001(권한 부여 요청 거절 등)은 통과한 요청으로 셉니다. 이 인스턴스가 켜진 뒤의 누계라 다시 켜면 0부터 셉니다. `tasty agent rate-limit-status`의 `throttled_count`는 제한 하나의, 다시 켜도 남는 누계라 이 값과 다릅니다.
