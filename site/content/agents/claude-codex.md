...
줄 중간이 잘리지 않습니다. 비워질 때 사라지는 것은 파일이 256 KiB 를 넘은 뒤에 쓰인 줄이고,
그것도 줄 단위입니다. **tasty 를 다시 시작하면 이전
...
통째로 비웁니다**(아직 안 읽은 줄이 있었다면 함께 사라집니다). 얼마를 버렸는지는 그때
로그에 남으므로, 알림이 안 온 이유를 나중에 되짚을 수 있습니다.
