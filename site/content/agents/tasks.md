...
에이전트가 할 일을 **작업(task)** 으로 만들고 의존 관계로 묶으면 그래프가 됩니다. tasty 의 러너가 순서를 지켜 실행하고, 실패하면 정해둔 대로 처리하고, 결과를 다음 작업에 넘깁니다. 사람은 창에서 진행 상황을 그래프로 봅니다.
...
tasty 를 재시작하면 러너는 자동으로 켜지지 않습니다. 작업 자체는 남아 있으니 다시 `start` 하면 이어서 진행합니다. DAG 창의 헤더에도 러너가 멈춰 있으면 다시 켜는 명령이 그대로 표시됩니다.
