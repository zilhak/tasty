...
- 분할, 새 탭, 탭 닫기·이동, 표면 변환, 닫은 항목 복원은 **원격에서 실행**되고 결과가 mirror 에 반영됩니다. 원격에 없는 종류의 표면을 만들면 토스트로 실패를 알립니다. 에이전트가 요청한 markdown 문서 이동과, 에이전트가 열 위치를 지정하지 않고 요청한 파일 열기가 원격에서 실패한 경우는 토스트 없이 로그에만 남습니다.
