...
const KO_COPY: Copy = Copy {
    ...
    title_tail: "터미널",
    lede: "에이전트한테 일을 맡겨두고, 나는 옆 탭에서 하던 걸 계속해요. \
           포커스도 스크롤도 그대로고요.",
    cta_primary: "다운로드",
    ...
    why_title: "에이전트가 일해도 내 자리는 그대로예요",
    why_body: "에이전트가 탭을 만들든 명령을 보내든, 내가 보던 화면은 움직이지 않아요. \
               잡아둔 선택도, 스크롤 위치도요. 사용자 입력을 흉내 내는 기능은 아예 없어요.",
    why_points: &[
        (
            "01",
            "내 조작과 분리",
            "에이전트가 무슨 일을 하든 포커스와 선택, 스크롤, 닫은 탭 기록에는 손대지 않아요. \
             내가 다른 탭을 보고 있어도 자기 터미널 안에서만 움직여요.",
        ),
        (
            "02",
            "ID 로 지정",
            "에이전트는 조작할 터미널을 ID 로 찍어서 불러요. 지금 뭐가 활성이냐에 따라 \
             엉뚱한 데 입력이 들어가는 일이 없어요.",
        ),
        (
            "03",
            "명령 하나로 전부",
            "분할부터 훅까지 전부 tasty 명령 하나로 해요. \
             에이전트한테 알려줄 건 명령어 목록뿐이에요.",
        ),
        (
            "04",
            "창 없이도",
            "서버나 CI 에서 창 없이 띄워도 같은 명령이 그대로 돌아요.",
        ),
    ],

    features_kicker: "기능",
    features_title: "터미널이 해야 할 일과, 에이전트가 필요로 하는 일",
    features_body: "자주 손이 가는 것만 골랐어요. 나머지는 가이드가 순서대로 다뤄요.",
    cards: &[
        (
            "grid",
            "GPU 렌더링",
            "셀 하나하나를 GPU 가 그려요. 분할을 열 개 넘게 띄워도 안 버벅여요.",
            "using/panes-tabs-splits.html",
        ),
        (
            "stack",
            "워크스페이스와 프리셋",
            "일감마다 워크스페이스를 따로 둬요. 자주 쓰는 배치는 프리셋으로 저장해 두고 꺼내 써요.",
            "using/workspaces.html",
        ),
        (
            "files",
            "터미널만 있는 게 아니에요",
            "탐색기와 마크다운, 이미지, 웹 화면을 터미널 옆에 나란히 띄워요.",
            "using/files.html",
        ),
        (
            "agents",
            "여러 에이전트를 한 번에",
            "Claude 와 Codex 자식을 띄워두면 끝나는 대로 알려줘요.",
            "agents/claude-codex.html",
        ),
        (
            "graph",
            "작업 DAG",
            "할 일을 의존 관계로 묶어 두면 순서대로 돌아요. 어디까지 갔는지는 그래프로 봐요.",
            "agents/tasks.html",
        ),
        (
            "terminal",
            "CLI 로 조작",
            "분할도 명령 전송도 출력 읽기도 tasty 명령 하나예요. 에이전트가 자기 터미널을 직접 다뤄요.",
            "agents/cli.html",
        ),
        (
            "parse",
            "명령 단위 출력",
            "셸 프롬프트 경계를 알아채서, 방금 돌린 명령의 출력만 딱 읽어요.",
            "using/terminal.html",
        ),
        (
            "gauge",
            "훅과 알림",
            "프로세스 종료나 특정 출력, 유휴 시간에 훅을 걸어두고 알림을 받아요.",
            "agents/hooks-notifications.html",
        ),
        (
            "script",
            "Lua 스크립트",
            "단축키나 창 · 탭 이벤트에 스크립트를 걸어두면 손 갈 일이 줄어요.",
            "customize/scripts.html",
        ),
        (
            "link",
            "원격 attach",
            "다른 머신에서 돌고 있는 워크스페이스를 SSH 로 그대로 가져와 봐요.",
            "remote/attach.html",
        ),
        (
            "plug",
            "플러그인",
            "탐색기와 마크다운, 이미지, git 보기가 기본으로 들어 있어요. 권한을 확인하고 켜거나 꺼요.",
            "plugins/index.html",
        ),
        (
            "palette",
            "테마",
            "Mocha 나 Latte 를 고르거나, TOML 로 직접 만들어요.",
            "customize/themes.html",
        ),
    ],

    agents_kicker: "다중 에이전트",
    agents_title: "여러 에이전트를 띄워두고, 끝나는 대로 받아봐요",
    agents_body: "자식을 띄우는 명령은 기다리지 않고 바로 끝나요. 완료 훅은 알아서 걸리고요. \
                  자식이 멈추거나 입력을 기다리면 띄운 쪽 터미널이 먼저 알아요.",
    agents_caption: "사람이 GUI 에서 하는 일은 에이전트도 CLI 로 똑같이 해요.",

    platform_kicker: "플랫폼",
    platform_title: "세 OS 모두 1 급",
    platform_body: "세 OS 에서 기능도 단축키도 CLI 도 똑같아요. \
                    설치 파일은 dmg, msi, deb, rpm, AppImage 로 내요.",
    stats: &[
        ("3", "운영체제"),
        ("7", "번들 플러그인"),
        ("3", "UI 언어"),
    ...
    cta_title: "가이드부터 읽어도 되고, 바로 설치해도 돼요",
    cta_body: "설치부터 에이전트 연동, 원격 attach 까지 가이드가 순서대로 짚어줘요.",
    cta_docs: "가이드 읽기",
    cta_download: "다운로드",
};
