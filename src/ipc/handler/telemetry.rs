...
/// `tasty.telemetry.anomaly.{ts:013}.{id}` 키로 Global scope 에 저장.
fn persist_anomaly(anomaly: &Anomaly) -> std::result::Result<(), String> {
...
fn fire_anomaly_notification(state: &mut AppState, anomaly: &Anomaly) {
    let Some(ws) = state.engine.workspaces.get(state.active_workspace) else {
        return;
    };
    let ws_id = ws.id;
...
// ============================================================
// Session summary — Phase 4.5
// ============================================================

/// `telemetry.session_summary` — workspace 별 활동 요약. raw event / approval /
/// anomaly 를 시간 윈도우로 모아 토큰·IPC·승인·이상 신호를 합산한다.
///
/// 파라미터:
/// - `workspace_id` (선택, 기본 = 활성 워크스페이스)
/// - `since` / `until` (선택, unix ms)
/// - `format` ∈ {`markdown`(기본), `json`}
/// - `top_n` (선택, 기본 10) — IPC 메서드 top-N
///
/// 응답은 format 에 따라 `{ format, summary: <string> }` 또는 `{ format,
/// summary: <object> }`. 본 구현은 LLM 없이 순수 집계 — 결정론적이라 테스트
/// 친화적이다. 영속 (`tasty.telemetry.session_summary.{ws}.{ts}`) 은 plan 의
/// "옵션" 으로, 본 sub-phase 에선 생략한다.
...
fn render_summary_markdown(s: &SessionSummary) -> String {
    use std::fmt::Write;
