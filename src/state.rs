...
pub struct AppState {
    ...
    /// `fullscreen::drain_on_close_hooks` 가 정확히 1 회 발화시킨다(ADR-0036 패턴).
    #[cfg(feature = "gui")]
    ...
    /// 이번 프레임 Esc 를 소비할 자격이 있는 host popup(ADR-0036).
    ...
    /// (ADR-0031, [`crate::plugin_bridge::user_navigation`]).
    #[cfg(feature = "gui")]
    ...
    /// popup 은 그 위에 ADR-0022의 비동기 host→plugin push 결과(git-viewer 원격 조회
...
impl AppState {
    ...
    /// 이 plugin popup instance 가 연 host popup(자식)이 아직 살아 있는가 (ADR-0036).
    ///
    ...
    /// 전체화면 무대 종료 — **닫는 경로 전부가 지나는 유일한 지점**(ADR-0036 패턴).
