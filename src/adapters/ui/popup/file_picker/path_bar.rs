...
/// 디자인 토큰을 이 프레임의 배율로 잰다.
///
/// 생성 상수는 배율 1 의 값이다 — `Theme` 은 `tasty-design-tokens` 에 의존하지 않아
/// 이것들을 접근자로 들 수 없고, 본체는 배율을 토큰에만 걸므로(ADR-0135) 여기서 한 번
/// 곱한다. 값 자체는 export 가 정한다.
fn scaled(v: LogicalPx, th: &Theme) -> f32 {
    (v.value() * th.ui_zoom).round()
}

/// 이 프레임의 치수 — 전부 `component.fp-*` 토큰이다.
fn caps(th: &Theme) -> Caps {
    use tasty_design_tokens::generated::component::fp;
    Caps {
        crumb_max: scaled(fp::CRUMB_MAX_WIDTH, th),
        ancestor_min: scaled(fp::CRUMB_MIN_WIDTH, th),
        current_min: scaled(fp::CRUMB_CURRENT_MIN_WIDTH, th),
        hysteresis: scaled(fp::BAR_HYSTERESIS, th),
    }
}
...
fn menu_band(th: &Theme) -> (f32, f32) {
    use tasty_design_tokens::generated::component::fp;
    (
        scaled(fp::CRUMB_MENU_MIN_WIDTH, th),
        scaled(fp::CRUMB_MENU_MAX_WIDTH, th),
    )
}
