...
fn draw_body(
    ...
    ui.vertical(|ui| {
        ...
        match view {
            SshView::List => {
                if let Some(a) = draw_list(ui, th, profiles) {
                    action = a;
                }
            }
            SshView::Form => {
...
fn draw_list(ui: &mut egui::Ui, th: &Theme, profiles: &SshProfiles) -> Option<SshAction> {
    let mut out = None;
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        for (i, p) in profiles.profiles.iter().enumerate() {
            if let Some(a) = draw_profile_row(ui, th, i, p) {
                out = Some(a);
            }
        }
    });
...
/// 한 프로필 행: name(+label) / destination / port + Edit/Delete.
fn draw_profile_row(
    ui: &mut egui::Ui,
    th: &Theme,
    idx: usize,
...
fn draw_form(ui: &mut egui::Ui, th: &Theme, ui_state: &mut SshUiState) -> Option<SshAction> {
    ...
    egui::Grid::new("ssh_tool.form_grid")
        ...
        .show(ui, |ui| {
            ...
            // port_mode 드롭다운.
            field_label(ui, th, t("ssh_tool.field_port_mode"));
            egui::ComboBox::from_id_salt("ssh_tool.port_mode")
                .selected_text(form.port_mode.clone())
                .show_ui(ui, |ui| {
                    for mode in PORT_MODES {
                        ui.selectable_value(&mut form.port_mode, (*mode).to_string(), *mode);
                    }
                });
            ui.end_row();
        });
...
fn build_profile(form: &SshForm, base: Option<&SshProfile>) -> SshProfile {
    ...
    p.port_mode = if PORT_MODES.contains(&form.port_mode.as_str()) {
        form.port_mode.clone()
    } else {
        "auto".to_string()
    };
    p
