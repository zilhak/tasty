...
pub fn debug_state_json(
    ...
    let sections_json: Vec<Value> = sections
        ...
        .map(|s| {
            json!({
                ...
                // ADR-0619: 빈 조합 섹션은 draw 가 "바인딩 없음" 플레이스홀더로 렌더한다.
    ...
    // ADR-0619 이후 빈 조합 홀드도 빈 섹션(플레이스홀더)이 남아 sections 가 비지 않으므로,
...
pub fn draw_modifier_hint(
    ...
    // 방어적 가드 — ADR-0619 이후 홀드 가능한 실경로에선 항상 최소 홀드 조합 자신의
...
fn draw_section(
    ...
    // 리스트를 과하게 늘어뜨리지 않게 한다(ADR-0619, 디자인 §6-5). 섹션 간 간격은 불변.
    ...
    if sec.is_empty() {
        // 바인딩·역할이 모두 없는 조합 → "바인딩 없음" 플레이스홀더 한 줄(ADR-0619).
...
mod tests {
    ...
    /// ADR-0619: 바인딩·역할이 전무한 조합을 홀드해도 오버레이는 visible 이고, debug 덤프의
