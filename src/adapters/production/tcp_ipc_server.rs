...
// 명령 큐 입장 상한과 이 파일의 상한들 사이의 관계(ADR-0006). 값은 파생이 아니고 관계만
...
impl TcpIpcServer {
    ...
    /// 있고, 그 호스트의 **살아 있는** 완료 로그를 지우게 된다(ADR-0041의 호스트 세대별 로그 보존 규칙).
    /// 그래서 그때는 `None`(청소 안 함)을 준다. 뿌리를 포트 파일 쪽 `notify/` 로 옮기지
    /// 않는 이유는 writer 가 그곳에 안 쓰기 때문이다 — 아무도 안 쓰는 디렉토리를 지우는
    /// 것은 청소가 아니다. 근거·대안: `docs/adr/0041-agent-state-and-completion.md`.
    ///
    ...
    /// `docs/adr/0021-occupancy-and-attach-admission.md`.
    ///
    ...
    /// (단계 6) 둘 중 하나. bulk 전용 연결(ADR-0022)이면 hub 에 결속을 등록한다 —
    ...
    ///
    /// 예전에는 이 자리가 무응답 종료였다(ADR-0006). client 는 닫힌 소켓만 보았고, 자기
    ...
    fn await_dispatch_response(
        ...
        // 답이든 상한에서 스스로 만든 답이든 호출자가 본 것이 적힌다(ADR-0008).
...
mod admission_tests {
    ...
    // 만료 순간 요청은 시작 전이라 답이 "실행 안 됨" 으로 갈린다(ADR-0007).
    ...
    // 명령을 꺼내도 실행되지 않는다 — 기다리는 쪽이 물러나며 막았기 때문이다(ADR-0007).
    #[test]
    ...
    // 장부가 재는 것은 대기열이지 처리 중인 일이 아니다(ADR-0006). 지금은 소비자가 꺼낸
...
mod notify_cleanup_tests {
    ...
    // 완료 로그가 다른 호스트의 로그를 지우지 않는지 검사한다(ADR-0041). 같은 데이터 루트에 기본 포트 파일로 뜬 호스트 A
