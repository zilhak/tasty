//! `terminal.*` IPC 핸들러 — 호스트 내재화된 child-terminal 관리 (ADR-0021).
//!
...
//!
//! **soft 점유 (ADR-0021 soft tier 소비)**: spawn 성공 시 child 를 `occupy_soft(child, parent)` 로 등록,
...
/// 때문에 키 집합과 값이 구조적으로 일치한다 — ADR-0041이 판정을 한 헬퍼
...
pub(crate) fn handle_kill(
    ...
    // soft 점유 해제(ADR-0021): surface.close 이전에 명시적 release. tier 무관 강제
...
mod tests {
    ...
    /// 소비자가 확정 판정과 휴리스틱을 구분할 수 없다(ADR-0041이 분리해 둔 축이
    /// 응답 단계에서 사라진다).
    #[test]
    ...
    /// 목록과 단건이 **같은 판정 3 축**을 보고해야 한다. ADR-0041이 판정 헬퍼를
