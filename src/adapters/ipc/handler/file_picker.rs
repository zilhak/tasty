//! `file_picker.trigger` IPC(ADR-0636) — plugin 이 host 소유 `file_picker`
...
//! (ADR-0622)와 동일한 "즉시 ack + 이벤트 push" shape.
//!
//! # 동시성 정책
//!
...
//! 크래시는 아니지만, 접수한 요청에 결과를 돌려줘야 한다는 ADR-0636의
...
struct FilePickerTriggerReq {
    ...
    /// (부모가 자식보다 먼저 닫히지 않게, 부모가 닫히면 자식을 정리; ADR-0636).
...
mod tests {
    ...
    fn trigger_from_plugin_records_requester() {
        ...
        // 신고하지 않으면 부모 없음 — popup 밖에서의 호출과 같은 취급(ADR-0636).
        assert_eq!(req.owner_popup_instance, None);
    }

    /// `owner_popup_instance` 를 실으면 부모-자식 스택이 성립한다(ADR-0636).
    #[test]
    ...
    /// 남는 효과가 사용자 입력 포커스를 가져가는 것뿐이었다(원칙 1 ①·2.3, ADR-0631).
    #[test]
