//! soft 점유(ADR-0021)의 호스트 내부 core 결선.
//!
...
//! 뿐이라 전부 in-process 호출이다(ADR-0021: 살아있는 임의 surface 를
...
impl CoreState {
    /// soft 점유 획득(표시만, write 차단 없음 — ADR-0021). 주체 = `parent` surface.
    ...
    /// soft 점유 self-release(ADR-0021: 주체 본인 해제). 주체(`parent`) 불일치 →
    ...
    /// surface 의 점유를 tier 무관 강제 해제(로컬 사용자 force-detach 공용, ADR-0021).
    ...
    /// focus 지연 청소(ADR-0021 해제·수명): **실 사용자 포커스**를 얻은 surface 가
