...
//! 전선 안에 새로 늘려도 실패한다.
//!
//! 전선은 경계마다 사유가 달라 한 줄이 아니다 — 사유가 다르면 줄도 다르고, 한 줄로
//! 뭉치면 어느 쪽이 줄었는지가 안 보인다. 실제로 그렇게 갈라 놓은 덕에 egui 쪽 경계
//! (값이 `f32` 기하와 섞이는 것)가 먼저 0 이 된 것이 보였고, 지금 남은 둘은 winit 쪽
//! 경계(좌표가 `f64` 로 들어오는 것)뿐이다.
//!
...
//!
//! # 전선은 선언이 아니라 관문을 넓혀야 줄어든다
//!
//! 남은 것을 선언 한 줄씩 `LogicalPx` 로 바꾸면 전선이 0 이 될 것 같지만, 그렇게 하면
//! 덧셈 한복판에 `.value()` 가 **새로 생긴다** — 상대가 상수가 아니기 때문이다. 그
//! 상대는 두 종류이고 처방이 서로 다르다: egui 가 f32 로 주고 f32 로 받는 사각
//! 기하(`rect.width()`·`vec2`·`TextWrapping.max_width`)는 정책상 벗겨도 되는 **외부
//! API 경계**고, 우리가 아직 안 넓힌 f32 관문(우리 타입의 `f32` 필드, `-> f32` 헬퍼)은
//! 벗기면 안 되는 자리다. 뒤쪽이 남아 있는 동안에는 선언만 옮기는 것이 타입을 넓히는
//! 대신 타입을 버리는 자리를 만드는 일이 된다.
//!
//! 그래서 순서가 정해진다 — **관문의 시그니처를 먼저 넓히고 선언을 옮긴다.**
//! `src/adapters/ui/` 가 그렇게 닫혔다: 즐겨찾기 컬럼 폭이 `TableColumnWidth`(우리
//! 위젯 크레이트의 `f32` 필드)를 건너던 것이라, 그 열거형과 `column_layout` ·
//! `compute_column_widths` 를 `LogicalPx` 로 넓힌 뒤에야 선언이 옮겨졌다. 벗기기는
//! `egui_extras::Column` 인자 자리로만 옮겨갔다(`docs/concepts/typed-length.md`
//! "외부 API 경계에서만 `.value()`").
//!
//! 남은 두 줄은 성질이 다르다 — winit 이 좌표를 `f64` 로 주는 경계라, 넓힐 우리 관문이
//! 그 사이에 없다.
//!
//! # 무엇을 스캔하고, 무엇을 안 세는가
//!
