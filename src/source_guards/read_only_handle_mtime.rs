...
///
/// ## 모수: 4 → 1 → 2 → 3 → **4** — 그리고 그때마다 이 표의 역할이 바뀐다
///
...
///
/// 파일 감시자가 mtime 대신 **내용 지문**으로 판정하게 바뀌면서, 그 판정이 *같은 mtime
/// 으로 다시 쓴 파일*을 보는지를 시험이 물어야 했다. 그러려면 다시 쓴 뒤 mtime 을 **원래
/// 값으로 되돌려야** 하고, 그것이 SDK `file_watch.rs` 의 사이트다(그 감시자가 plugin
/// 공용으로 올라가면서 markdown 에서 그리로 옮겨졌다). 여기도
/// `OpenOptions::new().write(true)` 로 연다 — 이 가드가 겨누는 함정은 그 자리에도 그대로
/// 적용된다.
///
/// ## 사이트가 넷이 된 이유 (2026-09-20)
///
/// 워크스페이스 번들 sync 가 debug 빌드로 제한되면서(`bundle_root_from_exe_dir`), 그 분기를
/// 두 프로필에서 각각 물어야 해 시험이 `builtin/bundle_selection_tests.rs` 로 갈라져 나왔다.
/// 그 시험은 "스테이징된 산출물이 **내용 판정으로** 갱신되는가" 를 묻고, 내용 판정이 시계에
/// 안 속는 것을 보이려면 스테이징본의 mtime 을 **거짓으로 세워야** 한다 — 그것이 그 파일의
/// 사이트다. `File::options().write(true)` 로 열므로 이 가드의 함정에는 안 걸린다.
///
/// ☆ 네 사이트 중 셋(`builtin.rs` · `file_watch.rs` · `builtin/bundle_selection_tests.rs`)이
/// **같은 이유로** 생겼다: 판정을 시계에서 내용으로 옮기면, 그 판정이 시계에 안 속는다는 것을
/// 보이려고 시험이 시계를 거짓으로 세운다. 그 형태가 또 오면 이 표에 줄이 하나 더 는다 —
/// 그때 이 문단을 늘려라.
///
/// ★ **이 표를 움직이는 것은 출하 코드만이 아니다.** 넷 다 `#[cfg(test)]` 안의 시험 헬퍼이고,
...
mod population_mutations {
    ...
    ///
    /// 모수가 1 을 넘어 이 변이는 **하한이 못 잡는다** — 한 파일이 통째로 사라져도 다른 파일의
    ...
    fn a_lost_file_is_named_not_just_counted() {
        ...
        // 지금 모수(4)에서는 **하한이 조용하다** — 그것이 이 표가 되찾은 폭이다.
        // 이 줄이 깨졌다면 모수가 다시 1 로 줄어 두 판정이 겹친 것이고, 그때는 doc 의
        // "모수: 4 → 1 → 2 → 3 → 4" 절과 이 주석을 그 실측으로 함께 고쳐라.
