...
pub(crate) struct App {
    ...
    /// Lua 워커 엔진 (ADR-0027). 부팅 시 1회 생성, VM 은 전용 워커 스레드 소유.
    ...
    /// winit 이 map 을 알리는 첫 `WindowEvent::Focused(_)` 에서 비운다(ADR-0017). 그 전에 창이
...
impl App {
    ...
    pub(crate) fn create_gpu_state(
        ...
        // 모달은 열릴 때 새로 만들어지므로 이 한 지점이 전부를 덮는다(ADR-0015).
