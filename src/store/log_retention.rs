...
//! 아래에 **파일로** 쌓이는 로그는 여기 못 들어온다 — [`LogRetention`] 이 키 prefix 와
//! `MemoryStorage` 를 좌변으로 쓰는데 파일에는 prefix 도 행 수도 없다. 그래서 정책이
//! 각자의 구현 옆에 산다. 아는 자리를 **수를 못 박지 않고** 열거한다(세어 두면 다음에
//! 하나가 늘 때 그 수가 조용히 거짓이 된다).
//!
