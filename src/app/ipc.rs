...
/// 것이 이 경로다. 그 구간이 실제로 있는지는 이 머신(x11)에서 잴 수 없다(ADR-0007).
...
impl App {
    ...
    fn ipc_dispatch_command(&mut self, cmd: crate::ipc::server::IpcCommand) -> IpcStep {
        ...
        // 기한이 큐에서 지났으면 실행하지 않고 답한다 — 게이트보다 앞이다(ADR-0007).
    ...
    /// 않으면 입력 주입 · `debug.lua.eval` 이 보존소 없이 실행된다(ADR-0005). 두 step 을 한
