//! Lua 워커가 발행한 [`HostCommand`] drain → 메인 스레드에서 적용 (ADR-0627).
//!
...
impl App {
    ...
    /// 읽기전용 트리 스냅샷을 워커에 발행한다 (ADR-0627 읽기 = 스냅샷).
    ///
    ...
    /// 프레임 예산을 침해하면 증분/lazy 발행으로 재검토(ADR-0627 Reconsideration Triggers).
