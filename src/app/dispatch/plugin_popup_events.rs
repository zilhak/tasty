...
/// 닫히는 plugin popup 이 부모인 host `file_picker`(자식)를 함께 정리한다(ADR-0036).
///
...
pub(crate) fn cancel_child_file_picker(
    ...
    if data.result.is_none() {
        ...
        // tick 의 result push 가 "소유 popup 이 사라진 채 결과가 도착했다"(ADR-0036
...
impl App {
    ...
    /// 부모 없이 떠 있는 고아가 된다 — ADR-0036이 "부모가 어떤 경로로
