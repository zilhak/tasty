...
- `tasty surface cursor-position|foreground-process|locate|respawn-terminal|fire-hook --surface <id>` — 커서 위치·전경 프로세스·소속 패인(과 생존 여부)·셸 재시작·훅 발화의 CLI 진입점. 전부 IPC 에만 있던 조회/조작이다.
...
- `~/.tasty/hook-failures.log` 의 `reason` 이 `general.language` 설정을 따라 한국어/일본어로 기록되던 것을 **영어 고정**으로 바꿨다. 셸 래퍼가 exit code 를 버려 이 파일이 hook 전달 실패의 유일한 흔적인데, 읽는 주체가 에이전트일 수 있어 문구가 사용자 설정에 따라 흔들리면 알려진 실패 패턴과 대조할 수도 `grep` 할 수도 없었다. **사용자에게 보이는 stderr 는 종전대로 번역문**이다 — 같은 실패의 두 산출물이 대상에 따라 언어를 달리한다. 레코드 형식(`key=value` 한 줄)은 그대로다.
