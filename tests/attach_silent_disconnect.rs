...
//! lock 이 heartbeat TTL 만료로 자동 해제되는지 검증(ADR-0021). 프로토콜 상세는
...
//! 그 초록을 GUI self-attach 거절의 증거로 세지 않는다(docs/dev-guide/attach-behavior.md#self-attach-거절-검증).
//!
...
//! 근거는 `docs/adr/0021-occupancy-and-attach-admission.md`.
...
fn silent_disconnect_releases_occupancy_via_heartbeat_ttl() {
    ...
    // `release_all_for_client` 체인만으로 동작 — ADR-0021 핵심 가정 검증).
...
// ───── 실패한 attach 는 점유를 남기지 않는다 (ADR-0021) ─────
...
/// Historical RTT notice threshold, retained only for diagnostics (docs/dev-guide/attach-behavior.md#self-attach-거절-검증).
