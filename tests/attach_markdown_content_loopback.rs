//! markdown mirror(ADR-0622) — 원격 attach 채널의 `markdown` role 직렬화와
...
//! 같은 자리를 mesh 채널로 검증했고, markdown 이 그 채널을 벗어나며(ADR-0629) 삭제됐다.
...
/// 하이브리드 신뢰 모델(ADR-0622): attach 점유가 유일한 인가
