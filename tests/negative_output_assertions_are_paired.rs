...
const ROOT_TESTS_FLOOR: Floor = Floor {
    ...
    why_this_gap: "루트 통합 테스트 타깃의 감소를 도달 가능한 창에서 다시 실측했다 \
                   ...
                   하나**다. ★ 여유 25(좌변 46, 2026-09-23) 는 그 최대 감소 **하나를 넘는다**(9 가 \
                   남는다) — 두 번 겹치면(32) 하한이 짖고, 그때 처방은 하한을 내리는 것이 아니라 다시 재는 것이다. \
...
const CRATE_TESTS_FLOOR: Floor = Floor {
    ...
    why_this_gap: "크레이트 60 개 중 15 가 통합 테스트를 갖는다(2026-09-23 `cc2e5e72e` \
                   ...
                   하한을 움직여야 하고 그것은 판단이라 여기서 안 한다. 이 자리가 루트보다 \
                   여유가 큰 것은 '크레이트가 통째로 \
...
const EXCEPTIONS: &[(&str, &str, &str)] = &[
    // 통합에서 처음 보였다. 둘 다 **게이트 출력이 아니라 소스 텍스트**를
    ...
    // 그 뒤 통합에서 보였고, **앞의 둘과도 모양이 다르다.** 앞의 둘은 소스 원문을
