...
# 있게 미달 갈래가 내역을 전량 찍는다. 독립 계측기를 상시 대조로 두는 안은 기각했다 —
# 실측(2026-09-09): 손으로 짠 러스트 렉서가 `tokei` 와 네 파일에서 정확히 일치했는데
# `render.rs` 한 곳만 차 6 이었고 **거기서는 tokei 가 옳았다**(doc 코드펜스를 Markdown
# 으로 가르는 쪽이 맞다). 상시 대조로 두면 옳은 측정에 판정 불가를 만들어 낸다.
# 근거·대안·재검토 조건: docs/adr/0258-the-measured-copy-is-neutralized-for-the-counter.md
