...
#
# 규칙의 목적은 **발행된 값과 산출물의 짝**이다 — bump 가 정하는 것은 반영 여부가 아니라
# 그 산출물이 어느 버전으로 발행됐는가다. `upgrade-builtins` 재sync 는 버전이 갈래를 고르고
# 그 안에서 내용이 판정한다: 같은 버전이면 내용이 다른 파일만 옮기고, 번들이 높으면 전량
# 덮어쓰고, 설치본이 더 높으면 건너뛴다. 그러니 bump 를 빠뜨려도 반영은 되지만, 같은 버전
# 문자열이 서로 다른 두 산출물을 가리키게 된다(`plugin.list`·업그레이드 판정·배포 아카이브가
# 그 문자열을 믿는다). 그래서 판정 기준도 "파일이 staged 되었는가"
...
# 없어 판정 대상에서 자연히 빠졌다 — 버전 그대로인 채 내용만 다른 산출물이 발행된다.
# `upgrade-builtins` 는 같은 버전이면 내용으로 재sync 하므로 파일은 옮겨 가지만, **같은 버전
# 문자열 아래 두 산출물**이 남아 무엇이 발행됐는지가 값으로 안 남는다. 빌드도 테스트도
# 초록인데 그 짝만 깨진다.
#
...
#
# 끝점 비교인 이유: 목적이 "발행될 때 version 이 올라 있는가" 라, 한 lane 의 커밋마다
# 올릴 필요가 없다. 발행 전에 한 번 오르면 값과 산출물의 짝이 맞는다.
#
