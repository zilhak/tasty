...
# 파서라 리터럴 내용을 보존한다.
#
# 실측(가드 이후 창, 이탈 41 쌍)에서 세 정규화를 **양극성으로** 비교했다:
#
#     정규화              부채로 남음   오탐으로 배제   비고
#     exact                    40             0
#     공백 전부 제거            36             4         배제된 4 는 전부 `style:` 커밋
#     rustfmt                  34             6         배제된 6 도 전부 `style:` 커밋
#
# rustfmt 정규화가 **삼킨 6 쌍은 전부 `style: cargo fmt` 커밋**이고, 남은 29 커밋은
# fix 9 · docs 8 · feat 7 · refactor 5 로 하나도 안 삼켰다 — 커밋 type 이라는 독립 축이
# 같은 경계선을 그린다.
#
