...
#
# 이 파일은 `source` 되는 라이브러리라 shebang 이 없다 — 실행 파일이 아니다.
# 정적 검사기는 shebang 으로 대상 셸을 고르므로, 없으면 어느 셸의 규칙을 적용할지
# 모른 채 검사를 줄인다. 위 지시자가 그 자리를 대신한다(shebang 을 붙이면 이
# 파일이 직접 실행 가능한 것처럼 보이는데, 그것은 사실이 아니다).
...
# 이유: source 한 게이트가 읽는 값이다 — 이 파일 안에는 소비자가 없다.
...
resolve_judge() {
    ...
    # 이유: 이 함수의 반환 통로다. source 한 게이트가 이 이름으로 받아 간다
    # (표준출력으로 돌려주면 판정기의 stderr 와 섞인다 — 위 머리말 참조).
    # shellcheck disable=SC2034
    JUDGE_BIN="$_jb_bin"
}
