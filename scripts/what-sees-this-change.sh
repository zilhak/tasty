...
#        이름을 소스에 **안 적는다**. `crates/tasty-doc-guards/tests/no_early_exit_consumer_in_shell_pipes.rs` 가
...
# ★ 주어진 이름이 아니라 **갈린 지점**을 쓴다. `main` 은 공유 ref 라 다른 lane 의 착지가
#   계속 밀고 있고, 그 상태에서 두 끝점을 비교하면 **남이 넣은 것까지 역방향으로** 변경집합에
#   들어온다. 그러면 이 도구는 안 건드린 파일을 잔뜩 내고, 그 목록은 "돌려 보라" 로 읽힌다.
#   실측 2026-09-07: 조립 중 `main` 이 회차 base 보다 61 커밋 앞서 있었다.
...
if [ "$(git rev-parse "$BASE")" != "$MERGE_BASE" ]; then
    ...
    BASE_SHOWN="$(git rev-parse --short=9 "$MERGE_BASE")  (★ '$BASE' 가 ${ahead} 커밋 앞서
            있어 갈린 지점으로 되돌렸다 — 두 끝점 비교였다면 남의 착지까지 셌다)"
fi
BASE=$MERGE_BASE
