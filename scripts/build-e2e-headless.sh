...
#
# ★★ 그리고 지금 두 층은 **한 눈금 어긋나 있다.** 위층은 동률(`==`)을 낡음으로 세도록
# 고쳤는데(판정 불가를 실패 방향으로), `find -newer` 는 엄격 초과라 동률을 통과시킨다.
# ⇒ 소스가 바이너리와 같은 눈금에 떨어지면 **여기서는 통과하고 하네스에서 패닉한다.**
# 안전하지만 인체공학이 깨진 상태이고, 이 줄의 판정을 맞추는 것은 별도 단위다.
stale=$(find "$ROOT/src" "$ROOT/crates" -name '*.rs' -newer "$BIN" -print -quit 2>/dev/null)
if [ -n "$stale" ]; then
