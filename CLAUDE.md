...
| 파일 SLOC (복잡도 게이트 파트 B) | `bash scripts/check-file-size.sh` | ✅ 자동 — `complexity-check.yml`(main push — **문서·site 만 담은 push 는 제외** · PR · 수동). **4 초면 끝나는데 커밋 전 채널이 없다** — pre-commit 도 pre-push 도 이것을 안 부르므로 위반은 push 하고 나서야 드러난다(실측 2026-09-13: 한 회차에서 새로 임계를 넘긴 파일 3 개가 전부 이 방식으로 드러났다). 처방은 **모듈 분할**이다 — `.complexity-file-allowlist` 등재는 그 파일 자신이 "부채 대장" 이라 부르는 자리이고, 등재하면 `frozen-sum-budget` 을 같은 커밋에 올려야 하며(`scripts/check-frozen-sum-ratchet.sh` 가 양방향으로 고정한다) 그 뒤로는 그 파일이 얼마나 자라든 **아무 신호도 안 난다**. 임계·동결 래칫의 근거는 [ADR-0168](docs/adr/0168-the-file-sloc-threshold-is-not-derived-and-the-freeze-ratchets-one-way.md), 트리거 이력은 [ADR-0131](docs/adr/0131-file-sloc-gate-needs-a-firing-trigger.md) |
