...
Tasty 는 cargo workspace 다 (본 바이너리 + `crates/*` 50 개 — 그중 `tasty-plugin-sdk-wasm` 은 workspace `exclude`). 빌드 프로필 3 종 (`dev` / `release` / `dist`).
...
| 셸 게이트 (Intent 규율 · 사유 없는 `#[allow]`) | `bash scripts/check-intent-discipline.sh` · `bash scripts/check-allow-reason.sh` | ✅ 자동 — `script-gates.yml`(main push · PR). 컴파일이 필요 없는 텍스트 스캔이라 초 단위로 끝난다. 뒤쪽은 잔여가 0 이 아니라 **상한 래칫**이다 — 늘어도 실패하고, **줄어도 실패한다**(상한을 같이 내리라는 뜻: 남는 여유가 곧 안 보는 구간이다) |
