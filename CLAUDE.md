...
| 셸 게이트 (Intent 규율 · 사유 없는 `#[allow]`) | `bash scripts/check-intent-discipline.sh` · `bash scripts/check-allow-reason.sh` | ✅ 자동 — `script-gates.yml`(main push · PR). 컴파일이 필요 없는 텍스트 스캔이라 초 단위로 끝난다. **뒤쪽은 리포트 전용이라 건수와 무관하게 `exit 0` 한다** — 그 스텝의 초록은 "위반이 없다" 가 아니라 "스캐너가 아직 돈다" 는 뜻이다 |
